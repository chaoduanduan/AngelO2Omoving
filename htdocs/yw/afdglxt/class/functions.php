...
function GetModTable($mid){
	...
	$r=$Avatar->fetch1("select tid,tbname from {$dbtbpre}enewsmod where mid='$mid'");
	return $r;
}
...
function DelListEnews(){
	$file=ECMS_PATH."afdglxt/data/fc/ListEnews.php";
	DelFiletext($file);
	$file1=ECMS_PATH."afdglxt/data/fc/ListClass0.php";
	DelFiletext($file1);
	$file2=ECMS_PATH."afdglxt/data/fc/ListClass1.php";
	DelFiletext($file2);
}

//删除模板临时缓存文件
function DelOneTempTmpfile($classid){
	$file=ECMS_PATH.'afdglxt/data/tmp/dt_temp'.$classid.'.php';
	if(file_exists($file))
	{
		DelFiletext($file);
	}
...
function ReturnCheckDoRep(){
	global $Avatar,$dbtbpre;
	//信息来源
	$befrom=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsbefrom");
	//作者
	$writer=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewswriter");
	//替换字符
	$words=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewswords");
	//内容关键字
	$key=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewskey");
...
function CheckTempGroup($gid){
	...
	$gid=(int)$gid;
	$r=$Avatar->fetch1("select gid,gname from {$dbtbpre}enewstempgroup where gid='$gid'");
	if(empty($r['gid']))
	{
		printerror("ErrorUrl","");
	}
...
function ReplaceKey($newstext,$classid=0){
	global $Avatar,$dbtbpre,$public_r,$class_r;
	...
	$sql=$Avatar->query("select keyname,keyurl from {$dbtbpre}enewskey".$where);
...
function ReplaceWord($newstext){
	...
	if(empty($newstext))
	{return $newstext;}
	$sql=$Avatar->query("select newword,oldword from {$dbtbpre}enewswords");
	while($r=$Avatar->fetch($sql))
	{
...
function AddInfoToZt($ztid,$zcid,$classid,$id,$newstime,$isgood=0,$ecms=0){
	global $Avatar,$dbtbpre,$class_r;
	if($ecms==1)//检查是否增加过
	{
		$infor=$Avatar->fetch1("select zid,ztid,cid from {$dbtbpre}enewsztinfo where ztid='$ztid' and classid='$classid' and id='$id' limit 1");
		if($infor['ztid'])
		{
			if($infor['cid']!=$zcid)
			{
				$Avatar->query("update {$dbtbpre}enewsztinfo set cid='$zcid',newstime='$newstime' where zid='$infor[zid]' limit 1");
			}
		}
		else
		{
			$mid=$class_r[$classid]['modid'];
			$Avatar->query("insert into {$dbtbpre}enewsztinfo(ztid,cid,classid,id,newstime,mid,isgood) values('$ztid','$zcid','$classid','$id','$newstime','$mid','$isgood');");
		}
	}
	else
	{
...
function AddMoreInfoToZt($ztid,$zcid,$tbname,$where,$ecms=0){
	...
	$sql=$Avatar->query("select id,classid,newstime from {$dbtbpre}ecms_".$tbname.($ecms==0?'':'_index')." where ".$where);
	while($r=$Avatar->fetch($sql))
	{
		$zinfor=$Avatar->fetch1("select zid,ztid,cid from {$dbtbpre}enewsztinfo where ztid='$ztid' and classid='$r[classid]' and id='$r[id]' limit 1");
		if($zinfor['ztid'])
		{
			if($zinfor['cid']!=$zcid)
			{
				$Avatar->query("update {$dbtbpre}enewsztinfo set cid='$zcid' where zid='$zinfor[zid]' limit 1");
			}
		}
		else
		{
			$mid=$class_r[$r[classid]]['modid'];
			$Avatar->query("insert into {$dbtbpre}enewsztinfo(ztid,cid,classid,id,newstime,mid,isgood) values('$ztid','$zcid','$r[classid]','$r[id]','$r[newstime]','$mid','0');");
		}
	}
}
...
function InsertZtInfo($ztids,$zcids,$oldztids,$oldzcids,$classid,$id,$newstime){
	...
	if($zcids)
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			{
				$zcr=$Avatar->fetch1("select ztid from {$dbtbpre}enewszttype where cid='$cid' limit 1");
				if(!$zcr['ztid'])
				{
					continue;
				}
	...
	if($oldztids)
	{
		...
		for($di=0;$di<$dcount;$di++)
		{
			...
			$Avatar->query("delete from {$dbtbpre}enewsztinfo where ztid='$dztid' and classid='$classid' and id='$id'");
		}
	}
}
...
function DelZtInfo($where){
	...
	$Avatar->query("delete from {$dbtbpre}enewsztinfo where ".$where);
}

//信息送审
function InfoInsertToWorkflow($id,$classid,$wfid,$userid,$username){
	global $Avatar,$dbtbpre,$class_r;
	$wfitemr=$Avatar->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}enewsworkflowitem where wfid='$wfid' order by tno limit 1");
	//状态更新
	$Avatar->query("insert into {$dbtbpre}enewswfinfo(id,classid,wfid,tid,groupid,userclass,username,checknum,tstatus,checktno) values('$id','$classid','$wfid','$wfitemr[tid]','$wfitemr[groupid]','$wfitemr[userclass]','$wfitemr[username]',1,'$wfitemr[tstatus]',0);");
	//日志
...
function InfoUpdateToWorkflow($id,$classid,$wfid,$userid,$username){
	global $Avatar,$dbtbpre,$class_r;
	$wfinfor=$Avatar->fetch1("select checknum,wfid,tid,checktno from {$dbtbpre}enewswfinfo where id='$id' and classid='$classid' limit 1");
	...
	if($wfinfor[tid])
	{
		$ywfitemr=$Avatar->fetch1("select tno from {$dbtbpre}enewsworkflowitem where tid='$wfinfor[tid]'");
		$wfitemr=$Avatar->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}enewsworkflowitem where wfid='$wfinfor[wfid]' and tno>$ywfitemr[tno] order by tno limit 1");
	}
	else
	{
		$wfitemr=$Avatar->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}enewsworkflowitem where wfid='$wfinfor[wfid]' order by tno limit 1");
	}
	//状态更新
	$Avatar->query("update {$dbtbpre}enewswfinfo set tid='$wfitemr[tid]',groupid='$wfitemr[groupid]',userclass='$wfitemr[userclass]',username='$wfitemr[username]',checknum=checknum+1,tstatus='$wfitemr[tstatus]',checktno='0' where id='$id' and classid='$classid' limit 1");
	//日志
...
function InsertWfLog($classid,$id,$wfid,$tid,$username,$checktext,$checknum,$checktype){
	global $Avatar,$dbtbpre,$class_r,$lur;
	$checktime=time();
	$checktext=RepPostStr($checktext);
	$Avatar->query("insert into {$dbtbpre}enewswfinfolog(id,classid,wfid,tid,username,checktime,checktext,checknum,checktype) values('$id','$classid','$wfid','$tid','$username','$checktime','$checktext','$checknum','$checktype');");
}
...
function eInsertTags($tags,$classid,$id,$newstime){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$r=$Avatar->fetch1("select tagid from {$dbtbpre}enewstags where tagname='$tagname' limit 1");
		if($r[tagid])
		{
			$datar=$Avatar->fetch1("select tagid,classid,newstime from {$dbtbpre}enewstagsdata where tagid='$r[tagid]' and id='$id' and mid='$mid' limit 1");
			if($datar[tagid])
			{
				if($datar[classid]!=$classid||$datar[newstime]!=$newstime)
				{
					$Avatar->query("update {$dbtbpre}enewstagsdata set classid='$classid',newstime='$newstime' where tagid='$r[tagid]' and id='$id' and mid='$mid' limit 1");
				}
			}
			else
			{
				$Avatar->query("update {$dbtbpre}enewstags set num=num+1 where tagid='$r[tagid]'");
				$Avatar->query("insert into {$dbtbpre}enewstagsdata(tagid,classid,id,newstime,mid) values('$r[tagid]','$classid','$id','$newstime','$mid');");
			}
		}
		else
		{
			$Avatar->query("insert into {$dbtbpre}enewstags(tagname,num,isgood,cid) values('$tagname',1,0,0);");
			$tagid=$Avatar->lastid();
			$Avatar->query("insert into {$dbtbpre}enewstagsdata(tagid,classid,id,newstime,mid) values('$tagid','$classid','$id','$newstime','$mid');");
		}
	}
}
...
function eReturnInfoTags($classid,$id,$mid){
	...
	$tags='';
	$dh='';
	$sql=$Avatar->query("select tagid from {$dbtbpre}enewstagsdata where id='$id' and mid='$mid' order by tagid");
	while($r=$Avatar->fetch($sql))
	{
		$tr=$Avatar->fetch1("select tagname from {$dbtbpre}enewstags where tagid='$r[tagid]'");
		$tags.=$dh.$tr[tagname];
		$dh=',';
	}
	return $tags;
...
function MoveCheckInfoData($tbname,$checked,$stb,$where){
	...
	$Avatar->query("replace into ".$ntbname." select * from ".$ytbname." where ".$where);
	$Avatar->query("replace into ".$ndatatbname." select * from ".$ydatatbname." where ".$where);
	//删除原表
	$Avatar->query("delete from ".$ytbname." where ".$where);
	$Avatar->query("delete from ".$ydatatbname." where ".$where);
}
...
function UpdateAllDataTbField($tbname,$update,$where,$upcheck=1,$updoc=1){
	global $Avatar,$dbtbpre;
	//已审核
	$tbr=$Avatar->fetch1("select datatbs from {$dbtbpre}enewstable where tbname='$tbname' limit 1");
	if($tbr['datatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." set ".$update.$where);
		}
	}
	//未审核
	if($upcheck==1)
	{
		$Avatar->query("update {$dbtbpre}ecms_".$tbname."_check_data set ".$update.$where);
	}
	//归档
	if($updoc==1)
	{
		$Avatar->query("update {$dbtbpre}ecms_".$tbname."_doc_data set ".$update.$where);
	}
}
...
function DelAllDataTbInfo($tbname,$where,$delcheck=1,$deldoc=1){
	...
	if($tbr['datatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			$Avatar->query("delete from {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." where ".$where);
		}
	}
	//未审核
	if($delcheck==1)
	{
		$Avatar->query("delete from {$dbtbpre}ecms_".$tbname."_check_data where ".$where);
	}
	//归档
	if($deldoc==1)
	{
		$Avatar->query("delete from {$dbtbpre}ecms_".$tbname."_doc_data where ".$where);
	}
}
...
function DelFileOtherTable($where,$tb='other'){
	...
	//删除附件
	$filesql=$Avatar->query("select filename,path,modtype,fpath from {$dbtbpre}enewsfile_{$tb} where ".$where);
	while($filer=$Avatar->fetch($filesql))
	{
		DoDelFile($filer);
	}
	$Avatar->query("delete from {$dbtbpre}enewsfile_{$tb} where ".$where);
}
...
function DelFileAllTable($where){
	...
	if($public_r['filedatatbs'])
	{
		...
		{
			//删除附件
			$filesql=$Avatar->query("select filename,path,classid,fpath from {$dbtbpre}enewsfile_".$dtbr[$i]." where ".$where);
			while($filer=$Avatar->fetch($filesql))
			{
				DoDelFile($filer);
			}
			$Avatar->query("delete from {$dbtbpre}enewsfile_".$dtbr[$i]." where ".$where);
		}
	}
}
...
function DelPlAllTable($where){
	...
	if($public_r['pldatatbs'])
	{
		...
		for($i=1;$i<$count;$i++)
		{
			$Avatar->query("delete from {$dbtbpre}enewspl_".$pldtbr[$i]." where ".$where);
		}
	}
}
...
function UpdateTheFile($id,$checkpass,$classid,$fstb=1){
	...
	$sql=$Avatar->query("update {$dbtbpre}enewsfile_{$fstb} set pubid='$pubid',classid='$classid',id='$id',cjid=0 where cjid='$checkpass'");
}
...
function UpdateTheFileEdit($classid,$id,$fstb=1){
	global $Avatar,$dbtbpre;
	$pubid=ReturnInfoPubid($classid,$id);
	$sql=$Avatar->query("update {$dbtbpre}enewsfile_{$fstb} set pubid='$pubid',cjid=0 where id='$id' and classid='$classid'");
}
...
function GetInfoTranFstb($classid,$id,$fstb){
	...
	if($id)
	{
		...
		$index_r=$Avatar->fetch1("select id,classid,checked from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_index where id='$id' limit 1");
		if(!$index_r['id'])
		{
			return $public_r['filedeftb'];
		}
		...
		$infor=$Avatar->fetch1("select fstb from ".$infotb." where id='$id' limit 1");
		$fstb=$infor['fstb'];
	}
	elseif($fstb)
	{
...
function UpdateTheIspic($classid,$id,$checked){
	...
	$r=$Avatar->fetch1("select titlepic,ispic from ".$infotb." where id='$id' limit 1");
	$ispic=$r['titlepic']?1:0;
	if($ispic<>$r['ispic'])
	{
		$Avatar->query("update ".$infotb." set ispic='$ispic' where id='$id'");
	}
}
...
function GetFpicToTpic($classid,$id,$num=1,$getfirsttitlespic=0,$swidth=0,$sheight=0,$fstb=1){
	...
	$num=(int)$num;
	$num=$num-1;
	$picr=$Avatar->fetch1("select fileid,filename,path,id,classid,no,fpath from {$dbtbpre}enewsfile_{$fstb} where pubid='$pubid' and type=1 order by fileid limit $num,1");
	$firsttitlepic="";
	if($picr['fileid'])
	{
...
function UpdateImgNexturl($classid,$id,$checked=1){
	global $Avatar,$dbtbpre,$class_r,$public_r,$emod_r;
	$mid=$class_r[$classid][modid];
	...
	if($tbdataf)
	{
		$r=$Avatar->fetch1("select id,classid,titleurl,groupid,newspath,filename,stb from ".$infotbname." where id='$id'");
		$infodatatbname=$checked?$dbtbpre.'ecms_'.$tbname.'_data_'.$r[stb]:$dbtbpre.'ecms_'.$tbname.'_check_data';
		$finfor=$Avatar->fetch1("select ".$pf." from ".$infodatatbname." where id='$id'");
		$r[$pf]=$finfor[$pf];
	}
	else
	{
		$r=$Avatar->fetch1("select id,classid,titleurl,groupid,newspath,filename,".$pf." from ".$infotbname." where id='$id'");
	}
	//存文本
	if($stf&&$stf==$pf)
	{
	...
	if($tbdataf)
	{
		$Avatar->query("update ".$infodatatbname." set ".$pf."='$newstext' where id='$id'");
	}
	else
	{
		$Avatar->query("update ".$infotbname." set ".$pf."='$newstext' where id='$id'");
	}
}
...
function GetKeyid($keyboard,$classid,$id,$link_num){
	global $Avatar,$public_r,$class_r,$fun_r,$dbtbpre,$eyh_r,$etable_r;
	if($keyboard)
	{
		if(empty($link_num))
		{
		...
		{
			$mr=$Avatar->fetch1("select sonclass from {$dbtbpre}enewsmod where mid='".$class_r[$classid][modid]."'");
		...
		$keyid="";
		$first=0;
		$key_sql=$Avatar->query("select id from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where ".$yhadd.$add.$where." and id<>$id order by newstime desc limit $link_num");
		while($link_r=$Avatar->fetch($key_sql))
		{
			if(empty($first))
			{
				$dh="";
...
function DelInfoSaveTxtfile($mid,$tbname,$where){
	global $Avatar,$dbtbpre,$public_r,$class_r,$emod_r;
	if(empty($where))
	{
		return '';
	}
	...
	{
		//已审核
		$txtsql=$Avatar->query("select ".$savetxtf." from {$dbtbpre}ecms_".$tbname." where ".$where);
		while($txtr=$Avatar->fetch($txtsql))
		{
		...
		//未审核
		$txtsql=$Avatar->query("select ".$savetxtf." from {$dbtbpre}ecms_".$tbname."_check where ".$where);
		while($txtr=$Avatar->fetch($txtsql))
		{
			$newstextfile=$txtr[$savetxtf];
			//$txtr[$savetxtf]=GetTxtFieldText($txtr[$savetxtf]);
		...
		//归档
		$txtsql=$Avatar->query("select ".$savetxtf." from {$dbtbpre}ecms_".$tbname."_doc where ".$where);
		while($txtr=$Avatar->fetch($txtsql))
		{
			$newstextfile=$txtr[$savetxtf];
			//$txtr[$savetxtf]=GetTxtFieldText($txtr[$savetxtf]);
...
function DelSingleInfoOtherData($classid,$id,$r,$delfile=0,$delpl=0){
	...
	$pubid=ReturnInfoPubid($classid,$id);
	//删除其它表记录
	$Avatar->query("delete from {$dbtbpre}enewswfinfo where id='$id' and classid='$classid'");
	$Avatar->query("delete from {$dbtbpre}enewswfinfolog where id='$id' and classid='$classid'");
	$Avatar->query("delete from {$dbtbpre}enewsinfovote where pubid='$pubid'");
	$Avatar->query("delete from {$dbtbpre}enewsdiggips where id='$id' and classid='$classid'");
	$Avatar->query("delete from {$dbtbpre}enewsztinfo where id='$id' and classid='$classid'");
	if($delfile==0)
	{
...
function DelMoreInfoOtherData($classid,$delfile=0,$delpl=0){
	global $Avatar,$dbtbpre,$public_r,$class_r,$emod_r;
	//删除其它表记录
	$Avatar->query("delete from {$dbtbpre}enewswfinfo where classid='$classid'");
	$Avatar->query("delete from {$dbtbpre}enewswfinfolog where classid='$classid'");
	$Avatar->query("delete from {$dbtbpre}enewsinfovote where classid='$classid'");
	$Avatar->query("delete from {$dbtbpre}enewsdiggips where classid='$classid'");
	$Avatar->query("delete from {$dbtbpre}enewsztinfo where classid='$classid'");
	//附件
	if($delfile==0)
	{
...
function UpdateSingleInfoOtherData($classid,$id,$to_classid,$r,$updatefile=0,$updatepl=0){
	...
	$pubid=ReturnInfoPubid($classid,$id);
	//更新其它表记录
	$Avatar->query("update {$dbtbpre}enewswfinfo set classid='$to_classid' where id='$id' and classid='$classid'");
	$Avatar->query("update {$dbtbpre}enewswfinfolog set classid='$to_classid' where id='$id' and classid='$classid'");
	$Avatar->query("update {$dbtbpre}enewsinfovote set classid='$to_classid' where pubid='$pubid'");
	$Avatar->query("update {$dbtbpre}enewsdiggips set classid='$to_classid' where id='$id' and classid='$classid'");
	$Avatar->query("update {$dbtbpre}enewsztinfo set classid='$to_classid' where id='$id' and classid='$classid'");
	//附件
	if($updatefile==0)
	{
		$Avatar->query("update {$dbtbpre}enewsfile_".$r['fstb']." set classid='$to_classid' where pubid='$pubid'");
	}
	//评论
	if($updatepl==0)
	{
		$Avatar->query("update {$dbtbpre}enewspl_".$r['restb']." set classid='$to_classid' where pubid='$pubid'");
	}
}
...
function UpdateMoreInfoOtherData($classid,$to_classid,$updatefile=0,$updatepl=0){
	global $Avatar,$dbtbpre,$public_r,$class_r,$emod_r;
	//更新其它表记录
	$Avatar->query("update {$dbtbpre}enewswfinfo set classid='$to_classid' where classid='$classid'");
	$Avatar->query("update {$dbtbpre}enewswfinfolog set classid='$to_classid' where classid='$classid'");
	$Avatar->query("update {$dbtbpre}enewsinfovote set classid='$to_classid' where classid='$classid'");
	$Avatar->query("update {$dbtbpre}enewsdiggips set classid='$to_classid' where classid='$classid'");
	$Avatar->query("update {$dbtbpre}enewsztinfo set classid='$to_classid' where classid='$classid'");
	//附件
	if($updatefile==0)
	{
		if($public_r['filedatatbs'])
		{
			...
			for($i=1;$i<$count-1;$i++)
			{
				$Avatar->query("update {$dbtbpre}enewsfile_".$dtbr[$i]." set classid='$to_classid' where classid='$classid'");
			}
		}
	}
	//评论
	if($updatepl==0)
	{
		if($public_r['pldatatbs'])
		{
			...
			for($i=1;$i<$count;$i++)
			{
				$Avatar->query("update {$dbtbpre}enewspl_".$pldtbr[$i]." set classid='$to_classid' where classid='$classid'");
			}
		}
	}
}
...
function DelNewsTheFile($id,$classid,$fstb='1',$delpl=0,$restb='1'){
	...
	$pubid=ReturnInfoPubid($classid,$id);
	$i=0;
	$sql=$Avatar->query("select classid,filename,path,fpath from {$dbtbpre}enewsfile_{$fstb} where pubid='$pubid'");
	while($r=$Avatar->fetch($sql))
	{
		$i=1;
		DoDelFile($r);
	...
	if($i)
	{
		$Avatar->query("delete from {$dbtbpre}enewsfile_{$fstb} where pubid='$pubid'");
	}
	//删除评论
	if($delpl==0)
	{
		$Avatar->query("delete from {$dbtbpre}enewspl_{$restb} where pubid='$pubid'");
	}
}
...
function DelNewsFile($filename,$newspath,$classid,$newstext,$groupid=0){
	...
	$r=explode("[!--Avatarnews.page--]",$newstext);
	for($i=1;$i<=count($r);$i++)
	{
		if(strstr($filename,'/'))
		{
...
function DelZtcFile($cid){
	global $Avatar,$dbtbpre,$class_zr;
	$cr=$Avatar->fetch1("select ztid,islist,maxnum,tnum,ttype from {$dbtbpre}enewszttype where cid='$cid'");
	if(!$cr['ztid'])
	{
		return '';
	}
	...
	//数量
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsztinfo where cid='$cid'");
...
function CopyImg($text,$copyimg,$copyflash,$classid,$qz,$username,$theid,$cjid,$mark,$fstb=1){
	global $Avatar,$public_r,$cjnewsurl,$navtheid,$dbtbpre;
	if(empty($text))
	{return "";}
...
function GetMySmallImg($classid,$no,$insertfile,$filepath,$yname,$maxwidth,$maxheight,$name,$id,$cjid,$userid,$username,$modtype=0,$fstb=1){
	global $Avatar,$dbtbpre,$public_r,$efileftp_fr;
	if(empty($yname))
	{
		return "";
    }
...
function ShowClass_AddClass($adminclass,$obclassid,$bclassid,$exp,$modid,$enews=0,$addminfocid=''){
	...
	if(empty($bclassid))
	{
		...
		if($enews==2)
		{
			$modr=$Avatar->fetch1("select sonclass from {$dbtbpre}enewsmod where mid='$modid'");
			$addminfocid=$modr['sonclass'];
		}
    }
	else
	{$exp="&nbsp;&nbsp;".$exp;}
	$sql=$Avatar->query("select classid,classname,bclassid,islast,openadd,modid,sonclass from {$dbtbpre}enewsclass where bclassid='$bclassid' and wburl='' order by myorder,classid");
	$returnstr="";
	while($r=$Avatar->fetch($sql))
	{
		//投稿显示
...
function GetListtempMid($tempid){
	global $Avatar;
	$r=$Avatar->fetch1("select modid from ".GetTemptb("enewslisttemp")." where tempid='$tempid'");
	return $r[modid];
}
...
function GetListTemp($tempid){
	global $Avatar;
	$r=$Avatar->fetch1("select temptext,subnews,listvar,rownum,showdate,modid,subtitle,docode from ".GetTemptb("enewslisttemp")." where tempid='$tempid'");
...
function GetClassTemp($tempid){
	global $Avatar;
	$r=$Avatar->fetch1("select temptext from ".GetTemptb("enewsclasstemp")." where tempid='$tempid'");
	return $r['temptext'];
}
...
function GetClassText($classid){
	global $Avatar,$dbtbpre;
	$r=$Avatar->fetch1("select classtext from {$dbtbpre}enewsclassadd where classid='$classid'");
	return $r['classtext'];
}
...
function GetZtText($ztid){
	global $Avatar,$dbtbpre;
	$r=$Avatar->fetch1("select classtext from {$dbtbpre}enewsztadd where ztid='$ztid'");
	return $r['classtext'];
}
...
function GetZtcText($cid){
	global $Avatar,$dbtbpre;
	$r=$Avatar->fetch1("select classtext from {$dbtbpre}enewszttypeadd where cid='$cid'");
	return $r['classtext'];
}
...
function GetIndextemp(){
	...
	{
		$r=$Avatar->fetch1("select temptext from {$dbtbpre}enewsindexpage where tempid='".$public_r['indexpageid']."'");
		return $r['temptext'];
	}
	$r=$Avatar->fetch1("select indextemp from ".GetTemptb("enewspubtemp")." limit 1");
	return $r['indextemp'];
}

//取得内容模板
function GetNewsTemp($newstempid){
	global $Avatar,$public_r;
	$r=$Avatar->fetch1("select temptext,showdate from ".GetTemptb("enewsnewstemp")." where tempid='$newstempid'");
...
function GetTheJstemp($tempid){
	global $Avatar;
	$r=$Avatar->fetch1("select temptext,showdate,modid,subnews,subtitle from ".GetTemptb("enewsjstemp")." where tempid='$tempid'");
	return $r;
}
...
function ReplaceTempvar($temp){
	...
	if(empty($temp))
	{return $temp;}
	$sql=$Avatar->query("select myvar,varvalue from ".GetTemptb("enewstempvar")." where isclose=0 order by myorder desc,varid");
	while($r=$Avatar->fetch($sql))
	{
...
function AddCheckClassLevel($classid,$groupid,$classpath){
	...
	$include1=$include;
	$include.='afdglxt/class/CheckClassLevel.php';
	$addlevel="<?php
	define('Avatarcms','wm_chief');
...
function ReClassBdInfo($classid){
	global $Avatar,$dbtbpre;
	$cr=$Avatar->fetch1("select classid,bdinfoid from {$dbtbpre}enewsclass where classid='$classid'");
...
function DoSpReFile($r,$spid=0){
	...
	if($spid)
	{
		$r=$Avatar->fetch1("select varname,refile,spfile,spfileline,spfilesub from {$dbtbpre}enewssp where spid='$spid' limit 1");
	}
	if(!$r['refile'])
	{
		return '';
...
function NewsBq($classid,$indextext,$enews=0,$doing=0){
	global $Avatar,$dbtbpre,$public_r,$emod_r,$class_r,$class_zr,$fun_r,$navclassid,$navinfor,$class_tr,$level_r,$etable_r;
	...
	if($enews==0)//生成大栏目
	{
		...
		$cr=$Avatar->fetch1("select classpagekey,intro,classimg,cgroupid".$cf." from {$dbtbpre}enewsclass where classid='$classid'");
		if(!empty($cf))
		{
		...
		$onclick="<script src=".$public_r[newsurl]."afdglxt/public/onclick/?enews=doclass&classid=$classid></script>";
		$truefile=eReturnTrueEcmsPath().ReturnSaveClassPath($classid,1);//moreport
		$file=ECMS_PATH.'afdglxt/data/tmp/class'.$classid.'.php';
	...
	elseif($enews==3)//专题
	{
		...
		$cr=$Avatar->fetch1("select ztpagekey,intro,ztimg".$cf." from {$dbtbpre}enewszt where ztid='$classid'");
		if(!empty($cf))
		{
		...
		$pagedes=ehtmlspecialchars($cr['intro']);
		$classimg=$cr['ztimg'];
		$onclick="<script src=".$public_r[newsurl]."afdglxt/public/onclick/?enews=dozt&ztid=$classid></script>";
		$truefile=ECMS_PATH.ReturnSaveZtPath($classid,1);
		$file=ECMS_PATH.'afdglxt/data/tmp/zt'.$classid.'.php';
		$indextext=str_replace("[!--newsnav--]",$url,$indextext);//位置导航
		$indextext=Class_ReplaceSvars($indextext,$url,$classid,$pagetitle,$pagekey,$pagedes,$classimg,$add,1);
	}
	elseif($enews==4)//专题子类
	{
		$cr=$Avatar->fetch1("select ztid,cname,ttype from {$dbtbpre}enewszttype where cid='$classid'");
		$GLOBALS['navclassid']=$classid;
		...
		$onclick="<script src=".$public_r[newsurl]."afdglxt/public/onclick/?enews=dozt&ztid=$cr[ztid]></script>";
		$truefile=ECMS_PATH.ReturnSaveZtPath($cr['ztid'],0).'/type'.$classid.$cr['ttype'];
		$file=ECMS_PATH.'afdglxt/data/tmp/ztc'.$classid.'.php';
		$indextext=str_replace("[!--newsnav--]",$url,$indextext);//位置导航
		$indextext=Class_ReplaceSvars($indextext,$url,$classid,$pagetitle,$pagekey,$pagedes,$classimg,$add,1);
	}
	elseif($enews==1)//生成首页文件
	{
		$pr=$Avatar->fetch1("select sitekey,siteintro,indexpagedt from {$dbtbpre}enewspublic limit 1");
		...
		$file=ECMS_PATH.'afdglxt/data/tmp/index.php';
...
function InfoNewsBq($classid,$indextext){
	...
	$file=ECMS_PATH.'afdglxt/data/tmp/temp'.$classid.'.php';
...
function GetInfoNewsBq($classid,$newstemp_r,$ecms_gr,$docheckrep){
	...
	if($_GET['reallinfotime'])
	{
		$file=ECMS_PATH.'afdglxt/data/tmp/tempnews'.$newstemp_r['tempid'].'_all.php';
	}
	else
	{
		$file=ECMS_PATH.'afdglxt/data/tmp/tempnews'.$newstemp_r['tempid'].'.php';
	}
	//变量处理
...
function RepBq($indextext){
	global $Avatar,$dbtbpre;
	$sql=$Avatar->query("select bq,funname from {$dbtbpre}enewsbq where isclose=0 order by bqid");
	while($r=$Avatar->fetch($sql))
	{
...
if(\$ecms_bq_sql){
while(\$bqr=\$Avatar->fetch(\$ecms_bq_sql)){
...
if(\$ecms_bq_sql){
while(\$indexbqr=\$Avatar->fetch(\$ecms_bq_sql)){
if(empty(\$class_r[\$indexbqr['classid']]['tbname'])){continue;}
\$bqr=\$Avatar->fetch1(\"select * from {\$dbtbpre}ecms_\".\$class_r[\$indexbqr['classid']]['tbname'].\" where id='\$indexbqr[id]'\");
\$bqsr=sys_ReturnEcmsLoopStext(\$bqr);
\$bqno++;
?>\\2<?php
}
...
function ListHtml($classid,$fields,$enews=0,$userlistr=""){
	global $Avatar,$dbtbpre,$emod_r,$public_r,$class_r,$class_zr,$fun_r,$class_tr,$level_r,$etable_r;
	//不生成栏目
	...
	if($enews==0)//子栏目列表
	{
		...
		$selfclassid=$classid;
		$doenews=0;
		$cr=$Avatar->fetch1("select classpagekey,intro,classimg,cgroupid,repagenum,bdinfoid from {$dbtbpre}enewsclass where classid='$classid'");
		//绑定信息
	...
	elseif($enews==5)//标题分类列表
	{
		...
		$selfclassid=$classid;
		$doenews=1;
		$cr=$Avatar->fetch1("select tnum,listtempid,maxnum,reorder,timg,intro,pagekey,listdt,repagenum from {$dbtbpre}enewsinfotype where typeid='$classid'");
		//页面
	...
	elseif($enews==3)//大栏目列表
	{
		...
		$selfclassid=$classid;
		$doenews=0;
		$cr=$Avatar->fetch1("select classpagekey,intro,classimg,cgroupid,repagenum from {$dbtbpre}enewsclass where classid='$classid'");
		$mid=$class_r[$classid][modid];
		//权限
		if($cr['cgroupid'])
		{
...
function ListHtmlIndex($classid,$fields,$enews=0,$userlistr=""){
	...
	if($enews==0)//专题列表
	{
		$selfclassid=$classid;
		$doenews=1;
		$cr=$Avatar->fetch1("select ztpagekey,intro,ztimg,classtempid from {$dbtbpre}enewszt where ztid='$classid'");
		//页面
		$pagetitle=ehtmlspecialchars($class_zr[$classid][ztname]);
		$pagekey=ehtmlspecialchars($cr['ztpagekey']);
		$pagedes=ehtmlspecialchars($cr['intro']);
	...
	elseif($enews==1)//专题子类列表
	{
		$selfclassid=$classid;
		$doenews=1;
		$cr=$Avatar->fetch1("select ztid,cname,islist,listtempid,maxnum,tnum,reorder,ttype from {$dbtbpre}enewszttype where cid='$classid'");
		$GLOBALS['navinfor']['ecmsbid']=$cr['ztid'];
		//页面
		$pagetitle=ehtmlspecialchars($cr['cname']);
		$pagekey=ehtmlspecialchars($cr['cname']);
		...
		$classname=$cr['cname'];
		$lencord=$cr['tnum'];//记录数
		$onclick="<script src='".$public_r[newsurl]."afdglxt/public/onclick/?enews=dozt&ztid=$ztid'></script>";
		//模板
		$listtempid=$cr['listtempid'];
	}
	elseif($enews==4)//按sql语句生成列表
	...
	while($k=$Avatar->fetch($sql))
	{
		...
		$infor=$Avatar->fetch1("select * from {$dbtbpre}ecms_".$class_r[$k[classid]][tbname]." where id='$k[id]' limit 1");
		if(empty($infor['id']))
		{
			$no++;
			continue;
...
function DoGetHtml($classid,$id){
	global $Avatar,$class_r,$public_r,$dbtbpre;
	...
	$r=$Avatar->fetch1("select * from {$dbtbpre}ecms_".$tbname." where id='$id'");
...
function GetHtmlRepVar($tempr,$classid){
	global $public_r,$class_r,$class_zr,$fun_r,$Avatar,$dbtbpre,$emod_r,$class_tr,$level_r,$etable_r;
	$mid=$class_r[$classid]['modid'];
	$tbname=$class_r[$classid][tbname];
	...
	for($i=1;$i<$fcount;$i++)
	{
		...
		{
			$value='empty('.$value.')?$public_r[newsurl].\'afdglxt/data/images/notimg.gif\':'.$value;
		}
		elseif($f=='title')//标题
		{
		}
	...
	//带链接的用户名
	$newstempstr=str_replace('[!--linkusername--]','<?=$ecms_gr[ismember]==1&&$ecms_gr[userid]?\'<a href="\'.$public_r[newsurl].\'afdglxt/space/?userid=\'.$ecms_gr[userid].\'" target=_blank>\'.$ecms_gr[username].\'</a>\':$ecms_gr[username]?>',$newstempstr);
	...
	//点击
	$onclick='<?=\'<script src="\'.$public_r[newsurl].\'afdglxt/public/onclick/?enews=donews&classid=\'.$ecms_gr[classid].\'&id=\'.$ecms_gr[id].\'"></script>\'?>';
	...
	{
	$infonext='<?php
	$next_r=$Avatar->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}ecms_".$class_r[$ecms_gr[classid]][tbname]." where id>$ecms_gr[id] and classid=\'$ecms_gr[classid]\' order by id limit 1");
	if(empty($next_r[id]))
	...
	{
	$infopre='<?php
	$next_r=$Avatar->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}ecms_".$class_r[$ecms_gr[classid]][tbname]." where id<$ecms_gr[id] and classid=\'$ecms_gr[classid]\' order by id desc limit 1");
	if(empty($next_r[id]))
	{$infonext="<a href=\'".$grclassurl."\'>'.$fun_r['HaveNoNextLink'].'</a>";}
	else
	{
...
function GetHtml($classid,$id,$add,$ecms=0,$doall=0){
	...
	if($ecms==0)//主表
	{
		$add=$Avatar->fetch1("select ".ReturnSqlTextF($mid,1)." from {$dbtbpre}ecms_".$tbname." where id='$id' limit 1");
	}
	$add['id']=$id;
	$add['classid']=$classid;
	if($add['isurl'])
	...
	//副表
	$addr=$Avatar->fetch1("select ".ReturnSqlFtextF($mid)." from {$dbtbpre}ecms_".$tbname."_data_".$add[stb]." where id='$add[id]' limit 1");
	...
	$newstemp_r=$Avatar->fetch1("select temptext,showdate from ".GetTemptb("enewsnewstemp")." where tempid='$add[newstempid]' limit 1");
	...
	if($add[groupid]||$class_r[$add[classid]]['cgtoinfo'])
	{
		...
		$include1=$include;
		$include.='afdglxt/class/CheckLevel.php';
		$filetype='.php';
		$addlevel="<?php
		define('Avatarcms','wm_chief');
	...
	if($pf&&strstr($add[$pf],$expage))//有分页
	{
		...
		for($j=1;$j<=$thispagenum;$j++)
		{
			...
			else
			{
				...
				$ti_r=explode('[/!--Avatarnews.page--]',$n_r[$k]);
				if(count($ti_r)>=2)
				{
					$ptitle=$ti_r[0];
					$n_r[$k]=$ti_r[1];
	...
	{
		$Avatar->query("update {$dbtbpre}ecms_".$class_r[$add[classid]][tbname]."_index set havehtml=1 where id='$add[id]' limit 1");
		$Avatar->query("update {$dbtbpre}ecms_".$class_r[$add[classid]][tbname]." set havehtml=1 where id='$add[id]' limit 1");
	}
}
...
function GetKeyboard($keyboard,$keyid,$classid,$id,$link_num){
	global $Avatar,$public_r,$class_r,$fun_r,$dbtbpre;
	if($keyid&&$link_num)
	{
		$add="id in (".$keyid.")";
		$tr=$Avatar->fetch1("select otherlinktemp,otherlinktempsub,otherlinktempdate from ".GetTemptb("enewspubtemp")." limit 1");//取得相关链接模板
		$temp_r=explode("[!--Avatarnews.listtemp--]",$tr[otherlinktemp]);
		$key_sql=$Avatar->query("select id,newstime,title,isurl,titleurl,classid,titlepic from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where ".$add." order by newstime desc limit $link_num");
		while($link_r=$Avatar->fetch($key_sql))
		{
...
function RepOtherTemp($temptext,$r,$tr){
	...
	if(empty($r[titlepic]))
	{
		$titlepic=$public_r[newsurl]."afdglxt/data/images/notimg.gif";
	}
	else
	{
		$titlepic=$r[titlepic];
...
function RepDownOnlinePathTemp($add,$downsofttemp,$pj,$showdown_r,$ecms){
	...
	if($ecms==0)//下载
	{
		$downurl=$public_r[newsurl]."afdglxt/DownSys/DownSoft/?classid=$add[classid]&id=$add[id]&pathid=$pj";
	}
	else//在线
	{
		$downurl=$public_r[newsurl]."afdglxt/DownSys/play/?classid=$add[classid]&id=$add[id]&pathid=$pj";
	}
...
function ReturnMorepicpathHtml($add){
	...
	for($pj=0;$pj<count($path_r);$pj++)
    {
		...
		$picpath.="<td align=center><a href='".$public_r[newsurl]."afdglxt/ViewImg/index.html?url=".$showdown_r[1]."' target=_blank><img src='".$showdown_r[0]."'".$width.$height." border=0>".$name."</a></td>";
		//分割
        if($p%$line==0)
		{
			$picpath.='</tr>';
...
function GetNewsJs($classid,$line,$sub,$showdate,$enews=0,$tempr){
	global $Avatar,$public_r,$class_r,$class_tr,$emod_r,$etable_r,$dbtbpre,$eyh_r;
	if(empty($line))
	{
		$line=10;
	}
	...
	$temp_r=explode("[!--Avatarnews.listtemp--]",$tempr[temptext]);
	$no=1;
	while($r=$Avatar->fetch($sql))
	{
		$r[oldtitle]=$r[title];
...
function ReUserjs($jsr,$addpath){
	...
	$temp_r=explode("[!--Avatarnews.listtemp--]",$jstemptext[temptext]);
	$query=$jsr[jssql];
...
function ReListHtml($classid,$enews=0){
	global $Avatar,$class_r,$dbtbpre;
	$classid=(int)$classid;
	if(!$classid)
	{
		printerror("NotChangeReClassid","history.go(-1)");
	}
	$r=$Avatar->fetch1("select classtempid,islist from {$dbtbpre}enewsclass where classid='$classid'");
...
function GetPageTemp($tempid){
	global $Avatar;
	$r=$Avatar->fetch1("select temptext from ".GetTemptb("enewspagetemp")." where tempid='$tempid'");
	return $r['temptext'];
}
...
function GetSearch($mid=0){
	global $Avatar,$public_r,$fun_r,$dbtbpre;
	//取得模板
	$tr=$Avatar->fetch1("select searchtemp,searchjstemp,searchjstemp1 from ".GetTemptb("enewspubtemp")." limit 1");
	//返回栏目搜索
	$fcfile=eReturnTrueEcmsPath()."afdglxt/data/fc/ListEnews.php";
	$fcjsfile=eReturnTrueEcmsPath()."afdglxt/data/fc/cmsclass.js";
	...
	//$options="<script src=".$public_r[newsurl]."afdglxt/data/fc/searchclass.js></script>";
...
function GetPlTempPage($pltempid=0){
	global $Avatar,$public_r,$fun_r,$dbtbpre;
	$pl_t_filename=eReturnTrueEcmsPath().'afdglxt/data/template/pltemp.txt';
	...
	$pagekey=$pagetitle;
	$pagedes=$pagetitle;
	$pr=$Avatar->fetch1("select plf from {$dbtbpre}enewspl_set limit 1");
	//回车字段
	$tobrf=',';
	$plfsql=$Avatar->query("select f from {$dbtbpre}enewsplf where ftype='VARCHAR' or ftype='TEXT' or ftype='MEDIUMTEXT' or ftype='LONGTEXT'");
	while($plfr=$Avatar->fetch($plfsql))
	{
		$tobrf.=$plfr[f].',';
	}
	$pr['pltobrf']=$tobrf;
	...
	$ptsql=$Avatar->query("select tempid,temptext from ".GetTemptb("enewspltemp").$where);
	while($ptr=$Avatar->fetch($ptsql))
	{
		$plfiletemp=$yplfiletemp;
		$pl_filename=eReturnTrueEcmsPath().'afdglxt/data/filecache/template/pl'.$ptr[tempid].'.php';
		$pltemp=$ptr['temptext'];
		//头部变量
		...
		//登录
		$pltemp=str_replace("[!--key.url--]",$public_r[newsurl]."afdglxt/ShowKey/?v=pl",$pltemp);
		$pltemp=str_replace("[!--lusername--]","<?=\$lusername?>",$pltemp);
		$pltemp=str_replace("[!--lpassword--]","<?=\$lpassword?>",$pltemp);
		//列表变量
		$listtemp_r=explode("[!--Avatarnews.listtemp--]",$pltemp);
		$plfiletemp=str_replace("<!--Avatar.listtemp.top-->",$listtemp_r[0],$plfiletemp);
		$plfiletemp=str_replace("<!--Avatar.listtemp.footer-->",$listtemp_r[2],$plfiletemp);
		//列表中间
		...
		$plfiletemp=str_replace("<!--Avatar.listtemp.center-->",$listtemp_center,$plfiletemp);
...
function GetPlJsPage(){
	global $Avatar,$public_r,$fun_r,$dbtbpre;
	$pl_t_filename=eReturnTrueEcmsPath()."afdglxt/data/template/pljstemp.txt";
	$pl_filename=eReturnTrueEcmsPath()."afdglxt/pl/more/index.php";
	...
	$pr['pltobrf']=$tobrf;
	//取得评论JS模板
	$pl_r=$Avatar->fetch1("select pljstemp from ".GetTemptb("enewspubtemp")." limit 1");
	...
	$listtemp_r=explode("[!--Avatarnews.listtemp--]",$pljstemp);
	$pltemp=str_replace("<!--Avatar.listtemp.top-->",$listtemp_r[0],$pltemp);
	$pltemp=str_replace("<!--Avatar.listtemp.footer-->",$listtemp_r[2],$pltemp);
	//列表中间
	$listtemp_center=str_replace("[!--plid--]","<?=\$r[plid]?>",$listtemp_r[1]);
	$listtemp_center=str_replace("[!--pltext--]","<?=\$saytext?>",$listtemp_center);
	$listtemp_center=str_replace("[!--pltime--]","<?=\$saytime?>",$listtemp_center);
	...
	$pltemp=str_replace("<!--Avatar.listtemp.center-->",$listtemp_center,$pltemp);
...
function ReGbooktemp(){
	global $Avatar,$public_r,$fun_r,$dbtbpre;
	$tfile=eReturnTrueEcmsPath()."afdglxt/data/template/gbooktemp.txt";
	$file=eReturnTrueEcmsPath()."afdglxt/tool/gbook/index.php";
	$gbtemp=ReadFiletext($tfile);
	//取得留言页面模板
	$pr=$Avatar->fetch1("select gbooktemp from ".GetTemptb("enewspubtemp")." limit 1");
	$url="<?=\$url?>";
	$pagetitle="<?=\$bname?>";
	...
	$listtemp_r=explode("[!--Avatarnews.listtemp--]",$pr['gbooktemp']);
	$gbtemp=str_replace("<!--Avatar.listtemp.top-->",$listtemp_r[0],$gbtemp);
	$gbtemp=str_replace("<!--Avatar.listtemp.footer-->",$listtemp_r[2],$gbtemp);
	//---列表中间
	//处理回复
	$restart="
<?
...
?>";
	...
	$gbtemp=str_replace("<!--Avatar.listtemp.center-->",$listtemp_center,$gbtemp);
	WriteFiletext($file,$gbtemp);
}

//更新控制面板模板
function ReCptemp(){
	global $Avatar,$public_r,$dbtbpre,$fun_r;
	$pr=$Avatar->fetch1("select cptemp from ".GetTemptb("enewspubtemp")." limit 1");
	$url="<?=\$url?>";
	$pagetitle="<?=defined('Avatarcms')?\$public_diyr[pagetitle]:'".$fun_r['membercp']."'?>";
	$temptext=ReplaceSvars($pr['cptemp'],$url,0,$pagetitle,$pagetitle,$pagetitle,$add,1);
	//生成头尾文件
	$r=explode("[!--Avatarnews.template--]",$temptext);
	$file1=eReturnTrueEcmsPath()."afdglxt/data/template/cp_1.php";
	WriteFiletext($file1,AddCheckViewTempCode().$r[0]);
	$file2=eReturnTrueEcmsPath()."afdglxt/data/template/cp_2.php";
	WriteFiletext($file2,AddCheckViewTempCode().$r[1]);
}

//更新登陆状态模板
function ReLoginIframe(){
	global $Avatar,$public_r,$dbtbpre;
	$tfile=eReturnTrueEcmsPath()."afdglxt/data/template/loginiframetemp.txt";
	$loginiframetemp=ReadFiletext($tfile);
	$pr=$Avatar->fetch1("select loginiframe,loginjstemp from ".GetTemptb("enewspubtemp")." limit 1");
	//框架登陆状态调用
	...
	$file=eReturnTrueEcmsPath()."afdglxt/member/iframe/index.php";
	...
	$file=eReturnTrueEcmsPath()."afdglxt/member/login/loginjs.php";
...
function ReturnVoteTemp($tempid,$enews=0){
	global $Avatar;
	$r=$Avatar->fetch1("select temptext from ".GetTemptb("enewsvotetemp")." where tempid='$tempid'");
	if($enews)
	{
...
function RepVoteTempAllvar($temptext,$r){
	global $public_r;
	$action=$public_r['newsurl']."afdglxt/enews/index.php";
	$temptext=str_replace("[!--vote.action--]",$action,$temptext);
	$temptext=str_replace("[!--title--]",$r[title],$temptext);
	$viewurl=$public_r[newsurl]."afdglxt/tool/vote/?voteid=".$r[voteid];
...
function GetPrintPage($printtempid=0){
	global $Avatar,$dbtbpre,$fun_r,$public_r;
	$file=eReturnTrueEcmsPath().'afdglxt/data/template/printtemp.txt';
	$string=ReadFiletext($file);
	$url="<?=\$url?>";
	$pagetitle="<?=ehtmlspecialchars(\$r[title])?> ".$fun_r['PrintPage'];
	//取得评论页面模板
	$where=$printtempid?" where tempid='$printtempid'":'';
	$ptsql=$Avatar->query("select tempid,temptext,showdate,modid from ".GetTemptb("enewsprinttemp").$where);
	while($ptr=$Avatar->fetch($ptsql))
	{
		$ptr[temptext]=ReplaceSvars($ptr[temptext],$url,0,$pagetitle,$pagetitle,$pagetitle,$add,1);
		$printtemp=RepPrintTempV($ptr);
		$printtemp=str_replace("<!--Avatar.print-->",$printtemp,$string);
		$truefile=eReturnTrueEcmsPath().'afdglxt/data/filecache/template/print'.$ptr[tempid].'.php';
...
function RepPrintTempV($tr){
	global $Avatar,$dbtbpre,$fun_r,$public_r,$emod_r;
...
function GetDownloadPage(){
	global $Avatar,$public_r,$dbtbpre,$fun_r;
	$pr=$Avatar->fetch1("select downpagetemp from ".GetTemptb("enewspubtemp")." limit 1");
	...
	$temptext=str_replace("[!--totaldown--]","<?=\$r[totaldown]?>",$temptext);
	$temptext=str_replace("[!--onclick--]","<?=\$r[onclick]?>",$temptext);
	$file=eReturnTrueEcmsPath()."afdglxt/data/template/downpagetemp.php";
...
function ReSchAlltemp(){
	global $Avatar,$public_r,$fun_r,$dbtbpre;
	$tfile=eReturnTrueEcmsPath()."afdglxt/data/template/schalltemp.txt";
	$file=eReturnTrueEcmsPath()."afdglxt/sch/index.php";
	$temp=ReadFiletext($tfile);
	//取得页面模板
	$pr=$Avatar->fetch1("select schalltemp,schallsubnum,schalldate from ".GetTemptb("enewspubtemp")." limit 1");
	$url="<?=\$url?>";
	$pagetitle=$fun_r['SearchAllNav'];
	$pr['schalltemp']=ReplaceSvars($pr['schalltemp'],$url,0,$pagetitle,$pagetitle,$pagetitle,$add,1);
	$temp=str_replace("<!--Avatar.listtemp.subnum-->",$pr['schallsubnum'],$temp);
	$temp=str_replace("<!--Avatar.listtemp.formatdate-->",$pr['schalldate'],$temp);
	...
	$listtemp_r=explode("[!--Avatarnews.listtemp--]",$pr['schalltemp']);
	$temp=str_replace("<!--Avatar.listtemp.top-->",$listtemp_r[0],$temp);
	$temp=str_replace("<!--Avatar.listtemp.footer-->",$listtemp_r[2],$temp);
	//---列表中间
	...
	$temp=str_replace("<!--Avatar.listtemp.center-->",$listtemp_center,$temp);
...
function ReturnLeftLevel($groupid){
	...
	{return "";}
	$groupid=(int)$groupid;
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewsgroup where groupid='$groupid'");
	return $r;
}

//password
function DoAvatarCMSAdminPassword($password,$salt,$salt2){
...
function CheckLevel($userid,$username,$classid,$enews){
	global $Avatar,$dbtbpre;
	$userid=(int)$userid;
	$r=$Avatar->fetch1("select groupid,adminclass from {$dbtbpre}enewsuser where userid='$userid' limit 1");
	//操作信息
	if($enews=="news")
	{
		//操作所有栏目权限
		$gr=$Avatar->fetch1("select doall,doselfinfo,doaddinfo,doeditinfo,dodelinfo,docheckinfo,dogoodinfo,dodocinfo,domoveinfo,domustcheck,docheckedit from {$dbtbpre}enewsgroup where groupid='$r[groupid]'");
		if(empty($gr[doall]))
		{
	...
	{
		//用户组
		$gr=$Avatar->fetch1("select * from {$dbtbpre}enewsgroup where groupid='$r[groupid]'");
		$enews="do".$enews;
		if(empty($gr[$enews]))
		{
...
function CheckAndUsernamesLevel($level,$id,$userid,$username,$groupid){
	...
	$getr=$Avatar->fetch1($getquery);
	if(!$getr[$id_field])
	{
		printerror('ErrorUrl','history.go(-1)');
	}
	$gr=$Avatar->fetch1("select groupid,".$level." from {$dbtbpre}enewsgroup where groupid='$groupid'");
	if(!$gr['groupid'])
	{
		printerror('NotLevel','history.go(-1)');
	}
...
function is_login($uid=0,$uname='',$urnd=''){
	...
	//db
	$adminr=$Avatar->fetch1("select userid,groupid,classid,userprikey,uprnd from {$dbtbpre}enewsuser where userid='$userid' and username='".$username."' and rnd='".$rnd."' and checked=0 limit 1");
	if(!$adminr['userid'])
	{
	...
	if(getcvar('eloginlic',1)<>"Avatarcmslic")
	{
		printerror("NotLogin","index.php");
	}
	$ur[userid]=$userid;
...
function is_login_other($userid,$username,$rnd){
	...
	$adminstyle=1;
	//db
	$adminr=$Avatar->fetch1("select userid,groupid,classid,userprikey from {$dbtbpre}enewsuser where userid='$userid' and username='".$username."' and rnd='".$rnd."' and checked=0 limit 1");
	if(!$adminr['userid'])
	{
		printerror("NotLogin","index.php");
	}
...
function DoECreateOtherRnd($userid,$username,$rnd,$ckoi=0){
	...
	$otherinfo=$ckoi==1?DoECkOtherInfo():'Avatar.cms';
	$r['otherrndtime']=time();
	$r['otherrndtwo']=make_password(12);
	$r['otherrndpass']=md5(md5($rnd.'-Avatarcms.2002!check.other-'.$ecms_config['cks']['ckrndtwo']).'-'.$ip.'Avatar.cms'.'-'.$otherinfo.'-'.$userid.'-'.$r['otherrndtime'].'-'.$username.'db.check.rnd'.'-'.$rnd.'-caity'.$r['otherrndtwo']);
	return $r;
}
...
function DoECheckOtherRnd($userid,$username,$rnd,$loginecmsotherpass,$loginecmsothertime,$loginecmsotherrndtwo,$ckoi=0,$outtime=1800){
	...
	$ip=$ecms_config['esafe']['ckhloginip']==0?'127.0.0.1':egetip();
	$otherinfo=$ckoi==1?DoECkOtherInfo():'Avatar.cms';
	$ecmsckpass=md5(md5($rnd.'-Avatarcms.2002!check.other-'.$ecms_config['cks']['ckrndtwo']).'-'.$ip.'Avatar.cms'.'-'.$otherinfo.'-'.$userid.'-'.$loginecmsothertime.'-'.$username.'db.check.rnd'.'-'.$rnd.'-caity'.$loginecmsotherrndtwo);
...
function hReturnAdminLoginFileInfo($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval){
	...
	define('AvatarCMSHDefHash',$defhash);
	$adminlogins.="<?php
...
function DoECreatFileRnd($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval){
	global $ecms_config;
	$file=ECMS_PATH.'afdglxt/data/adminlogin/user'.$userid.'_'.md5(md5($username.'-Avatarcms!check.file'.$truelogintime.'-'.$rnd.$ecms_config['esafe']['ecookiernd']).'-'.$ip.'-'.$userid.'-'.$rnd.$adminstyle.'-'.$groupid.'-'.$dbdata.$sessval).'.php';
...
function DoEDelFileRnd($userid){
	$path=ECMS_PATH.'afdglxt/data/adminlogin/';
	$hand=@opendir($path);
...
function DoECreatAndAuthRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval){
	global $Avatar,$dbtbpre,$ecms_config;
	$andauth=md5(md5($rnd.'-'.$username.'-Avatarcms!check.andauth'.$truelogintime.'-'.$ecms_config['esafe']['ecookiernd'].$userkey).$sessval.'-'.$ip.'-'.$userid.$rnd.'-'.$adminstyle.'-'.$groupid.$username.'-'.$dbdata);
	DoEDelAndAuthRnd($userid);
	$Avatar->query("replace into {$dbtbpre}enewsuserloginck(userid,andauth) values('$userid','$andauth');");
}

function DoECheckAndAuthRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime){
	global $Avatar,$dbtbpre,$ecms_config;
	$anduser_r=$Avatar->fetch1("select andauth from {$dbtbpre}enewsuserloginck where userid='$userid'");
	...
	//$otherinfo=DoECkOtherInfo();
	$sessval=ReESessionRnd();
	$ckandauth=md5(md5($rnd.'-'.$username.'-Avatarcms!check.andauth'.$truelogintime.'-'.$ecms_config['esafe']['ecookiernd'].$userkey).$sessval.'-'.$ip.'-'.$userid.$rnd.'-'.$adminstyle.'-'.$groupid.$username.'-'.$dbdata);
...
function DoEDelAndAuthRnd($userid){
	global $Avatar,$dbtbpre;
	$Avatar->query("delete from {$dbtbpre}enewsuserloginck where userid='$userid'");
}

//写入操作日志
function insert_dolog($doing,$pubid=0){
	global $Avatar,$enews,$caity,$logininid,$loginin,$ecms_config,$dbtbpre;
	...
	$sql=$Avatar->query("insert into {$dbtbpre}enewsdolog(username,logip,logtime,enews,doing,pubid,ipport) values('$loginin','$logip','$logtime','$enews','$doing','$pubid','$ipport');");
}

//返回安全提问问题
function ReturnHLoginQuestionStr($userid,$username,$question,$answer){
	$pass=md5(md5('-#20Avatar27#-'.$question.'-Avatarcms-'.$userid.'-www.caity.net-'.$answer.'-wm-').'-dg2002-'.$answer.'-wm_chief-'.$userid.'-wangmeng-');
	return $pass;
}
...
function ReturnFtpInfo($ftpid){
	global $Avatar,$dbtbpre;
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewspublic limit 1");
	return $r;
}

//初使化发布任务
function AddPostUrlData($postdata,$userid,$username){
	global $Avatar,$fun_r,$dbtbpre;
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$r[1]=(int)$r[1];
		$r[0]=AddAddsData($r[0]);
		$sql=$Avatar->query("insert into {$dbtbpre}enewspostdata(rnd,postdata,ispath) values('$rnd','$r[0]','$r[1]');");
    }
	$line=(int)$_POST['line'];
	if($line==0)
	{
...
function PostUrlData($start,$rnd,$userid,$username){
	global $Avatar,$fun_r,$dbtbpre,$incftp;
	$rnd=RepPostVar($rnd);
	if(empty($rnd))
	...
	if(empty($incftp))
	{
		@include(ECMS_PATH."afdglxt/class/ftp.php");
	}
	...
	$start=(int)$start;
	$b=0;
	$sql=$Avatar->query("select postid,postdata,ispath from {$dbtbpre}enewspostdata where rnd='$rnd' and postid>$start order by postid limit ".$line);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$r[postid];
	...
	if(empty($b))
	{
		$sql=$Avatar->query("delete from {$dbtbpre}enewspostdata where rnd='$rnd'");
		//操作日志
		insert_dolog("");
...
function CheckFtpConnect($ftphost,$ftpport,$ftpusername,$ftppassword,$ftppath,$ftpssl=0,$pasv=0,$tranmode=0,$timeout=0){
	if(!defined('InAvatarCMSFtp'))
	{
		include(ECMS_PATH.'afdglxt/class/ftp.php');
	}
	$eftp=new AvatarCMSFTP();
...
function CopyEcmsTb($otb,$tb){
	global $Avatar;
	$usql=$Avatar->query("SET SQL_QUOTE_SHOW_CREATE=1;");//设置引号
	$r=$Avatar->fetch1("SHOW CREATE TABLE `$otb`;");//数据表结构
...
function TogSaveTxtF($ecms=0){
	global $Avatar,$dbtbpre;
	$savesql=$Avatar->query("select f,tbname from {$dbtbpre}enewsf where savetxt=1");
	$savef=',';
	while($saver=$Avatar->fetch($savesql))
	{
		$savef.=$saver[tbname].'.'.$saver[f].',';
	}
	$Avatar->query("update {$dbtbpre}enewspublic set savetxtf='$savef' limit 1");
	if($ecms==0)
	{
		GetConfig();
	}
...
function ReturnMFileF($enter,$tbname,$tid,$fform="file"){
	...
	$f=",";
	$sql=$Avatar->query("select f from ".$tbname." where fform='$fform'".$a);
	while($r=$Avatar->fetch($sql))
	{
...
function ChGetFname($mid,$f){
	global $Avatar,$dbtbpre,$emod_r;
	$r=$Avatar->fetch1("select fname from {$dbtbpre}enewsf where f='$f' and tid='".$emod_r[$mid]['tid']."' limit 1");
...
function ChIsOnlyAddF($mid,$id,$f,$value,$isq=0){
	...
	if(strstr($emod_r[$mid]['onlyf'],','.$f.','))
	{
		...
		//已审核
		$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_".$emod_r[$mid]['tbname']." where ".$f."='".addslashes($value)."'".$and." limit 1");
		//未审核
		if(empty($num))
		{
			$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_".$emod_r[$mid]['tbname']."_check where ".$f."='".addslashes($value)."'".$and." limit 1");
		}
		if($num)
		{
...
function SameDataAddF($id,$classid,$mid,$f,$value){
	global $Avatar,$public_r,$dbtbpre,$emod_r,$emod_pubr;
	if(strstr($emod_pubr['linkfields'],','.$emod_r[$mid]['tbname'].'.'.$f.'|'))
	{
		$index_r=$Avatar->fetch1("select checked from {$dbtbpre}ecms_".$emod_r[$mid]['tbname']."_index where id='$id' limit 1");
		//返回表
		$infotb=empty($index_r['checked'])?$dbtbpre.'ecms_'.$emod_r[$mid]['tbname'].'_check':$dbtbpre.'ecms_'.$emod_r[$mid]['tbname'];
		$value=addslashes($value);
		$r=$Avatar->fetch1("select ".$f." from ".$infotb." where id='$id' limit 1");
		if($r[$f]<>$value)
		{
			...
			if($ltbname&&$lf)
			{
				$Avatar->query("update {$dbtbpre}ecms_".$ltbname." set ".$lf."='$value' where ".$lf."='$r[$f]'");
			}
		}
	}
}
...
function ReturnAddF($add,$modid,$userid,$username,$do=0,$rdata=0,$ch=0){
	global $Avatar,$public_r,$dbtbpre,$emod_r;
	if($do==0||$do==1)
	{
		//导入gd处理文件
		if($add['mark']||$add['getfirsttitlespic']||$add['mcreatespic'])
		{
			include_once(ECMS_PATH.'afdglxt/class/gd.php');
		}
	}
	...
	if(empty($do))//增加
	{
		...
		for($i=1;$i<$count;$i++)
		{
			...
			if($f=="newstext")
			{
				...
				//自动分页
				if($add[autopage]&&!strstr($value,"[!--Avatarnews.page--]"))
				{
...
function SaveMorepicFile($varname,$msavepic,$i,$picurl,$picname,$classid,$id,$add,$modtype=0,$fstb=1){
	global $public_r,$Avatar,$loginin,$dbtbpre,$ecms_config;
...
function GetClassNavCache($line,$navfh){
	...
	$navs='';
	$fh='';
	$sql=$Avatar->query("select classid,classname,wburl,listdt,classurl,classpath from {$dbtbpre}enewsclass where bclassid=0 and showclass=0 order by myorder,classid".$limit);
	while($r=$Avatar->fetch($sql))
	{
...
function GetConfig($domod=0){
	$filename=eReturnTrueEcmsPath()."afdglxt/config/config.php";
	$exp='//-------AvatarCMS.Public.Cache-------';
...
function GetPubCache(){
	...
	//扩展变量
	$pvstring='';
	$pvsql=$Avatar->query("select myvar,varvalue from {$dbtbpre}enewspubvar where tocache=1");
	while($pvr=$Avatar->fetch($pvsql))
	{
		$pvstring.=",'add_".$pvr['myvar']."'=>'".addslashes($pvr['varvalue'])."'";
	}
	//公共变量
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewspublic limit 1");
	$tr=$Avatar->fetch1("select downsofttemp,onlinemovietemp,listpagetemp from ".GetTemptb("enewspubtemp")." limit 1");
	$fsr=$Avatar->fetch1("select purl from {$dbtbpre}enewspostserver where ptype=1 limit 1");
	$plr=$Avatar->fetch1("select * from {$dbtbpre}enewspl_set limit 1");
	$memberconnectnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsmember_connect_app where isclose=0");
...
function GetModCache(){
	global $Avatar,$dbtbpre;
	//数据表
	$tablesql=$Avatar->query("select tbname,deftb,yhid,mid,intb from {$dbtbpre}enewstable");
	while($tabler=$Avatar->fetch($tablesql))
	{
...
";
	...
	//系统模型
	$alllinkfields='|';//关联同步
	$modsql=$Avatar->query("select * from {$dbtbpre}enewsmod");
	while($mr=$Avatar->fetch($modsql))
	{
		...
		$fsql=$Avatar->query("select * from {$dbtbpre}enewsf where tid='$mr[tid]'");
		while($fr=$Avatar->fetch($fsql))
		{
			if($fr['tbdataf'])
			{
				$dataf.=$fr['f'].',';
		...
		//表数据
		$tr=$Avatar->fetch1("select * from {$dbtbpre}enewstable where tid='$mr[tid]'");
		//字符
...
function GetMemberLevel(){
	global $Avatar,$dbtbpre;
	$file=eReturnTrueEcmsPath()."afdglxt/data/dbcache/MemberLevel.php";
	$sql=$Avatar->query("select * from {$dbtbpre}enewsmembergroup order by groupid");
	while($r=$Avatar->fetch($sql))
	{
...
function GetYh(){
	global $Avatar,$dbtbpre;
	$sql=$Avatar->query("select * from {$dbtbpre}enewsyh");
	while($r=$Avatar->fetch($sql))
	{
...
function GetClass(){
	global $Avatar,$dbtbpre;
	$fileqz=eReturnTrueEcmsPath().'afdglxt/data/dbcache/';
	$filename=$fileqz.'class.php';
	$line=250;//每个文件存放栏目数
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsclass");
	$sql=$Avatar->query("select * from {$dbtbpre}enewsclass");
	$no=0;
	$p=0;
	$l="";
	$mod="";
...
";
		if($no%$line==0||($num%$line<>0&&$num==$no))
		{
			$p++;
			$file="class".$p.".php";
			$include.="require(ECMS_PATH.'afdglxt/data/dbcache/".$file."');\r\n";
			$classes="<?php
".$classes."?>";
	...
	//-----专题缓存
	$zsql=$Avatar->query("select * from {$dbtbpre}enewszt");
	$zt="";
	$zfile=$fileqz."ztclass.php";
	while($zr=$Avatar->fetch($zsql))
	{
...
".$zt.GetTitleTypeCache()."?>";
	...
	$include.="require(ECMS_PATH.'afdglxt/data/dbcache/ztclass.php');\r\n";
	$include="<?php
".AddCheckViewCode()."
\$class_r=array();
\$class_zr=array();
...
?>";
	...
	{
		$mid=$er[$i];
		$usql=$Avatar->query("update {$dbtbpre}enewsmod set sonclass='".$mod[$mid]."' where mid='$mid'");
    }
}
...
function GetTitleTypeCache(){
	global $Avatar,$dbtbpre;
	$sql=$Avatar->query("select typeid,tname,mid,yhid,tpath,tid,tbname,listdt,ttype from {$dbtbpre}enewsinfotype");
	while($r=$Avatar->fetch($sql))
	{
...
function GetSearchAllTb(){
	global $Avatar,$dbtbpre;
	$file=eReturnTrueEcmsPath()."afdglxt/data/dbcache/SearchAllTb.php";
	$sql=$Avatar->query("select tbname,titlefield,smalltextfield from {$dbtbpre}enewssearchall_load");
	while($r=$Avatar->fetch($sql))
	{
...
function GetMoreportCache(){
	global $Avatar,$dbtbpre;
	$sql=$Avatar->query("select * from {$dbtbpre}enewsmoreport");
	$i=0;
	while($r=$Avatar->fetch($sql))
	{
		$i++;
...
function Moreport_UpdateIsclose(){
	global $Avatar,$dbtbpre,$public_r;
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsmoreport where isclose=0");
	if($num>1)
	{
		$purl=addslashes($public_r['newsurl']);
		$ppath=addslashes(ReturnAbsEcmsPath());
		$postpass=make_password(60);
		$Avatar->query("update {$dbtbpre}enewsmoreport set purl='$purl',ppath='$ppath',postpass='$postpass' where pid=1");
	}
	else
	{
		$postpass=make_password(60);
		$Avatar->query("update {$dbtbpre}enewsmoreport set purl='',ppath='',postpass='$postpass' where pid=1");
	}
	return $num;
}
