...
function SetPl($add,$userid,$username){
	...
	$sql=$Avatar->query("update {$dbtbpre}enewspl_set set pltime='$add[pltime]',plsize='$add[plsize]',plincludesize='$add[plincludesize]',plkey_ok='$add[plkey_ok]',plfacenum='$add[plfacenum]',plgroupid='$add[plgroupid]',plclosewords='".eaddslashes($add[plclosewords])."',pl_num='$add[pl_num]',plurl='$add[plurl]',plmaxfloor='$add[plmaxfloor]',plquotetemp='".eaddslashes2($add[plquotetemp])."' limit 1");
	GetConfig();//更新缓存
	if($sql)
	{
		insert_dolog("");//操作日志
...
function DelPl_all($plid,$id,$bclassid,$classid,$userid,$username){
	...
	//更新数据表
	$fsql=$Avatar->query("select id,classid,plid,pubid from {$dbtbpre}enewspl_{$restb} where ".$add);
	while($r=$Avatar->fetch($fsql))
	{
		if($class_r[$r[classid]][tbname]&&$r['pubid']>0)
		{
			...
			$Avatar->query("update ".$infotb." set plnum=plnum-1 where id='$r[id]'");
		}
    }
	$sql=$Avatar->query("delete from {$dbtbpre}enewspl_{$restb} where ".$add);
	if($sql)
	{
		//操作日志
...
function CheckPl_all($plid,$id,$bclassid,$classid,$userid,$username){
	...
	$add=substr($add,0,strlen($add)-4);
	$sql=$Avatar->query("update {$dbtbpre}enewspl_{$restb} set checked=0 where ".$add);
	if($sql)
	{
		//操作日志
		insert_dolog("classid=".$classid."<br>classname=".$class_r[$classid][classname]);
...
function DoGoodPl_all($plid,$id,$bclassid,$classid,$isgood,$userid,$username){
	...
	$add=substr($add,0,strlen($add)-4);
	$sql=$Avatar->query("update {$dbtbpre}enewspl_{$restb} set isgood='$isgood' where ".$add);
	if($sql)
	{
		//操作日志
...
function CheckRePlF($add,$ecms=0){
	...
	//主表
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}enewspl_1");
	$b=0;
...
function AddPlF($add,$userid,$username){
	...
	//新增字段
	$tbr=$Avatar->fetch1("select pldatatbs from {$dbtbpre}enewspl_set limit 1");
	if($tbr['pldatatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			$Avatar->query("alter table {$dbtbpre}enewspl_".$dtbr[$i]." add ".$field);
		}
	}
	//处理变量
	$add[ismust]=(int)$add[ismust];
	$sql=$Avatar->query("insert into {$dbtbpre}enewsplf(f,fname,fzs,ftype,flen,ismust) values('$add[f]','$add[fname]','".addslashes($add[fzs])."','$add[ftype]','$add[flen]','$add[ismust]');");
...
function EditPlF($add,$userid,$username){
	...
	//验证字段重复
	CheckRePlF($add,1);
	$cr=$Avatar->fetch1("select * from {$dbtbpre}enewsplf where fid='$fid'");
	//改变字段
	if($cr[f]<>$add[f]||$cr[ftype]<>$add[ftype]||$cr[flen]<>$add[flen])
	{
		...
		if($tbr['pldatatbs'])
		{
			...
			for($i=1;$i<$count-1;$i++)
			{
				$Avatar->query("alter table {$dbtbpre}enewspl_".$dtbr[$i]." change `".$cr[f]."` ".$field);
			}
		}
	}
	//处理变量
	$add[ismust]=(int)$add[ismust];
	$sql=$Avatar->query("update {$dbtbpre}enewsplf set f='$add[f]',fname='$add[fname]',fzs='".addslashes($add[fzs])."',ftype='$add[ftype]',flen='$add[flen]',ismust='$add[ismust]' where fid=$fid");
	UpdatePlF();//更新字段
	GetConfig();//更新缓存
	if($sql)
	{
...
function DelPlF($add,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"plf");
	$r=$Avatar->fetch1("select f from {$dbtbpre}enewsplf where fid=$fid");
	if(!$r[f])
	{
		printerror("EmptyFid","history.go(-1)");
	}
	...
	if($tbr['pldatatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			$Avatar->query("alter table {$dbtbpre}enewspl_".$dtbr[$i]." drop COLUMN `".$r[f]."`");
		}
	}
	$sql=$Avatar->query("delete from {$dbtbpre}enewsplf where fid=$fid");
	UpdatePlF();//更新字段
	GetConfig();//更新缓存
	if($sql)
	{
...
function UpdatePlF(){
	...
	$plf=',';
	$plmustf=',';
	$sql=$Avatar->query("select f,ismust from {$dbtbpre}enewsplf");
	while($r=$Avatar->fetch($sql))
	{
		$plf.=$r[f].',';
		if($r[ismust])
	...
	$Avatar->query("update {$dbtbpre}enewspl_set set plf='$plf',plmustf='$plmustf' limit 1");
}
...
function AddPlDataTable($add,$userid,$username){
	echo'This is the Free Version of AvatarCMS.';
	exit();
}
