...
function EditCjNews($add,$newstext,$userid,$username){
	...
	$record="<!--record-->";
	$field="<!--field--->";
	$cr=$Avatar->fetch1("select newsclassid,tbname from {$dbtbpre}enewsinfoclass where classid='$add[classid]'");
	$r=$Avatar->fetch1("select cj from {$dbtbpre}enewsmod where mid='".$class_r[$cr[newsclassid]][modid]."'");
	...
	$sql=$Avatar->query("update {$dbtbpre}ecms_infotmp_".$cr[tbname]." set keyboard='".eaddslashes2($add[keyboard])."',newstime='$add[newstime]'".$update." where id='$add[id]'");
	if($sql)
	{
		//操作日志
...
function DelCjNews($classid,$id,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$cr=$Avatar->fetch1("select newsclassid,tbname from {$dbtbpre}enewsinfoclass where classid='$classid'");
	$r=$Avatar->fetch1("select title from {$dbtbpre}ecms_infotmp_".$cr[tbname]." where id='$id'");
	$sql=$Avatar->query("delete from {$dbtbpre}ecms_infotmp_".$cr[tbname]." where id='$id'");
	if($sql)
	{
		//操作日志
...
function DelCjNews_all($classid,$id,$userid,$username){
	...
	if(!$count)
	{printerror("NotDelCjNewsid","history.go(-1)");}
	$cr=$Avatar->fetch1("select newsclassid,tbname,classname from {$dbtbpre}enewsinfoclass where classid='$classid'");
	...
	$sql=$Avatar->query("delete from {$dbtbpre}ecms_infotmp_".$cr[tbname]." where ".$add);
	if($sql)
	{
		//操作日志
...
function DoClearSmalltextVal($value){
	$value=str_replace(array("\r\n","<br />","<br>","&nbsp;","[!--Avatarnews.page--]","[/!--Avatarnews.page--]"),array("","\r\n","\r\n"," ","",""),$value);
...
function CjNewsIn($classid,$id,$checked,$uptime,$userid,$username){
	global $class_r,$Avatar,$public_r,$dbtbpre,$emod_r;
	$checked=(int)$checked;
	$classid=(int)$classid;
	if(empty($classid))
	{
	...
	$cr=$Avatar->fetch1("select * from {$dbtbpre}enewsinfoclass where classid='$classid'");
	//副表
	$cra=$Avatar->fetch1("select * from {$dbtbpre}ecms_infoclass_".$cr[tbname]." where classid='$classid'");
	//组合两数组
	...
	$record="<!--record-->";
	$field="<!--field--->";
	$mr=$Avatar->fetch1("select cj from {$dbtbpre}enewsmod where mid='".$class_r[$cr[newsclassid]][modid]."'");
	...
	$sql=$Avatar->query("select * from {$dbtbpre}ecms_infotmp_".$cr[tbname]." where ".$a." and checked=0 order by id desc");
	$todaytime=time();
	$filetime=$todaytime;
	while($r=$Avatar->fetch($sql))
	{
		...
		//索引表
		$havehtml=0;
		$indexsql=$Avatar->query("insert into {$dbtbpre}ecms_".$class_r[$cr[newsclassid]][tbname]."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('$cr[newsclassid]','$checked','$r[newstime]','$r[truetime]','$r[truetime]','$havehtml');");
		$id=$Avatar->lastid();
		$infotbr=ReturnInfoTbname($class_r[$cr[newsclassid]][tbname],$checked,$stb);
		//主表
		$isurl=$r['titleurl']?1:0;
		$isql=$Avatar->query("insert into ".$infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard,newstime".$ifield.") values('$id','$cr[newsclassid]',0,0,0,0,'$newspath','$filename','$r[userid]','$r[username]',0,0,'$ispic',0,'$isqf',0,'$isurl','$r[truetime]','$r[truetime]','$havehtml',0,0,'$r[titlefont]','$r[titleurl]','$stb','$public_r[filedeftb]','$public_r[pldeftb]','$r[keyboard]','$r[newstime]'".$ivalue.");");
		//副表
		$fisql=$Avatar->query("insert into ".$infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$dataifield.") values('$id','$cr[newsclassid]','$keyid',1,'$newstempid',0,0,''".$dataivalue.");");
		//更新栏目信息数
	...
	{
		$del=$Avatar->query("delete from {$dbtbpre}ecms_infotmp_".$cr[tbname]." where ".$a);
	}
	else
	{
		$del=$Avatar->query("update {$dbtbpre}ecms_infotmp_".$cr[tbname]." set checked=1,keyboard=''".$updatefield." where ".$a);
	}
	//操作日志
...
function CjNewsIn_all($classid,$checked,$uptime,$start,$userid,$username){
	global $class_r,$Avatar,$public_r,$dbtbpre,$fun_r,$emod_r;
	$checked=(int)$checked;
	$classid=(int)$classid;
	$start=(int)$start;
	if(empty($classid))
	...
	$cjr=explode($record,$mr[cj]);
	$ccount=count($cjr);
	$sql=$Avatar->query("select * from {$dbtbpre}ecms_infotmp_".$cr[tbname]." where classid='$classid' and checked=0 and id>$start order by id limit ".$cr[insertnum]);
	$todaytime=time();
	$filetime=$todaytime;
	$b=0;
	while($r=$Avatar->fetch($sql))
	...
	if(empty($b))
	{
		...
		if($cr['delloadinfo'])//删除
		{
			$del=$Avatar->query("delete from {$dbtbpre}ecms_infotmp_".$cr[tbname]." where classid='$classid'");
		}
		else
		{
			$del=$Avatar->query("update {$dbtbpre}ecms_infotmp_".$cr[tbname]." set checked=1,keyboard=''".$updatefield." where classid='$classid'");
		}
		if($fm)
		{
...
//编码转换
function doCjUtfAndGbk($str,$caity=0){
	//正常编码
	if($caity==0)
	{
		return $str;
	}
	if($caity==1)//UTF8->GB2312
	{
		$str=DoIconvVal("UTF8","GB2312",$str);
	}
	elseif($caity==2)//UTF8->BIG5
	{
		$str=DoIconvVal("UTF8","BIG5",$str);
	}
	elseif($caity==3)//BIG5->GB2312
	{
		$str=DoIconvVal("BIG5","GB2312",$str);
	}
	elseif($caity==4)//GB2312->BIG5
	{
		$str=DoIconvVal("GB2312","BIG5",$str);
	}
	elseif($caity==5)//UNICODE->GB2312
	{
		$str=DoIconvVal("UNICODE","GB2312",$str);
	}
	elseif($caity==6)//UNICODE->BIG5
	{
		$str=DoIconvVal("UNICODE","BIG5",$str);
	}
	elseif($caity==7)//GB2312->UTF8
	{
		$str=DoIconvVal("GB2312","UTF8",$str);
	}
	elseif($caity==8)//BIG5->UTF8
	{
		$str=DoIconvVal("BIG5","UTF8",$str);
	}
	elseif($caity==9)//UNICODE->UTF8
	{
...
function EchoUrl($text,$exp,$exp1,$dr,$url,$classid,$num,$checkrnd){
	...
	for($i=1;$i<count($r)&&$i<=$num;$i++)
	{
		...
		{
			$dbnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$dr[tbname]." where oldurl='$dourl' limit 1");
			if($dbnum)
			{continue;}
		}
		//图片链接
		...
		//将地址写入数据库
		$sql=$Avatar->query("insert into {$dbtbpre}enewslinktmp(newsurl,checkrnd,titlepic) values('$dourl','$checkrnd','$titlepic');");
		echo $dourl."<br>";
	}
}
...
function PageEchoUrl($classid,$cr,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			$dbnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$cr[tbname]." where oldurl='$dourl' limit 1");
			if($dbnum)
			{continue;}
		}
		//将地址写入数据库
		$sql=$Avatar->query("insert into {$dbtbpre}enewslinktmp(newsurl,checkrnd,titlepic) values('$dourl','$checkrnd','');");
		echo $dourl."<br>";
	}
...
function CJUrl($classid,$start,$checkrnd,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$r=$Avatar->fetch1("select endday,num,zz_smallurl,zz_newsurl,httpurl,infourl,newsclassid,relistnum,zz_titlepicl,z_titlepicl,qz_titlepicl,save_titlepicl,tbname,recjtheurl,enpagecode,pagerepad,oldpagerep,newpagerep,keeptime,infourlispage from {$dbtbpre}enewsinfoclass where classid='$classid'");
...
function ViewCjList($classid,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$r=$Avatar->fetch1("select endday,num,zz_smallurl,zz_newsurl,httpurl,infourl,newsclassid,relistnum,zz_titlepicl,z_titlepicl,qz_titlepicl,save_titlepicl,infourlispage from {$dbtbpre}enewsinfoclass where classid='$classid'");
	if(empty($r[newsclassid]))
	{printerror("NotCjid","history.go(-1)");}
	//直接内容页列表
	if($r[infourlispage])
...
function ViewCjUrl($classid,$listpage,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$r=$Avatar->fetch1("select endday,num,zz_smallurl,zz_newsurl,httpurl,infourl,newsclassid,relistnum,zz_titlepicl,z_titlepicl,qz_titlepicl,save_titlepicl,tbname,recjtheurl,enpagecode,pagerepad,oldpagerep,newpagerep from {$dbtbpre}enewsinfoclass where classid='$classid'");
	if(empty($r[newsclassid]))
	{printerror("NotCjid","history.go(-1)");}
	//导入编码文件
	if($r['enpagecode'])
...
function GetMoreCjPagetext($self,$newstextzz,$smallpagezz,$pagezz,$pagetype,$firsttext,$the_r){
	...
	{return "";}
	//是否保留分页
	$addpagetag=empty($the_r['doaddtextpage'])?'[!--Avatarnews.page--]':'';
	//取得地址前缀
...
function GetNewsInfo($classid,$checkrnd,$start,$userid,$username){
	global $Avatar,$class_r,$fun_r,$dbtbpre;
	$userid=(int)$userid;
	$classid=(int)$classid;
	$start=(int)$start;
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewsinfoclass where classid='$classid'");
	//副表
	$ra=$Avatar->fetch1("select * from {$dbtbpre}ecms_infoclass_".$r[tbname]." where classid='$classid'");
	//组合两数组
	$r=TogTwoArray($r,$ra);
	//导入编码文件
	if($r['enpagecode'])
	{
		@include_once(ECMS_PATH."afdglxt/class/doiconv.php");
	}
	//取得模型
	$record="<!--record-->";
	$field="<!--field--->";
	$mr=$Avatar->fetch1("select cj,tid,tbname from {$dbtbpre}enewsmod where mid='".$class_r[$r[newsclassid]][modid]."'");
	$cjr=explode($record,$mr[cj]);
	$count=count($cjr);
	if(empty($start))
	{$start=0;}
	$b=0;
	$sql=$Avatar->query("select linkid,newsurl,titlepic from {$dbtbpre}enewslinktmp where checkrnd='$checkrnd' and linkid>".$start." order by linkid limit ".$r[renum]);
	//采集时间
	$newstime=date("Y-m-d H:i:s");
    $truetime=time();
	$tmptime=$newstime;
	while($nr=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$nr[linkid];
		...
		{
			$tmpnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$r[tbname]." where oldurl='$nr[newsurl]' limit 1");
			if($tmpnum)
			{continue;}
		}
		//取得远程页面，三次取不到值的将不取值
		...
		for($c=0;$c<$count-1;$c++)
		{
			...
			if($dofield=="title")
			{
				...
				{
					//入库比较
					$onum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_".$class_r[$r[newsclassid]][tbname]." where title='".addslashes($zzvalue)."' and classid='$r[newsclassid]' limit 1");
					if($onum)
					{
						$next=1;
						break;
					...
					/*采集库比较
					$onum=$Avatar->num("select id from {$dbtbpre}ecms_infotmp_".$r[tbname]." where title='".addslashes($zzvalue)."' limit 1");
					if($onum)
					{
						$next=1;
						break;
				...
				if($r[titlelen])//标题相似
				{
					...
					$onum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_".$class_r[$r[newsclassid]][tbname]." where title like '%$c_title%' and classid='$r[newsclassid]' limit 1");
					if($onum)
					{
						$next=1;
						break;
					...
					/*采集比较
					$onum=$Avatar->num("select id from {$dbtbpre}ecms_infotmp_".$r[tbname]." where title like '%$c_title%' limit 1");
					if($onum)
					{
						$next=1;
						break;
		...
		{continue;}
		//入临时库
		$isql=$Avatar->query("insert into {$dbtbpre}ecms_infotmp_".$r[tbname]."(classid,oldurl,checked,keyboard,newstime,truetime,tmptime,userid,username".$ifield.") values($classid,'$nr[newsurl]',0,'".addslashes($keyboard)."','$newstime',$truetime,'$tmptime',$userid,'".addslashes($username)."'".$ivalue.");");
    }
	//采集结束
	if(empty($b))
	{
		//删除记录
		$del=$Avatar->query("delete from {$dbtbpre}enewslinktmp where checkrnd='$checkrnd'");
		//更新最后时间
		$ucjsql=$Avatar->query("update {$dbtbpre}enewsinfoclass set lasttime='".time()."' where classid='$classid'");
		//自动入库
		if($r['justloadin'])
		{
...
function ViewGetNewsInfo($classid,$newspage,$userid,$username){
	...
	$record="<!--record-->";
	$field="<!--field--->";
	$mr=$Avatar->fetch1("select cj from {$dbtbpre}enewsmod where mid='".$class_r[$r[newsclassid]][modid]."'");
	$cjr=explode($record,$mr[cj]);
	$count=count($cjr);
	//采集时间
	$newstime=date("Y-m-d H:i:s");
	...
	if(empty($r[recjtheurl]))
	{
		$tmpnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$r[tbname]." where oldurl='$newspage' limit 1");
		if($tmpnum)
		{
			echo $fun_r['ReCj'];
			exit();
...
function LoadOutCj($classid,$userid,$username){
	...
	//副表
	$infocr=$Avatar->fetch1("select * from {$dbtbpre}ecms_infoclass_".$cr[tbname]." where classid='$classid'");
	...
	$cjstr=AvatarCMS_VERSION.$recordexp.$mainstr.$recordexp.$datafield.$recordexp.$datastr;
	$file=$cr['tbname'].time().".cj";
	$filepath=ECMS_PATH."afdglxt/data/tmp/cj/".$file;
...
function LoadOutCjDataField($tid,$tbname){
	...
	$field='';
	$dh='';
	$fsql=$Avatar->query("select f from {$dbtbpre}enewsf where tid='$tid' and iscj=1");
...
function LoadInCj($add,$file,$file_name,$file_type,$file_size,$userid,$username){
	...
	$cr=$Avatar->fetch1("select classid,islast,tid,tbname from {$dbtbpre}enewsclass where classid='$classid' and islast=1");
	if(!$cr['classid'])
	{
	...
	$path=ECMS_PATH.'afdglxt/data/tmp/cj/uploadcj'.time().'.cj';
	//上传文件
	...
	$cjr=$Avatar->fetch1("select classname from {$dbtbpre}enewsinfoclass where classid='$infoclassid'");
	//操作日志
...
function LoadInCjInsertMainstr($classid,$tid,$tbname,$mainstr,$fieldexp,$sfieldexp){
	...
	$Avatar->query("insert into {$dbtbpre}enewsinfoclass(classid,bclassid,newsclassid,tid,tbname,".$mainfield.") values(NULL,'0','$classid','$tid','$tbname'".$mainvalues.");");
...
function LoadInCjInsertDatastr($classid,$tid,$tbname,$infoclassid,$dataf,$datastr,$fieldexp,$sfieldexp){
	...
	if(empty($dataf))
	{
		$Avatar->query("insert into {$dbtbpre}ecms_infoclass_".$tbname."(classid) values('$infoclassid');");
		return '';
	}
	//返回表字段
	...
	$Avatar->query("insert into {$dbtbpre}ecms_infoclass_".$tbname."(classid".$datafield.") values('$infoclassid'".$datavalues.");");
}
?>