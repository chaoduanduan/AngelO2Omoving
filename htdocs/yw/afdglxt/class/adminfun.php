...
function InsertErrorLoginNum($username,$password,$loginauth,$ip,$time){
	...
	$Avatar->query("delete from {$dbtbpre}enewsloginfail where lasttime<$chtime");
	$r=$Avatar->fetch1("select ip from {$dbtbpre}enewsloginfail where ip='$ip' limit 1");
	if($r['ip'])
	{
		$Avatar->query("update {$dbtbpre}enewsloginfail set num=num+1,lasttime='$time' where ip='$ip' limit 1");
	}
	else
	{
		$Avatar->query("insert into {$dbtbpre}enewsloginfail(ip,num,lasttime) values('$ip',1,'$time');");
	}
	//日志
...
function CheckLoginNum($ip,$time){
	...
	//数据库验证
	$chtime=$time-$public_r['logintime']*60;
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsloginfail where ip='$ip' and num>=$public_r[loginnum] and lasttime>$chtime limit 1");
	if($num)
	{
		printerror("LoginOutNum","index.php");
	}
...
function login($username,$password,$key,$post){
	global $Avatar,$public_r,$dbtbpre,$ecms_config;
	...
	$user_r=$Avatar->fetch1("select userid,password,salt,salt2,lasttime,lastip,addtime,addip,userprikey,lastipport,addipport from {$dbtbpre}enewsuser where username='".$username."' and checked=0 limit 1");
	if(!$user_r['userid'])
	{
		InsertErrorLoginNum($username,$password,0,$loginip,$logintime);
		printerror("LoginFail","index.php");
	}
	$ch_password=DoAvatarCMSAdminPassword($password,$user_r['salt'],$user_r['salt2']);
	...
	//安全问答
	$user_addr=$Avatar->fetch1("select userid,equestion,eanswer,openip,certkey from {$dbtbpre}enewsuseradd where userid='$user_r[userid]'");
	if(!$user_addr['userid'])
	{
		InsertErrorLoginNum($username,$password,0,$loginip,$logintime);
		printerror("LoginFail","index.php");
	...
	$sql=$Avatar->query("update {$dbtbpre}enewsuser set rnd='$rnd',loginnum=loginnum+1,lastip='$loginip',lasttime='$logintime',pretime='$user_r[lasttime]',preip='".RepPostVar($user_r[lastip])."',lastipport='$loginipport',preipport='".RepPostVar($user_r[lastipport])."' where username='$username' limit 1");
	$r=$Avatar->fetch1("select groupid,userid,styleid,userprikey from {$dbtbpre}enewsuser where username='$username' limit 1");
	//样式
	if(empty($r[styleid]))
	{
	...
	{
		$styler=$Avatar->fetch1("select path,styleid from {$dbtbpre}enewsadminstyle where styleid='$r[styleid]'");
	...
	//设置备份
	$cdbdata=0;
	$bnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsgroup where groupid='$r[groupid]' and dodbdata=1");
	if($bnum)
	{
		$cdbdata=1;
		$set5=esetcookie("ecmsdodbdata","Avatarcms",0,1);
    }
	else
	{
	...
	$set5=esetcookie("eloginlic","Avatarcmslic",0,1);
	...
	if($set1&&$set2&&$set3)
	{
		...
		?>
			<script>
			AdminWin=window.open("<?=$cache_url?>","AvatarCMS","scrollbars");
			AdminWin.moveTo(0,0);
...
function insert_log($username,$password,$status,$loginip,$loginauth){
	global $Avatar,$ecms_config,$dbtbpre;
	...
	$sql=$Avatar->query("insert into {$dbtbpre}enewslog(username,loginip,logintime,status,password,loginauth,ipport) values('$username','$loginip','$logintime','$status','$password','$loginauth','$ipport');");
}
...
function loginout($userid,$username,$rnd){
	...
	//取得随机密码
	$rnd=make_password(20);
	$sql=$Avatar->query("update {$dbtbpre}enewsuser set rnd='$rnd' where userid='$userid'");
