...
function AddMemberF($add,$userid,$username){
	...
	//字段是否重复
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}enewsmemberadd");
	$b=0;
	...
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}enewsmember");
	$b=0;
	while($r=$Avatar->fetch($s))
	{
		if($r[Field]==$add[f])
	...
	//新增字段
	$asql=$Avatar->query("alter table {$dbtbpre}enewsmemberadd add ".$field);
	//替换代码
	...
	$sql=$Avatar->query("insert into {$dbtbpre}enewsmemberf(f,fname,fform,fhtml,fzs,myorder,ftype,flen,fvalue,fformsize) values('$add[f]','$add[fname]','$add[fform]','".eaddslashes2($fhtml)."','".eaddslashes($add[fzs])."',$add[myorder],'$add[ftype]','$add[flen]','".eaddslashes2($add[fvalue])."','$add[fformsize]');");
...
function EditMemberF($add,$userid,$username){
	...
	$field="`".$add[f]."` ".$type." NOT NULL".$def;
	$usql=$Avatar->query("alter table {$dbtbpre}enewsmemberadd change `".$add[oldf]."` ".$field);
	//替换代码
	...
	//处理变量
	$add[myorder]=(int)$add[myorder];
	$sql=$Avatar->query("update {$dbtbpre}enewsmemberf set f='$add[f]',fname='$add[fname]',fform='$add[fform]',fhtml='".eaddslashes2($fhtml)."',fzs='".eaddslashes($add[fzs])."',myorder=$add[myorder],ftype='$add[ftype]',flen='$add[flen]',fvalue='".eaddslashes2($add[fvalue])."',fformsize='$add[fformsize]' where fid=$fid");
	//更新表单
	$record="<!--record-->";
	$field="<!--field--->";
	...
	$slike=",".$add[oldf].",";
	$newslike=",".$add[f].",";
	$csql=$Avatar->query("select fid,ftemp,enter,mustenter,filef,imgf,tobrf,viewenter,searchvar,canaddf,caneditf,checkboxf from {$dbtbpre}enewsmemberform where enter like '%$like%'");
	while($cr=$Avatar->fetch($csql))
	{
		...
		$cusql=$Avatar->query("update {$dbtbpre}enewsmemberform set enter=REPLACE(enter,'$like','$newlike')".$setf." where fid='$cr[fid]'");
		//生成表单页面
...
function DelMemberF($add,$userid,$username){
	...
	$r=$Avatar->fetch1("select f from {$dbtbpre}enewsmemberf where fid=$fid");
	if(!$r[f]){
		printerror("EmptyFid","history.go(-1)");
	}
	$usql=$Avatar->query("alter table {$dbtbpre}enewsmemberadd drop COLUMN `".$r[f]."`");
	$sql=$Avatar->query("delete from {$dbtbpre}enewsmemberf where fid=$fid");
	//更新表单表
	$record="<!--record-->";
	$field="<!--field--->";
	...
	while($cr=$Avatar->fetch($csql))
	{
		...
		$cusql=$Avatar->query("update {$dbtbpre}enewsmemberform set enter='$enter'".$setf." where fid='$cr[fid]'");
	}
	if($usql&&$sql)
	{
...
function EditMemberFOrder($fid,$myorder,$userid,$username){
	...
	for($i=0;$i<count($myorder);$i++)
	{
		...
		$usql=$Avatar->query("update {$dbtbpre}enewsmemberf set myorder=$newmyorder where fid='$fid[$i]'");
    }
...
function ChangeMemberForm($fid,$mtemp){
	...
	$sql=$Avatar->query("select f,fhtml from {$dbtbpre}enewsmemberf");
...
function AddMemberForm($add,$userid,$username){
	...
	$sql=$Avatar->query("insert into {$dbtbpre}enewsmemberform(fname,ftemp,fzs,enter,mustenter,filef,imgf,tobrf,viewenter,searchvar,canaddf,caneditf,checkboxf) values('$add[fname]','".eaddslashes2($add[ftemp])."','".addslashes($add[fzs])."','$enter','$mustenter','$filef','$imgf','$tobrf','$viewenter','$searchvar','$canaddf','$caneditf','$checkboxf');");
	$fid=$Avatar->lastid();
	//生成表单页面
...
function EditMemberForm($add,$userid,$username){
	...
	$sql=$Avatar->query("update {$dbtbpre}enewsmemberform set fname='$add[fname]',ftemp='".eaddslashes2($add[ftemp])."',fzs='".addslashes($add[fzs])."',enter='$enter',mustenter='$mustenter',filef='$filef',imgf='$imgf',tobrf='$tobrf',viewenter='$viewenter',searchvar='$searchvar',canaddf='$canaddf',caneditf='$caneditf',checkboxf='$checkboxf' where fid=$fid");
	//生成表单页面
	ChangeMemberForm($fid,$add[ftemp]);
	if($sql)
	{
...
function DelMemberForm($add,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"memberf");
	$r=$Avatar->fetch1("select fid,fname from {$dbtbpre}enewsmemberform where fid=$fid;");
	if(!$r['fid'])
	{
		printerror("EmptyMemberFormId","history.go(-1)");
	}
	$sql=$Avatar->query("delete from {$dbtbpre}enewsmemberform where fid=$fid;");
	//删除表单文件
	$file="../data/html/memberform".$fid.".php";
	DelFiletext($file);
	if($sql)
...
function AddMemberGroup($add,$userid,$username){
	...
	$sql=$Avatar->query("insert into {$dbtbpre}enewsmembergroup(groupname,level,checked,favanum,daydown,msglen,msgnum,canreg,formid,regchecked,spacestyleid,dayaddinfo,infochecked,plchecked) values('$add[groupname]',$add[level],$add[checked],$add[favanum],$add[daydown],$msglen,$msgnum,$canreg,$formid,$regchecked,$spacestyleid,'$add[dayaddinfo]','$add[infochecked]','$add[plchecked]');");
	$groupid=$Avatar->lastid();
	//更新缓存
	GetMemberLevel();
	if($sql)
	{
...
function EditMemberGroup($add,$userid,$username){
	...
	$add[infochecked]=(int)$add[infochecked];
	$add[plchecked]=(int)$add[plchecked];
	$sql=$Avatar->query("update {$dbtbpre}enewsmembergroup set groupname='$add[groupname]',level=$add[level],checked=$add[checked],favanum=$add[favanum],daydown=$add[daydown],msglen=$msglen,msgnum=$msgnum,canreg=$canreg,formid=$formid,regchecked=$regchecked,spacestyleid=$spacestyleid,dayaddinfo='$add[dayaddinfo]',infochecked='$add[infochecked]',plchecked='$add[plchecked]' where groupid='$add[groupid]'");
	//更新缓存
	GetMemberLevel();
	if($sql)
	{
...
function DelMemberGroup($groupid,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"member");
	$r=$Avatar->fetch1("select groupname from {$dbtbpre}enewsmembergroup where groupid='$groupid'");
	$sql=$Avatar->query("delete from {$dbtbpre}enewsmembergroup where groupid='$groupid'");
	//更新缓存
	GetMemberLevel();
	if($sql)
	{
