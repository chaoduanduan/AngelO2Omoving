...
function eCheckIpAddInfoNum($ip,$tbname,$mid,$checked=1){
	global $Avatar,$dbtbpre,$public_r,$emod_r;
	...
	$num=$Avatar->gettotal("select count(*) as total from ".$infotb." where newstime>$cktime and infoip='$ip'");
...
function CheckQdoinfo($classid,$id,$userid,$tbname,$adminqinfo,$ecms=0){
	...
	$r=$Avatar->fetch1("select * from ".$infotb." where id='$id' and classid='$classid' and ismember=1 and userid='$userid' limit 1");
	if(!$r['id'])
	{
		printerror("HaveNotLevelQInfo","history.go(-1)",1);
	}
...
function ReturnQAddinfoF($mid,$add,$infor,$classid,$filepass,$userid,$username,$ecms=0){
	global $Avatar,$dbtbpre,$public_r,$emod_r,$ecms_config;
	$ret_r=array();
	$pr=$Avatar->fetch1("select qaddtran,qaddtransize,qaddtranimgtype,qaddtranfile,qaddtranfilesize,qaddtranfiletype,closewords,closewordsf from {$dbtbpre}enewspublic limit 1");
	$isadd=$ecms==0?1:0;
...
function DelYQTranFile($classid,$id,$file,$tf,$fstb='1'){
	...
	$count=count($r);
	$filename=$r[$count-1];
	$fr=$Avatar->fetch1("select filename,path,fileid,fpath,classid from {$dbtbpre}enewsfile_{$fstb} where classid='$classid' and id='$id' and filename='$filename' limit 1");
	if($fr['fileid'])
	{
		$sql=$Avatar->query("delete from {$dbtbpre}enewsfile_{$fstb} where fileid='$fr[fileid]'");
		DoDelFile($fr);
	}
}
...
function DodoInfo($add,$ecms=0){
	global $Avatar,$public_r,$emod_r,$level_r,$class_r,$dbtbpre,$fun_r;
	//验证来源
	if($ecms==0||$ecms==1)
	{
		CheckCanPostUrl();
	...
	if($ecms==0)
	{
		...
		//索引表
		$indexsql=$Avatar->query("insert into {$dbtbpre}ecms_".$tbname."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('$classid','$checked','$newstime','$truetime','$truetime','$havehtml');");
		$id=$Avatar->lastid();
		//返回表信息
		$infotbr=ReturnInfoTbname($tbname,$checked,$ret_r[4]);
		//主表
		$sql=$Avatar->query("insert into ".$infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard".$ret_r[0].") values('$id','$classid','$ttid',0,0,0,'$newspath','','".$muserid."','".addslashes($musername)."',0,0,0,'$isqf',1,0,'$truetime','$truetime','$havehtml',0,0,'$titlefont','','$ret_r[4]','$public_r[filedeftb]','$public_r[pldeftb]','$keyboard'".$ret_r[1].");");
		//副表
		$fsql=$Avatar->query("insert into ".$infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$ret_r[2].") values('$id','$classid','$keyid',1,'$newstempid',0,'$haveaddfen',''".$ret_r[3].");");
		//扣点记录
		if($haveaddfen)
		{
			if($cr['addinfofen']<0)
		...
		$usql=$Avatar->query("update ".$infotbr['tbname']." set filename='$filename',titleurl='$infourl' where id='$id'");
		//修改ispic
		...
		if($sql)
		{
			...
			if($add['gotoinfourl']&&$checked)//返回内容页
			{
				if($cr['showdt']==1)
				{
					$reurl=$public_r[newsurl]."afdglxt/action/ShowInfo/?classid=$classid&id=$id";
				}
				elseif($cr['showdt']==2)
				{
	...
	elseif($ecms==1)
	{
		...
		//索引表
		$indexsql=$Avatar->query("update {$dbtbpre}ecms_".$tbname."_index set lastdotime=$lastdotime,havehtml=0".$iaddfield." where id='$id'");
		//返回表信息
		$infotbr=ReturnInfoTbname($tbname,$ychecked,$infor['stb']);
		//主表
		$sql=$Avatar->query("update ".$infotbr['tbname']." set lastdotime=$lastdotime,havehtml=0,ttid='$ttid'".$addfield.$ret_r[0]." where id=$id and classid=$classid and userid='$muserid' and ismember=1");
		//副表
		$fsql=$Avatar->query("update ".$infotbr['datatbname']." set classid='$classid'".$faddfield.$ret_r[3]." where id='$id'");
		//修改ispic
	...
	elseif($ecms==2)
	{
		...
		$indexsql=$Avatar->query("delete from {$dbtbpre}ecms_".$tbname."_index where id='$id'");
		$sql=$Avatar->query("delete from ".$infotbr['tbname']." where id=$id and classid=$classid and userid='$muserid' and ismember=1");
...
function DoQCheckAddNum($userid,$groupid){
	global $Avatar,$dbtbpre,$level_r,$public_r;
	$ur=$Avatar->fetch1("select userid,todayinfodate,todayaddinfo from {$dbtbpre}enewsmemberpub where userid='$userid' limit 1");
	$thetoday=date("Y-m-d");
	if($ur['userid'])
	{
...
function DoQTranFile($add,$file,$file_name,$file_type,$file_size,$userid,$username,$rnd,$ecms=0){
	global $Avatar,$dbtbpre,$class_r,$public_r,$ecms_config;
	...
	if($infoid)
	{
		$index_r=$Avatar->fetch1("select classid,checked from {$dbtbpre}ecms_".$class_r[$classid][tbname]."_index where id='$infoid'");
		if(!$index_r['classid']||$classid!=$index_r['classid'])
		{
			$ecms!=1?printerror("EmptyQTranFile","",9):ECMS_QEditorPrintError(1,'','','EmptyQTranFile','','');
		}
		$infotb=ReturnInfoMainTbname($class_r[$classid][tbname],$index_r['checked']);
		$infor=$Avatar->fetch1("select classid,fstb from ".$infotb." where id='$infoid'");
	...
	$type=(int)$add['type'];
	$pr=$Avatar->fetch1("select qaddtran,qaddtransize,qaddtranimgtype,qaddtranfile,qaddtranfilesize,qaddtranfiletype from {$dbtbpre}enewspublic limit 1");
	if($type==1)//图片
	{
		if(!$pr['qaddtran'])
		{
