...
function DelFile($fileid,$userid,$username){
	...
	$fstb=(int)$_GET['fstb'];
	$r=$Avatar->fetch1("select filename,path,classid,fpath from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid' limit 1");
	$sql=$Avatar->query("delete from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid'");
	DoDelFile($r);
	if($sql)
	{
		//操作日志
...
function DelFile_all($fileid,$userid,$username){
	...
	{
		$userid=(int)$userid;
		$ur=$Avatar->fetch1("select groupid,adminclass,filelevel from {$dbtbpre}enewsuser where userid='$userid' limit 1");
		if($ur['filelevel'])
		{
			$gr=$Avatar->fetch1("select dofile from {$dbtbpre}enewsgroup where groupid='$ur[groupid]'");
			if(!$gr['dofile'])
			{
	...
	for($i=0;$i<count($fileid);$i++)
	{
		...
		$r=$Avatar->fetch1("select filename,path,classid,fpath from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid[$i]' limit 1");
		$sql=$Avatar->query("delete from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid[$i]'");
		DoDelFile($r);
    }
	if($sql)
	{
...
function DelPathFile($filename,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$dfnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsfile_1 where filename='$dfile'");
		if($dfnum)
		{
			$Avatar->query("delete from {$dbtbpre}enewsfile_1 where filename='$dfile'");
			//FileServer
...
function DoMarkSmallPic($add,$userid,$username){
	global $Avatar,$class_r,$dbtbpre,$public_r,$efileftp_fr;
	//导入gd处理文件
	if($add['getsmall']||$add['getmark'])
	{
		@include(ECMS_PATH."afdglxt/class/gd.php");
	}
	else
	{
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$r=$Avatar->fetch1("select classid,filename,path,no,fpath from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid[$i]'");
