...
define('InAvatarCMSTfun',TRUE);
require_once(ECMS_PATH."afdglxt/class/userfun.php");
...
function sys_ReturnBqQuery($classid,$line,$enews=0,$do=0,$ewhere='',$eorder=''){
	global $Avatar,$public_r,$class_r,$class_zr,$navclassid,$dbtbpre,$fun_r,$class_tr,$emod_r,$etable_r,$eyh_r;
	if($enews==24)//按sql查询
	{
...
function sys_ReturnBqTemp($tempid){
	...
	$r=$Avatar->fetch1("select tempid,modid,temptext,showdate,listvar,subnews,rownum,docode from ".GetTemptb("enewsbqtemp")." where tempid='$tempid'");
	if(empty($r[tempid]))
	{
...
function sys_OtherLinkQuery($classid,$line,$enews,$doing){
	global $Avatar,$public_r,$class_r,$class_zr,$navinfor,$dbtbpre,$eyh_r,$etable_r,$class_tr;
	if($enews==1)//按表
	{
		$tbname=$classid;
	}
...
function sys_GetOtherLinkInfo($tempid,$classid='',$line=0,$strlen=60,$have_class=0,$enews=0,$doing=0){
	global $Avatar,$navinfor,$public_r;
...
function sys_GetAd($adid){
	...
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewsad where adid='$adid'");
	//到期
	...
	{
		$ad_url=$public_r[newsurl]."afdglxt/public/ClickAd?adid=".$adid;//广告链接
	}
	//----------------------文字广告
	if($r[t]==1)
	{
...
function sys_GetVote($voteid){
	global $Avatar,$public_r,$dbtbpre;
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewsvote where voteid='$voteid'");
	if(empty($r[votetext]))
	{
		return '';
	}
...
function sys_GetInfoVote($classid,$id){
	global $Avatar,$public_r,$dbtbpre;
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewsinfovote where id='$id' and classid='$classid' limit 1");
	if(empty($r[votetext]))
	{
		return '';
	}
...
function sys_GetSitelink($line,$num,$enews=0,$classid=0,$stats=0){
	...
	$sql=$Avatar->query("select * from {$dbtbpre}enewslink where checked=1".$a.$whereclass." order by myorder,lid limit ".$num);
	//输出
	$i=0;
	while($r=$Avatar->fetch($sql))
	{
		...
		if(empty($stats))
		{
			$linkurl=$public_r[newsurl]."afdglxt/public/GotoSite/?lid=".$r[lid]."&url=".urlencode($r[lurl]);
		}
		else
		{
			$linkurl=$r[lurl];
...
function sys_TotalData($classid,$enews=0,$day=0,$totaltype=0){
	global $Avatar,$class_r,$class_zr,$dbtbpre,$fun_r,$class_tr;
	if(empty($classid))
	{
		return "";
    }
...
function sys_FlashPixpic($classid,$line,$width,$height,$showtitle=true,$strlen,$enews=0,$sec=5,$ewhere='',$eorder=''){
	global $Avatar,$public_r,$class_r,$class_zr;
...
<!--
 ...
 var swfpath="<?=$public_r[newsurl]?>afdglxt/data/images/pixviewer.swf";
 var swfpatha="<?=$public_r[newsurl]?>afdglxt/data/images/pixviewer.swf";
...
function sys_ShowSearchKey($line,$num,$classid=0,$enews=0){
	...
	$sql=$Avatar->query("select searchid,keyboard from {$dbtbpre}enewssearch where iskey=0".$add." order by ".$order." desc limit ".$num);
	$i=0;
	$returnkey="";
	while($r=$Avatar->fetch($sql))
	{
		$i++;
		$keyurl=$public_r[newsurl]."afdglxt/search/result/?searchid=$r[searchid]";
		$br="";
		if($i%$line==0)
		{
			$br="<br>";
...
function sys_ForSonclassDataFirstInfo($temptext,$cr,$ecms=0,$subtitle=0,$subnews=0,$fdoing=0){
	...
	elseif($ecms==2)//推荐信息
	{
		$r=$Avatar->fetch1("select * from {$dbtbpre}ecms_".$class_r[$cr[classid]][tbname]." where isgood>0 and (".$where.")".$add." order by newstime desc limit 1");
	}
	elseif($ecms==3)//头条信息
	{
		$r=$Avatar->fetch1("select * from {$dbtbpre}ecms_".$class_r[$cr[classid]][tbname]." where firsttitle>0 and (".$where.")".$add." order by newstime desc limit 1");
	}
	elseif($ecms==4)//最新信息
	{
		$r=$Avatar->fetch1("select * from {$dbtbpre}ecms_".$class_r[$cr[classid]][tbname]." where (".$where.")".$add." order by newstime desc limit 1");
	}
	else
	{
		return $temptext;
...
function sys_ForSonclassData($classid,$line,$strlen,$have_class=0,$enews=0,$tempid,$doing=0,$cline=0,$dofirstinfo=0,$fsubtitle=0,$fsubnews=0,$fdoing=0,$ewhere='',$eorder=''){
	global $Avatar,$public_r,$class_r,$class_zr,$navclassid,$dbtbpre;
	//多栏目
	if(strstr($classid,","))
	{
	...
	$csql=$Avatar->query("select classid".$addclassfield." from {$dbtbpre}enewsclass where ".$where." and wburl='' order by myorder,classid".$limit);
	while($cr=$Avatar->fetch($csql))
	{
...
function sys_ShowClassByTemp($classid,$tempid,$show=0,$cline=0){
	global $navclassid,$Avatar,$class_r,$public_r,$dbtbpre;
	//当前栏目
	if($classid=="selfinfo")
	{
		if(empty($navclassid))
	...
	$no=1;
	$changerow=1;
	$sql=$Avatar->query("select classid,classname,islast,sonclass,tbname,intro,classimg,infos from {$dbtbpre}enewsclass where bclassid='$classid' and showclass=0 order by myorder,classid".$limit);
	while($r=$Avatar->fetch($sql))
	{
		//显示类别数据数
		if($show)
...
function sys_ForShowSonClass($classid,$tempid,$show=0,$cline=0){
	...
	$no=1;
	$sql=$Avatar->query("select classid,classname,islast,sonclass,tbname,intro,classimg,infos from {$dbtbpre}enewsclass where ".$where." and showclass=0 order by myorder,classid".$limit);
	while($r=$Avatar->fetch($sql))
	{
		//显示栏目数据数
		if($show)
...
function sys_GetShowClassMore($bclassid,$bcr,$tr,$bno,$bnum,$show=0){
	...
	$bclassimg=$bcr[classimg]?$bcr[classimg]:$public_r[newsurl]."afdglxt/data/images/notimg.gif";//栏目图片
	...
	$no=1;
	$changerow=1;
	$sql=$Avatar->query("select classid,classname,islast,sonclass,tbname,intro,classimg,infos from {$dbtbpre}enewsclass where bclassid='$bclassid' and showclass=0 order by myorder,classid");
	while($r=$Avatar->fetch($sql))
	{
		//显示栏目数据数
		if($show)
...
function ReplaceShowClassVars($no,$listtemp,$r,$num,$ecms=0,$subnews=0){
	...
	if(empty($r[classimg]))
	{
		$r[classimg]=$public_r[newsurl]."afdglxt/data/images/notimg.gif";
	}
...
function sys_ShowLyInfo($line,$tempid,$bid=0){
	...
	$no=1;
	$changerow=1;
	$sql=$Avatar->query("select lyid,name,email,lytime,lytext,retext from {$dbtbpre}enewsgbook where checked=0".$a." order by lyid desc limit ".$line);
	while($r=$Avatar->fetch($sql))
	{
		//替换列表变量
...
function sys_ShowZtData($tempid,$zcid=0,$cline=0,$classid=0){
	...
	$no=1;
	$changerow=1;
	$sql=$Avatar->query("select ztid,ztname,intro,ztimg from {$dbtbpre}enewszt where showzt=0".$a." order by myorder,ztid desc".$limit);
	while($r=$Avatar->fetch($sql))
	{
		//替换列表变量
...
function sys_ShowPlInfo($line,$tempid,$classid=0,$id=0,$isgood=0,$enews=0){
	global $Avatar,$dbtbpre,$class_r,$public_r;
	$a="";
	if($isgood)
	{
		$a.=" and isgood='$isgood'";
	...
	$no=1;
	$changerow=1;
	$sql=$Avatar->query("select plid,userid,username,saytime,id,classid,zcnum,fdnum,saytext from {$dbtbpre}enewspl_".$public_r['pldeftb']." where checked=0".$a." order by ".$order." limit ".$line);
	while($r=$Avatar->fetch($sql))
	{
		//替换列表变量
...
function ReplaceShowPlVars($no,$listtemp,$r,$formatdate,$subnews=0){
	global $public_r,$Avatar,$dbtbpre,$class_r;
	//标题
	$infor=$Avatar->fetch1("select isurl,titleurl,classid,id,title,titlepic from {$dbtbpre}ecms_".$class_r[$r[classid]][tbname]." where id='$r[id]' limit 1");
	...
	if($r['userid'])
	{
		$r['username']="<a href='".$public_r[newsurl]."afdglxt/space/?userid=$r[userid]' target='_blank'>$r[username]</a>";
	}
	if(empty($r['username']))
	{
		$r['username']='匿名';
	...
	$titlepic=$infor['titlepic']?$infor['titlepic']:$public_r['newsurl'].'afdglxt/data/images/notimg.gif';
...
function sys_ShowMemberInfo($userid=0,$fields=''){
	global $Avatar,$dbtbpre,$public_r,$navinfor,$level_r;
	...
	$r=$Avatar->fetch1("select ".$fields." from ".eReturnMemberTable()." u LEFT JOIN {$dbtbpre}enewsmemberadd ui ON u.".egetmf('userid')."=ui.userid where u.".egetmf('userid')."='$uid' limit 1");
...
function sys_ListMemberInfo($line=10,$ecms=0,$groupid=0,$userids=0,$fields=''){
	...
	$sql=$Avatar->query("select ".$fields." from ".eReturnMemberTable()." u LEFT JOIN {$dbtbpre}enewsmemberadd ui ON u.".egetmf('userid')."=ui.userid where u.".egetmf('checked')."=1".$where." order by ".$order." limit ".$line);
	return $sql;
}

//显示TAGS
function sys_eShowTags($cid,$num=0,$line=0,$order='',$isgood='',$isgoodshow='',$showjg='',$shownum=0,$cs='',$vartype=''){
	global $Avatar,$dbtbpre,$public_r,$navinfor;
	$str='';
	if(empty($showjg))
	{
		$showjg=' &nbsp; ';
	...
	if($cid=='selfinfo')
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			{
				$tagsurl=$public_r[newsurl].'afdglxt/tags/?tagname='.urlencode($r[$i]).$cs;
			}
	...
	else
	{
		...
		$jg='';
		$snum='';
		$sql=$Avatar->query("select tagid,tagname,num,isgood from {$dbtbpre}enewstags".$where." order by".$order.$limit);
		while($r=$Avatar->fetch($sql))
		{
			if($shownum)
			{
			...
			{
				$tagsurl=$public_r[newsurl].'afdglxt/tags/?'.($vartype=='tagid'?'tagid='.$r[tagid]:'tagname='.urlencode($r[tagname])).$cs;
			}
...
function sys_ReturnTogQuery($type,$id,$line,$classid='',$mid='',$ewhere='',$eorder='',$selectf=''){
	global $Avatar,$public_r,$class_r,$class_zr,$navclassid,$dbtbpre,$class_tr,$emod_r;
	if($type=='tags')//TAGS
	{
		$idf='tagid';
...
function sys_eShowTagsInfo($tagid,$line,$strlen,$tempid,$classid='',$mid=''){
	...
	while($r=$Avatar->fetch($sql))
	{
		...
		$infor=$Avatar->fetch1("select * from {$dbtbpre}ecms_".$class_r[$r[classid]][tbname]." where id='$r[id]' limit 1");
		if(empty($infor['id']))
		{
			continue;
		}
...
function sys_eShowSpInfo($spvar,$line=10,$strlen=0){
	...
	$spr=$Avatar->fetch1("select spid,spname,sppic,spsay,tempid,sptype from {$dbtbpre}enewssp where varname='$spvar' limit 1");
...
function sys_eShowSp1($spid,$spr,$line,$strlen){
	...
	$no=1;
	$changerow=1;
	$sql=$Avatar->query("select sid,title,titlepic,bigpic,titleurl,smalltext,titlefont,newstime,titlepre,titlenext from {$dbtbpre}enewssp_1 where spid='$spid' order by newstime desc limit ".$line);
	while($r=$Avatar->fetch($sql))
	{
		$r[oldtitle]=$r[title];
		//替换列表变量
...
function sys_eShowSp3($spid){
	...
	$r=$Avatar->fetch1("select sptext from {$dbtbpre}enewssp_3 where spid='$spid' limit 1");
	echo $r['sptext'];
}

//调用生成缩图
function sys_ResizeImg($file,$width,$height,$docut=0,$target_filename='',$target_path='afdglxt/data/tmp/titlepic/'){
...
function sys_ModShowMorepic($epicswidth=120,$epicsheight=80,$temptext=''){
	...
	if(empty($temptext))
	{
		$temptext='<table><tr>[!--Avatarnews.listtemp--]<td bgcolor="#cccccc" align="center" id="espicid\'+i+\'"\'+cname+\'><a href="#Avatarcms" onclick="ecmsShowPic(\'+i+\');"><img src="\'+ecmspicr[i][0]+\'" width="\'+epicswidth+\'" height="\'+epicsheight+\'" border="0"></a><br>\'+(i+1)+\'/\'+ecmspicnum+\'</td>[!--Avatarnews.listtemp--]</tr></table>';
	}
	else
	{
	...
	$tempr=explode('[!--Avatarnews.listtemp--]',$temptext);
	//图片地址
	$rexp="\r\n";
	$fexp="::::::";
	$rr=explode($rexp,$morepic);
...
function ecmsShowSmallPics(){
	...
	for(i=0;i<ecmspicnum;i++)
	{
		...
		listpages+='<a href="#Avatarcms" id="epiclpid'+i+'" onclick="ecmsShowPic('+i+');"'+lname+'>'+(i+1)+'</a> ';
	}
	if(eopensmallpics==1)
	{
...
<script type="text/javascript" src="<?=$public_r['newsurl']?>afdglxt/data/modadd/morepic/Avatarcmsmorepic.js"></script>
