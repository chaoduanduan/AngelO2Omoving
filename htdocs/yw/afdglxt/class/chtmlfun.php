...
function ReNewsHtml($start,$classid,$from,$retype,$startday,$endday,$startid,$endid,$tbname,$havehtml){
	global $Avatar,$public_r,$class_r,$fun_r,$dbtbpre,$etable_r;
	...
	$b=0;
	$sql=$Avatar->query("select id,classid from {$dbtbpre}ecms_".$tbname."_index where ".$yhadd."id>$start".$add1." and checked=1 order by id limit ".$public_r[renewsnum]);
	...
	if(empty($b))
	{
		...
		if($yhadd.$add1=='')
		{
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_index set havehtml=1 where havehtml=0 and checked=1");
			$Avatar->query("update {$dbtbpre}ecms_".$tbname." set havehtml=1 where havehtml=0");
		}
		else
		{
			if($updatehavehtml)
			...
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_index set havehtml=1 where ".$truewhereindex.$updatehavehtml);
			$Avatar->query("update {$dbtbpre}ecms_".$tbname." set havehtml=1 where ".$truewhere.$updatehavehtml);
		}
...
function ReListHtml_all($start,$do,$from){
	global $Avatar,$public_r,$fun_r,$class_r,$dbtbpre;
	$start=(int)$start;
	$b=0;
	if($do=="all")
	{
	...
	//栏目
	$sql=$Avatar->query("select classid,classtempid,islast,islist from {$dbtbpre}enewsclass where classid>$start and nreclass=0 order by classid limit ".$public_r[relistnum]);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		if(!$r[islast])//大栏目
...
function ReZtListHtml_all($start,$do,$from){
	...
	{
		$zcsql=$Avatar->query("select cid from {$dbtbpre}enewszttype where cid>$start and (endtime=0 or endtime>$time) order by cid limit ".$public_r['relistnum']);
		while($c_r=$Avatar->fetch($zcsql))
		{
			$b=1;
	...
	$zsql=$Avatar->query("select ztid from {$dbtbpre}enewszt where ztid>$start and (endtime=0 or endtime>$time) order by ztid limit ".$public_r['relistnum']);
	while($z_r=$Avatar->fetch($zsql))
	{
		$b=1;
...
function DoReZtListHtml($ztid,$ecms=0){
	...
	if($ecms==1)
	{
		$csql=$Avatar->query("select cid from {$dbtbpre}enewszttype where ztid='$ztid'");
...
function ReTtListHtml_all($start,$do,$from){
	...
	$start=(int)$start;
	$b=0;
	$tsql=$Avatar->query("select typeid from {$dbtbpre}enewsinfotype where typeid>$start and listdt=0 order by typeid limit ".$public_r['relistnum']);
	while($t_r=$Avatar->fetch($tsql))
	{
		$b=1;
...
function ReAllNewsJs($start,$do,$from){
	...
	if($do=="all")
	{
		$pr=$Avatar->fetch1("select hotnum,newnum,goodnum,hotplnum,firstnum,jstempid from {$dbtbpre}enewspublic limit 1");
	...
	{
		//$from=urlencode($from);
		$sql=$Avatar->query("select typeid,newline,hotline,goodline,hotplline,firstline,jstempid from {$dbtbpre}enewsinfotype where typeid>$start and nrejs=0 order by typeid limit $line");
		while($r=$Avatar->fetch($sql))
		{
	...
	{
		//$from=urlencode($from);
		$sql=$Avatar->query("select classid,newline,hotline,goodline,hotplline,firstline,jstempid from {$dbtbpre}enewsclass where classid>$start and nrejs=0 and wburl='' order by classid limit $line");
		while($r=$Avatar->fetch($sql))
		{
			$jstemptext=GetTheJstemp($r[jstempid]);//js模板
			$b=1;
...
function ReHot_NewNews(){
	...
	$public_r=$Avatar->fetch1("select hotnum,newnum,goodnum,hotplnum,firstnum,jstempid from {$dbtbpre}enewspublic limit 1");
...
function ReSingleJs($classid,$doing=0){
	...
	if($doing==0)
	{
		$r=$Avatar->fetch1("select classid,newline,hotline,goodline,hotplline,firstline,jstempid from {$dbtbpre}enewsclass where classid='$classid'");
		$jstemptext=GetTheJstemp($r[jstempid]);//js模板
		GetNewsJs($r[classid],$r[newline],$r[newstrlen],$r[newshowdate],0,$jstemptext);
		GetNewsJs($r[classid],$r[hotline],$r[hotstrlen],$r[hotshowdate],1,$jstemptext);
		GetNewsJs($r[classid],$r[goodline],$r[goodstrlen],$r[goodshowdate],2,$jstemptext);
	...
	{
		$r=$Avatar->fetch1("select typeid,newline,hotline,goodline,hotplline,firstline,jstempid from {$dbtbpre}enewsinfotype where typeid='$classid'");
		$jstemptext=GetTheJstemp($r[jstempid]);//js模板
		GetNewsJs($r[typeid],$r[newline],$r[newstrlen],$r[newshowdate],25,$jstemptext);
		GetNewsJs($r[typeid],$r[hotline],$r[hotstrlen],$r[hotshowdate],26,$jstemptext);
		GetNewsJs($r[typeid],$r[goodline],$r[goodstrlen],$r[goodshowdate],27,$jstemptext);
...
function ReUserpageAll($start=0,$from,$userid,$username){
	...
	$start=(int)$start;
	$b=0;
	$sql=$Avatar->query("select id,path,pagetext,title,pagetitle,pagekeywords,pagedescription,tempid from {$dbtbpre}enewspage where id>$start order by id limit ".$public_r['reuserpagenum']);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$r[id];
...
function ReUserlistAll($start=0,$from,$userid,$username){
	...
	$start=(int)$start;
	$b=0;
	$sql=$Avatar->query("select listid,pagetitle,filepath,filetype,totalsql,listsql,maxnum,lencord,listtempid,pagekeywords,pagedescription from {$dbtbpre}enewsuserlist where listid>$start order by listid limit ".$public_r['reuserlistnum']);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$r[listid];
...
function ReUserjsAll($start=0,$from,$userid,$username){
	...
	$start=(int)$start;
	$b=0;
	$sql=$Avatar->query("select jsid,jsname,jssql,jstempid,jsfilename from {$dbtbpre}enewsuserjs where jsid>$start order by jsid limit ".$public_r['reuserjsnum']);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$r[jsid];
...
function ReSpAll($start=0,$from,$userid,$username){
	...
	$start=(int)$start;
	$b=0;
	$sql=$Avatar->query("select spid,varname,refile,spfile,spfileline,spfilesub from {$dbtbpre}enewssp where refile=1 and spid>$start order by spid limit ".$public_r['reuserpagenum']);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$r['spid'];
...
function GoReListHtmlMore($classid,$gore,$from,$ecms=0){
	...
	if(empty($gore))
	{
		$caity="ReListHtmlMore";
	}
	elseif($gore==2)//标题分类
	{
		$caity="ReListTtHtmlMore";
	}
	else//专题
	{
		$caity="ReListZtHtmlMore";
	}
	echo"<script>self.location.href='ecmschtml.php?enews=$caity&classid=$cid&from=".urlencode($from).hReturnEcmsHashStrHref(0)."&ecms=$ecms';</script>";
	exit();
}
...
function ReListHtmlMore($start,$classid,$from){
	...
	$b=0;
	$sql=$Avatar->query("select classid,classtempid,islast,islist from {$dbtbpre}enewsclass where classid>$start and classid in(".$classid.") order by classid limit ".$public_r[relistnum]);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		//大栏目
...
function ReListZtHtmlMore($start,$classid,$from,$ecms=0){
	...
	$b=0;
	//刷新专题
	$zsql=$Avatar->query("select ztid from {$dbtbpre}enewszt where ztid>$start and ztid in (".$classid.") order by ztid limit ".$public_r[relistnum]);
    while($z_r=$Avatar->fetch($zsql))
	{
		$b=1;
...
function ReListTtHtmlMore($start,$classid,$from){
	...
	$b=0;
	//刷新标题分类
	$tsql=$Avatar->query("select typeid from {$dbtbpre}enewsinfotype where typeid>$start and typeid in (".$classid.") order by typeid limit ".$public_r[relistnum]);
    while($t_r=$Avatar->fetch($tsql))
	{
		$b=1;
		ListHtml($t_r[typeid],$ret_r,5);
...
function ReSingleInfo($userid,$username){
	global $Avatar,$public_r,$class_r,$dbtbpre;
	if($_GET['classid'])
	{
	...
	$sql=$Avatar->query("select * from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where ".$add);
	while($r=$Avatar->fetch($sql))
	{
...
function ReClassPath($start=0){
	...
	$start=(int)$start;
	$sql=$Avatar->query("select classid,classpath,islast from {$dbtbpre}enewsclass where wburl='' and classid>$start order by classid limit ".$public_r[relistnum]);
	$b=0;
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
	...
	if(empty($b))
	{
		$zsql=$Avatar->query("select ztid,ztpath from {$dbtbpre}enewszt order by ztid");
		while($zr=$Avatar->fetch($zsql))
		{
			CreateZtPath($zr[ztpath]);
		}
		//恢复标题分类目录
		$tsql=$Avatar->query("select typeid,tpath from {$dbtbpre}enewsinfotype order by typeid");
		while($tr=$Avatar->fetch($tsql))
		{
...
function UpdateClassInfosAll($add){
	...
	$b=0;
	//栏目
	$sql=$Avatar->query("select classid from {$dbtbpre}enewsclass where classid>$start and islast=1 order by classid limit ".$public_r[relistnum]);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$end_classid=$r['classid'];
