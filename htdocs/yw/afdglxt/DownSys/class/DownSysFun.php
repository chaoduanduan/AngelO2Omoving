...
function DownSoft($classid,$id,$pathid,$p,$pass){
	global $Avatar,$dbtbpre,$public_r,$level_r,$class_r,$emod_r,$ecms_config;
	//验证IP
	...
	$ok=1;
	$r=$Avatar->fetch1("select * from {$dbtbpre}ecms_".$tbname." where id='$id' limit 1");
	if(empty($r['id'])||$r['classid']!=$classid)
	{
		printerror("ExiestSoftid","history.go(-1)",1);
	}
	//副表
	$finfor=$Avatar->fetch1("select ".ReturnSqlFtextF($mid)." from {$dbtbpre}ecms_".$tbname."_data_".$r[stb]." where id='$r[id]' limit 1");
	...
	if($downgroup)
	{
		...
		$rnd=RepPostVar($rnd);
		//取得会员资料
		$u=$Avatar->fetch1("select ".eReturnSelectMemberF('*')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' and ".egetmf('rnd')."='$rnd' limit 1");
		if(empty($u['userid']))
		...
		{
			//---------是否有历史记录
			$bakr=$Avatar->fetch1("select id,truetime from {$dbtbpre}enewsdownrecord where id='$id' and classid='$classid' and userid='$userid' and pathid='$pathid' and online=0 order by truetime desc limit 1");
			...
			else
			{
				...
				else
				{
					...
					//去除点数
					$usql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('userfen')."=".egetmf('userfen')."-".$showdown_r[3]." where ".egetmf('userid')."='$userid'");
				}
				//备份下载记录
		...
		if($setuserday)
		{
			$usql=$Avatar->query($setuserday);
		}
	}
	//总下载数据增一
    $usql=$Avatar->query("update {$dbtbpre}ecms_".$class_r[$classid][tbname]." set totaldown=totaldown+1 where id='$id'");
    $downurl=stripSlashes($showdown_r[1]);
	$downurlr=ReturnDownQzPath($downurl,$showdown_r[4]);
	$downurl=$downurlr['repath'];
	//防盗链
	@include(ECMS_PATH."afdglxt/DownSys/class/enpath.php");
...
function GetSofturl($classid,$id,$pathid,$p,$pass,$onlinetime,$onlinepass){
	global $Avatar,$dbtbpre,$public_r,$class_r,$emod_r,$level_r,$ecms_config;
	//验证IP
	...
	if($downgroup)
	{
		...
		{
			//---------是否有历史记录
		    $bakr=$Avatar->fetch1("select id,truetime from {$dbtbpre}enewsdownrecord where id='$id' and classid='$classid' and userid='$userid' and pathid='$pathid' and online=1 order by truetime desc limit 1");
			if($bakr[id]&&(time()-$bakr[truetime]<=$public_r[redodown]*3600))
			{}
			else
			{
