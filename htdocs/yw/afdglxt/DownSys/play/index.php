...
function ViewOnlineKFen($showdown_r,$u,$userid,$classid,$id,$pathid,$r){
	global $level_r,$class_r,$dbtbpre,$public_r,$Avatar,$have_bak,$have_fen;
	if($showdown_r[2])
	{
		//下载次数限制
		$setuserday="";
...
if($downgroup)
{
	...
	{
		//---------是否有历史记录
			$bakr=$Avatar->fetch1("select id,truetime from {$dbtbpre}enewsdownrecord where id='$id' and classid='$classid' and userid='$user[userid]' and pathid='$pathid' and online=1 order by truetime desc limit 1");
...
{
	$playerr=$Avatar->fetch1("select filename from {$dbtbpre}enewsplayer where id='$r[playerid]'");
	if($playerr['filename'])
	{
