...
if($downgroup)
{
	$user=islogin();
	//取得会员资料
	$u=$Avatar->fetch1("select ".eReturnSelectMemberF('*')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$user[userid]' and ".egetmf('rnd')."='$user[rnd]' limit 1");
	if(empty($u['userid']))
	{
	...
	{
		//---------是否有历史记录
		$bakr=$Avatar->fetch1("select id,truetime from {$dbtbpre}enewsdownrecord where id='$id' and classid='$classid' and userid='$user[userid]' and pathid='$pathid' and online=0 order by truetime desc limit 1");
...
$titlepic=$r['titlepic']?$r['titlepic']:$public_r[newsurl]."afdglxt/data/images/notimg.gif";
$ip=egetip();
