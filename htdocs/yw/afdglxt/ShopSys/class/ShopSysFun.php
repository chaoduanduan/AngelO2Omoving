...
function ShopSys_ReturnSet(){
	...
	$shoppr=$Avatar->fetch1("select * from {$dbtbpre}enewsshop_set limit 1");
	return $shoppr;
}
...
function Shopsys_CutMaxnum($ddid,$buycar,$havecut,$shoppr,$ecms=0){
	global $class_r,$Avatar,$dbtbpre,$public_r;
	if(empty($buycar))
	{
		return '';
	}
	...
	for($i=0;$i<$bcount-1;$i++)
	{
		...
		$Avatar->query("update {$dbtbpre}ecms_".$class_r[$classid][tbname]." set pmaxnum=pmaxnum".$fh.$pnum.",psalenum=psalenum".$salefh.$pnum." where id='$id'");
	}
	$newhavecut=$ecms==0?1:0;
	$Avatar->query("update {$dbtbpre}enewsshopdd set havecutnum='$newhavecut' where ddid='$ddid'");
}
...
function AddBuycar($classid,$id,$pn=1,$add){
	...
	//验证产品是否存在
	$infor=$Avatar->fetch1("select id,classid,pmaxnum,price,buyfen from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where id='$id' limit 1");
...
function ShopCheckAddDdGroup($shoppr){
	...
	if($shoppr['shopddgroupid'])
	{
		if(!getcvar('mluserid'))
		{
			...
			$gotourl=$public_r['newsurl']."afdglxt/member/login/login.php?prt=1&from=".$phpmyself;
			$petype=1;
...
function ShopSys_GetPre($precode,$totalmoney,$user,$classids){
	...
	$prer=$Avatar->fetch1("select id,prename,precode,premoney,pretype,reuse,endtime,groupid,classid,musttotal,usenum,haveusenum from {$dbtbpre}enewsshop_precode where precode='$precode' limit 1");
	if(!$prer['id'])
	{
		printerror("EmptyPreCode","history.go(-1)",1);
	}
...
function AddDd($add){
	...
	{
		$pr=$Avatar->fetch1("select pid,pname,price from {$dbtbpre}enewsshopps where pid='$add[psid]' and isclose=0");
		if(empty($pr['pid']))
		{
			printerror("NotPsid","history.go(-1)",1);
		}
	...
	{
		$payr=$Avatar->fetch1("select payid,payname,payurl,userpay,userfen from {$dbtbpre}enewsshoppayfs where payid='$add[payfsid]' and isclose=0");
		if(empty($payr['payid']))
		{
			printerror("NotPayfsid","history.go(-1)",1);
		}
	...
	if($payr[userfen])	//直接扣点
	{
		...
		else
		{
			if($userid)
			{
				...
				//扣除点数
				$usql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('userfen')."=".egetmf('userfen')."-".$buyallfen." where ".egetmf('userid')."='$userid'");
				if($usql)
				{
					$mess="AddDdSuccessa";
					$payby=1;
	...
	elseif($payr[userpay])	//帐号余额扣除
	{
		    if($userid)
			{
				...
				//扣除金额
				$usql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('money')."=".egetmf('money')."-".$buyallmoney." where ".egetmf('userid')."='$userid'");
				if($usql)
				{
					$mess="AddDdSuccessa";
					$payby=2;
	...
	if($prer['id'])
	{
		...
		if($prer['reuse']==0)
		{
			$Avatar->query("delete from {$dbtbpre}enewsshop_precode where id='".$prer['id']."'");
		}
		elseif($prer['reuse']&&$prer['usenum'])
		{
			...
			{
				$Avatar->query("update {$dbtbpre}enewsshop_precode set haveusenum=haveusenum+1 where id='".$prer['id']."'");
			}
		}
	}
	if($sql)
...
function ReturnBuycardd($shoppr){
	...
	for($i=0;$i<count($r)-1;$i++)
	{
		...
		//取得产品信息
		$productr=$Avatar->fetch1("select title,tprice,price,isurl,titleurl,classid,id,titlepic,buyfen,pmaxnum from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where id='$id' limit 1");
...
function ShopDdToPay($ddid){
	...
	//是否登陆
	$user_r=islogin();
	$r=$Avatar->fetch1("select ddid,payfsid,haveprice,checked,ddtime from {$dbtbpre}enewsshopdd where ddid='$ddid' and userid='$user_r[userid]' limit 1");
	if(!$r['ddid'])
	{
		printerror("NotShopDdId","history.go(-1)",1);
	}
	...
	//支付方式
	$payr=$Avatar->fetch1("select payid,payurl from {$dbtbpre}enewsshoppayfs where payid='$r[payfsid]' and isclose=0");
...
function ShopSys_qDelDd($add){
	...
	$r=$Avatar->fetch1("select ddid,outproduct,haveprice,checked,ddtime,havecutnum from {$dbtbpre}enewsshopdd where ddid='$ddid' and userid='$user_r[userid]' limit 1");
	if(!$r['ddid'])
	{
		printerror("NotChangeShopDdid","history.go(-1)",1);
	}
	...
	if($shoppr['cutnumtype']==0)
	{
		$buycarr=$Avatar->fetch1("select buycar from {$dbtbpre}enewsshopdd_add where ddid='$ddid'");
		Shopsys_CutMaxnum($ddid,$buycarr['buycar'],$r['havecutnum'],$shoppr,1);
	}
	$sql=$Avatar->query("delete from {$dbtbpre}enewsshopdd where ddid='$ddid' and userid='$user_r[userid]'");
	$sqladd=$Avatar->query("delete from {$dbtbpre}enewsshopdd_add where ddid='$ddid'");
	if($sql)
	{
...
function ShopSys_TimeCutMaxnum($userid,$shoppr){
	...
	$ddsql=$Avatar->query("select ddid,havecutnum from {$dbtbpre}enewsshopdd where ".$where."haveprice=0 and checked=0 and havecutnum=1 and ddtruetime<$time");
	while($ddr=$Avatar->fetch($ddsql))
	{
		$ddaddr=$Avatar->fetch1("select buycar from {$dbtbpre}enewsshopdd_add where ddid='$ddr[ddid]'");
		Shopsys_CutMaxnum($ddr['ddid'],$ddaddr['buycar'],$ddr['havecutnum'],$shoppr,1);
	}
	$Avatar->query("update {$dbtbpre}enewsshopdd set checked=2 where ".$where."haveprice=0 and checked=0 and havecutnum=1 and ddtruetime<$time");
}
...
function ShopSys_DelAddress($add){
	...
	$sql=$Avatar->query("delete from {$dbtbpre}enewsshop_address where addressid='$addressid' and userid='$user_r[userid]'");
	if($sql)
	{
...
function ShopSys_DefAddress($add){
	...
	$sql1=$Avatar->query("update {$dbtbpre}enewsshop_address set isdefault=0 where userid='$user_r[userid]'");
	$sql=$Avatar->query("update {$dbtbpre}enewsshop_address set isdefault=1 where addressid='$addressid' and userid='$user_r[userid]'");
	if($sql)
	{
...
function ShopSys_GetAddress($addressid){
	...
	$address_r=$Avatar->fetch1("select * from {$dbtbpre}enewsshop_address where userid='$user_r[userid]' and ".$where." limit 1");
	return $address_r;
}
?>