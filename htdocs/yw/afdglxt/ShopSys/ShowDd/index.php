...
$user=islogin();
$ddid=(int)$_GET['ddid'];
$r=$Avatar->fetch1("select * from {$dbtbpre}enewsshopdd where ddid='$ddid' and userid='$user[userid]' limit 1");
if(empty($r['ddid']))
{
	printerror('此订单不存在','',1,0,1);
}
$addr=$Avatar->fetch1("select * from {$dbtbpre}enewsshopdd_add where ddid='$ddid' limit 1");
//需要发票
$fp="否";
if($r[fp])
{
...
{
	//是否网银支付
	$payfs_r=$Avatar->fetch1("select payurl from {$dbtbpre}enewsshoppayfs where payid='$r[payfsid]'");
	if($payfs_r['payurl'])
	{
...
//导入模板
require(ECMS_PATH.'afdglxt/template/ShopSys/ShowDd.php');
db_close();
$Avatar=null;
?>