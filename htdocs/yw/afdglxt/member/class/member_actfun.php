...
function QMReturnCheckThePass($ckuserid,$ckpass,$cktime,$authstr,$ecms=1){
	global $Avatar,$dbtbpre,$ecms_config,$public_r;
	$f=$ecms==2?'acttime':'getpasstime';
	$time=time();
	$pr=$Avatar->fetch1("select ".$f." from {$dbtbpre}enewspublic limit 1");
	$ar=explode('||',$authstr);
...
function SendGetPasswordEmail($add){
	...
	$ur=$Avatar->fetch1("select ".eReturnSelectMemberF('userid,username,email')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username' limit 1");
	$useremail=$ur['email'];
	...
	$url=eReturnDomainSiteUrl().'afdglxt/member/GetPassword/getpass.php?id='.$ur['userid'].'&cc='.$passr[checkpass].'&tt='.$passr['dotime'];
	//发送邮件
	$pr=$Avatar->fetch1("select getpasstext,getpasstitle from {$dbtbpre}enewspublic limit 1");
	@include(ECMS_PATH.'afdglxt/class/SendEmail.inc.php');
...
function CheckGetPassword($add,$ecms=1){
	...
	$ur=$Avatar->fetch1("select ".eReturnSelectMemberF('userid,username,checked,groupid')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$r[id]' limit 1");
	if(empty($ur['userid']))
	{
		printerror('GPErrorPass',$public_r['newsurl'],1);
	}
	$addur=$Avatar->fetch1("select authstr from {$dbtbpre}enewsmemberpub where userid='$r[id]' limit 1");
	if(!$addur['authstr'])
	{
		printerror('GPErrorPass',$public_r['newsurl'],1);
	}
...
function DoGetPassword($add){
	...
	$sql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('password')."='$password',".egetmf('salt')."='$salt' where ".egetmf('userid')."='$r[id]'");
	$usql=$Avatar->query("update {$dbtbpre}enewsmemberpub set authstr='' where userid='$r[id]'");
...
function SendActUserEmail($userid,$username,$email){
	...
	$url=eReturnDomainSiteUrl().'afdglxt/member/doaction.php?enews=DoActUser&id='.$userid.'&cc='.$passr[checkpass].'&tt='.$passr['dotime'];
	//发送邮件
	$pr=$Avatar->fetch1("select acttext,acttitle from {$dbtbpre}enewspublic limit 1");
	@include(ECMS_PATH.'afdglxt/class/SendEmail.inc.php');
...
function DoActUser($add){
	...
	if(!$r['checked'])
	{
		...
		if($checked)
		{
			$sql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('checked')."=1 where ".egetmf('userid')."='$r[id]'");
		}
	}
	$usql=$Avatar->query("update {$dbtbpre}enewsmemberpub set authstr='' where userid='$r[id]'");
...
function DoRegSend($add){
	...
	//密码
	$ur=$Avatar->fetch1("select ".eReturnSelectMemberF('userid,salt,password')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username' limit 1");
	if(!$ur['userid'])
	{
	...
	$addr=$Avatar->fetch1("select userid,authstr from {$dbtbpre}enewsmemberpub where userid='".$r['userid']."' limit 1");
