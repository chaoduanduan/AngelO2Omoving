...
function CheckMemberGroupCanReg($groupid){
	...
	$groupid=(int)$groupid;
	$r=$Avatar->fetch1("select groupid from {$dbtbpre}enewsmembergroup where groupid='$groupid' and canreg=1");
	if(empty($r['groupid']))
	{
...
function eCheckIpRegTime($ip,$time){
	...
	$uaddr=$Avatar->fetch1("select userid from {$dbtbpre}enewsmemberadd where regip='$ip' order by userid desc limit 1");
	if(empty($uaddr['userid']))
	{
		return '';
	}
	$ur=$Avatar->fetch1("select ".eReturnSelectMemberF('userid,registertime')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$uaddr[userid]' limit 1");
	if(empty($ur['userid']))
	{
		return '';
	}
...
function register($add){
	global $Avatar,$dbtbpre,$public_r,$ecms_config;
	//关闭注册
	...
	//用户字数
	$pr=$Avatar->fetch1("select min_userlen,max_userlen,min_passlen,max_passlen,regretime,regclosewords,regemailonly from {$dbtbpre}enewspublic limit 1");
	...
	//重复用户
	$num=$Avatar->gettotal("select count(*) as total from ".eReturnMemberTable()." where ".egetmf('username')."='$username' limit 1");
	if($num)
	{
	...
	if($pr['regemailonly'])
	{
		$num=$Avatar->gettotal("select count(*) as total from ".eReturnMemberTable()." where ".egetmf('email')."='$email' limit 1");
		if($num)
		{
	...
	$sql=$Avatar->query("insert into ".eReturnMemberTable()."(".eReturnInsertMemberF('username,password,rnd,email,registertime,groupid,userfen,userdate,money,zgroupid,havemsg,checked,salt,userkey').") values('$username','$password','$rnd','$email','$registertime','$groupid','$public_r[reggetfen]','0','0','0','0','$checked','$salt','$userkey');");
	//取得userid
	$userid=$Avatar->lastid();
	//附加表
	$addr=$Avatar->fetch1("select * from {$dbtbpre}enewsmemberadd where userid='$userid'");
	if(!$addr[userid])
	{
		$spacestyleid=ReturnGroupSpaceStyleid($groupid);
		$sql1=$Avatar->query("insert into {$dbtbpre}enewsmemberadd(userid,spacestyleid,regip,lasttime,lastip,loginnum,regipport,lastipport".$member_r[0].") values('$userid','$spacestyleid','$regip','$lasttime','$regip','1','$regipport','$regipport'".$member_r[1].");");
	}
	//更新附件
	...
	if($sql)
	{
		...
		$r=$Avatar->fetch1("select ".eReturnSelectMemberF('*')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' limit 1");
		...
		if($returnurl&&!strstr($returnurl,"afdglxt/member/iframe")&&!strstr($returnurl,"afdglxt/member/register")&&!strstr($returnurl,"enews=exit"))
		{
			$location=$returnurl;
		}
