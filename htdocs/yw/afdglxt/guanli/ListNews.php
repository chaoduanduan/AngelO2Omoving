...
//栏目
$thiscr=$Avatar->fetch1("select tbname,listorder,allinfos,infos from {$dbtbpre}enewsclass where classid='$classid'");
if(!$thiscr['tbname'])
{
	printerror('ErrorUrl','history.go(-1)');
}
...
$infomod_r=$Avatar->fetch1("select enter,tbname,sonclass,listfile from {$dbtbpre}enewsmod where mid=".$modid);
...
if($_GET['showretitle']==1)
{
	...
	$srtid="";
	$first=1;
	$srtsql=$Avatar->query("select id,title from ".$infotb." where classid='$classid' group by title having(count(*))>1");
...
{
	$singletablenum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsclass where tbname='".$class_r[$classid][tbname]."' and islast=1");
...
//标题分类
$tts='';
$ttsql=$Avatar->query("select typeid,tname from {$dbtbpre}enewsinfotype where mid='$modid' order by myorder");
...
//导入页面
$deftempfile=ECMS_PATH.'afdglxt/data/html/list/listinfo.php';
if($infomod_r[listfile])
{
	$tempfile=ECMS_PATH.'afdglxt/data/html/list/'.$infomod_r[listfile].'.php';
