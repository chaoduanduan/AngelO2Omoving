...
$changetbs='';
$havetb=0;
$tbsql=$Avatar->query("select tid,tbname,tname from {$dbtbpre}enewstable order by tid");
...
//取得相应的信息
$user_r=$Avatar->fetch1("select groupid,adminclass from {$dbtbpre}enewsuser where userid='$logininid'");
//取得用户组
$gr=$Avatar->fetch1("select doall,doselfinfo from {$dbtbpre}enewsgroup where groupid='$user_r[groupid]'");
//管理员
$where='';
$and='';
$ewhere='';
...
//模型
$infomod_r=$Avatar->fetch1("select mid,listfile from {$dbtbpre}enewsmod where mid='$modid'");
//标题分类
$ttid=(int)$_GET['ttid'];
if($ttid)
{
...
//标题分类
$tts='';
$ttsql=$Avatar->query("select typeid,tname from {$dbtbpre}enewsinfotype where mid='$infomod_r[mid]' order by myorder");
...
if($_GET['showretitle']==1)
{
	...
	$srtid="";
	$first=1;
	$srtsql=$Avatar->query("select id,title from ".$infotb." group by title having(count(*))>1");
	while($srtr=$Avatar->fetch($srtsql))
	{
		if($first==1)
		{
...
//导入页面
$deftempfile=ECMS_PATH.'afdglxt/data/html/list/alllistinfo.php';
if($infomod_r[listfile])
{
	$tempfile=ECMS_PATH.'afdglxt/data/html/list/all'.$infomod_r[listfile].'.php';
