...
function DoCropImage($add,$userid,$username){
	global $Avatar,$dbtbpre,$public_r,$class_r,$ecms_config,$efileftp_fr,$efileftp_dr;
	//参数处理
	$pic_x=(int)$add['pic_x'];
	$pic_y=(int)$add['pic_y'];
	$pic_w=(int)$add['pic_w'];
	...
	$filer=$Avatar->fetch1("select fileid,path,filename,classid,fpath,no from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid'");
	...
	if(file_exists($insert_file))
	{
		if(!$doing)
		{
			$Avatar->query("delete from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid'");
