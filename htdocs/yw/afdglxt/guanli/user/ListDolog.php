...
function DelDoLog($logid,$userid,$username){
	...
	$sql=$Avatar->query("delete from {$dbtbpre}enewsdolog where logid='$logid'");
	if($sql)
	{
		//操作日志
...
function DelDoLog_all($logid,$userid,$username){
	...
	$sql=$Avatar->query("delete from {$dbtbpre}enewsdolog where".$add);
	if($sql)
	{
		//操作日志
		insert_dolog("");
...
function DelDoLog_date($add,$userid,$username){
	...
	$endday=$end.' 23:59:59';
	$sql=$Avatar->query("delete from {$dbtbpre}enewsdolog where logtime<='$endday' and logtime>='$startday'");
	if($sql)
	{
		//操作日志
  ...
  ?>
    <tr bgcolor="#FFFFFF"> 
      <td height="25" colspan="5"> 
        <?=$returnpage?>&nbsp;&nbsp;
        <input type="submit" name="Submit" value="批量删除"> <input name="enews" type="hidden" id="caity" value="DelDoLog_all">&nbsp;
