...
function Ebak_Rep($tablename,$dbname,$userid,$username){
	global $Avatar,$ecms_config;
	//验证权限
	CheckLevel($userid,$username,$classid,"dbdata");
	$dbname=RepPostVar($dbname);
	$Avatar->usequery("use `$dbname`");
	$count=count($tablename);
	if(empty($count))
	{printerror("MustChangeOneTable","history.go(-1)");}
	for($i=0;$i<$count;$i++)
	{
		//$sql1=$Avatar->query("OPTIMIZE TABLE `$tablename[$i]`;");
		//$sql2=$Avatar->query("CHECK TABLE `$tablename[$i]`;");
		//$sql3=$Avatar->query("ANALYZE TABLE `$tablename[$i]`;");
		$sql4=$Avatar->query("REPAIR TABLE `$tablename[$i]`;");
    }
	$Avatar->usequery("use `".$ecms_config['db']['dbname']."`");
	//操作日志
...
function Ebak_Drop($tablename,$dbname,$userid,$username){
	...
	$sql1=$Avatar->query("DROP TABLE IF EXISTS ".$a.";");
	$Avatar->usequery("use `".$ecms_config['db']['dbname']."`");
	//操作日志
	insert_dolog("dbname=".$dbname);
...
function Ebak_DropDb($dbname,$userid,$username){
	...
	$sql=$Avatar->query("DROP DATABASE `$dbname`");
	if($sql)
	{
		//操作日志
		insert_dolog("dbname=".$dbname);
...
function Ebak_CreatDb($dbname,$dbchar,$userid,$username){
	...
	$sql=$Avatar->query("CREATE DATABASE IF NOT EXISTS `$dbname`".$a);
	if($sql){
		//操作日志
		insert_dolog("dbname=".$dbname);
...
function Ebak_EmptyTable($tablename,$dbname,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		$sql1=$Avatar->query("TRUNCATE `".$tablename[$i]."`;");
    }
	$Avatar->usequery("use `".$ecms_config['db']['dbname']."`");
	//操作日志
	insert_dolog("dbname=".$dbname);
...
function Ebak_DoEbak($add,$userid,$username){
	global $Avatar,$public_r,$fun_r,$ecms_config;
	//验证权限
	CheckLevel($userid,$username,$classid,"dbdata");
	...
	echo $fun_r['FirstBakSuccess']."<script>self.location.href='caity.php?caity=$caity&t=0&s=0&p=0&mypath=$add[mypath]&waitbaktime=$waitbaktime".hReturnEcmsHashStrHref(0)."';</script>";
	exit();
}

//执行备份(按文件大小)
function Ebak_BakExe($t,$s,$p,$mypath,$alltotal,$thenof,$fnum,$stime=0,$userid,$username){
	global $Avatar,$public_r,$fun_r;
	//验证权限
	CheckLevel($userid,$username,$classid,"dbdata");
	if(empty($mypath)){
...
?>";
	...
	//选择数据库
	$u=$Avatar->usequery("use `$b_dbname`");
	//编码
	if($b_dbchar=='auto')
	{
		if(empty($s))
	...
	$sql=$Avatar->query("select * from `".$btb[$t]."` limit $s,$num");
	//取得字段数
	if(empty($fnum))
	{
	...
	while($r=$Avatar->fetch($sql))
	{
		...
		if(strlen($dumpsql)>=$b_filesize*1024)
		{
			...
			$Avatar->free($sql);
			//echo $fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s)."<script>self.location.href='caity.php?caity=BakExe&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&stime=$stime".hReturnEcmsHashStrHref(0)."';</script>";

			echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=caity.php?caity=BakExe&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&stime=$stime&waitbaktime=$waitbaktime&collation=$collation".hReturnEcmsHashStrHref(0)."\">".$fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s);
			exit();
		}
	}
	//最后一个备份
	...
	$Avatar->free($sql);
	//进入下一个表
	//echo $btb[$t-1].$fun_r['OneTableBakSuccess']."<script>self.location.href='caity.php?caity=BakExe&s=0&p=0&t=$t&mypath=$mypath&stime=$stime".hReturnEcmsHashStrHref(0)."';</script>";

	echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=caity.php?caity=BakExe&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime".hReturnEcmsHashStrHref(0)."\">".$btb[$t-1].$fun_r['OneTableBakSuccess'];
	exit();
}
...
?>";
	...
	if($b_autoauf==1&&$auf)
	{
		$sql=$Avatar->query("select * from `".$btb[$t]."` where ".$auf.">".$aufval." order by ".$auf." limit $b_bakline");
	}
	else
	{
		$sql=$Avatar->query("select * from `".$btb[$t]."` limit $s,$b_bakline");
	}
	//完整插入
	$inf='';
	if($b_beover==1)
	...
	if(empty($b))
	{
		...
		$Avatar->free($sql);
		//进入下一个表
		//echo $btb[$t-1].$fun_r['OneTableBakSuccess']."<script>self.location.href='caity.php?caity=BakExeT&s=0&p=0&t=$t&mypath=$mypath&stime=$stime".hReturnEcmsHashStrHref(0)."';</script>";

		echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=caity.php?caity=BakExeT&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime".hReturnEcmsHashStrHref(0)."\">".$btb[$t-1].$fun_r['OneTableBakSuccess'];
		exit();
	}
	//进入下一组
	$p++;
	...
	WriteFiletext_n($sfile,$dumpsql);
	$Avatar->free($sql);
	//echo $fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s)."<script>self.location.href='caity.php?caity=BakExeT&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&auf=$auf&aufval=$lastaufval&stime=$stime".hReturnEcmsHashStrHref(0)."';</script>";

	echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=caity.php?caity=BakExeT&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&auf=$auf&aufval=$lastaufval&stime=$stime&waitbaktime=$waitbaktime&collation=$collation".hReturnEcmsHashStrHref(0)."\">".$fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s);
	exit();
}
...
function Ebak_GetTotal($dbname,$tbname){
	global $Avatar;
	/*
	$tr=$Avatar->fetch1("select count(*) AS total from ".$btb[$t]);
	$num=$tr[total];
	*/
	$tr=$Avatar->fetch1("SHOW TABLE STATUS LIKE '".$tbname."';");
	return $tr;
}
...
function Ebak_GetSetChar($char){
	...
	$r=$Avatar->fetch1("SHOW COLLATION LIKE '".$char."';");
	return $r['Charset'];
}
...
function Ebak_ReturnTbfield($dbname,$tbname,$autofield){
	global $Avatar;
	$sql=$Avatar->query("SHOW FIELDS FROM `".$tbname."`");
	$i=0;//字段数
	$autof=",";//去除自增字段列表
	$f='';//自增字段名
	while($r=$Avatar->fetch($sql))
...
function E_C($sql){
	global $Avatar;
	$Avatar->query(Ebak_AddDbchar($sql));
}
...
function Ebak_Returnstru($table,$strufour){
	...
	//设置引号
	$usql=$Avatar->query("SET SQL_QUOTE_SHOW_CREATE=1;");
	//数据表结构
	$r=$Avatar->fetch1("SHOW CREATE TABLE `$table`;");
...
/*
		SoftName : AvatarBak
		Author   : wm_chief
		Copyright: Powered by www.caity.net
*/
