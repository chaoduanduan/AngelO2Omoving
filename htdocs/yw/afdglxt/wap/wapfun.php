...
function DoWapShowMsg($error,$returnurl='index.php',$ecms=0){
	...
	{
		@include(ECMS_PATH.'afdglxt/wap/message.php');
	}
	db_close();
	$Avatar=null;
	exit();
...
<card id="Avatarcms_wml" title="<?php echo $title;?>">
<?php
}
...
?>
<p><br/><small>Powered by AvatarCMS</small></p>
</card></wml>
<?php
	$str=ob_get_contents();
	ob_end_clean();
...
function ReturnWapStyle($add,$style){
	global $Avatar,$dbtbpre,$pr,$class_r;
	$style=(int)$style;
	...
	{
		$cr=$Avatar->fetch1("select wapstyleid from {$dbtbpre}enewsclass where classid='$classid'");
		if($cr['wapstyleid'])
		{
	...
	$sr=$Avatar->fetch1("select path from {$dbtbpre}enewswapstyle where styleid='$styleid'");
	$wapstyle=$sr['path'];
	if(empty($wapstyle))
	{
		$wapstyle=1;
...
$pr=$Avatar->fetch1("select wapopen,wapdefstyle,wapshowmid,waplistnum,wapsubtitle,wapshowdate,wapchar from {$dbtbpre}enewspublic limit 1");
