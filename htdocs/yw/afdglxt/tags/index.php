...
if($tagid)
{
	$tagr=$Avatar->fetch1("select tagname,num from {$dbtbpre}enewstags where tagid='$tagid'");
	if(!$tagr['tagname'])
	{
...
else
{
	...
	$tagr=$Avatar->fetch1("select tagid,num from {$dbtbpre}enewstags where tagname='$tagname' limit 1");
	if(!$tagr['tagid'])
	{
		printerror('HaveNotTags','',1);
	}
...
$pagekey=$tagname;
$pagedes=$tagname;
$classimg=$public_r[newsurl].'afdglxt/data/images/notimg.gif';
...
$tempr=$Avatar->fetch1("select tempid,temptext,subnews,listvar,rownum,showdate,modid,subtitle,docode from ".GetTemptb("enewslisttemp")." where tempid='$tempid'");
