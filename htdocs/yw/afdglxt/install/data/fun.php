...
function InstallReturnDbChar(){
	if(AvatarCMS_CHARVER=='UTF-8')//简体UTF-8
	{
		$ret_r['dbchar']='utf8';
		$ret_r['setchar']='utf8';
		$ret_r['headerchar']='utf-8';
	}
	elseif(AvatarCMS_CHARVER=='BIG5')//繁体BIG5
	{
		$ret_r['dbchar']='big5';
		$ret_r['setchar']='big5';
		$ret_r['headerchar']='big5';
	}
	elseif(AvatarCMS_CHARVER=='TC-UTF-8')//繁体UTF-8
	{
		$ret_r['dbchar']='utf8';
		$ret_r['setchar']='utf8';
		$ret_r['headerchar']='utf-8';
...
function GetZend(){
	...
	@ob_end_clean();
	if($string=="www.caity.net"||strstr($string,"bytes in"))
	{
		$r['word']="支持";
		$r['result']=ReturnResult(1);
	}
...
function InstallSuccessShowInfo(){
	...
	$getver=base64_encode("Avatarcms,".AvatarCMS_VERSION.",".AvatarCMS_CHARVER.",".AvatarCMS_LASTTIME.",".$_SERVER['HTTP_HOST'].",".$_SERVER['REMOTE_ADDR'].",".urlencode($_SERVER['PHP_SELF'])."");
	$dourl="http://ecmsuser.caity.net/Avatarcmsupdate/?ecms=AvatarCMSUpdate&time=$time&key=$key&ver=$getver";
...
function DoRunQuery($sql,$mydbchar,$mydbtbpre,$mydbver){
	$sql=str_replace("\r","\n",str_replace(' `caity_',' `'.$mydbtbpre,$sql));
	$ret=array();
	$num=0;
	foreach(explode(";\n",trim($sql)) as $query)
	{
...
//password
function ins_DoAvatarCMSAdminPassword($password,$salt,$salt2){
...
function FirstAdmin($add){
	...
	$password=ins_DoAvatarCMSAdminPassword($add['password'],$salt,$salt2);
...
//链接数据库
function InstallConnectDb($caity_use_dbver,$caity_db_server,$caity_db_port,$caity_db_username,$caity_db_password,$caity_db_dbname,$caity_db_char,$caity_db_dbchar){
	$dblocalhost=$caity_db_server;
	//端口
	if($caity_db_port)
	{
		$dblocalhost.=":".$caity_db_port;
	}
	$link=@mysql_connect($dblocalhost,$caity_db_username,$caity_db_password);
	if(!$link)
	{
		InstallShowMsg('您的数据库用户名或密码有误，链接不上MYSQL数据库');
	}
	//mysql版本
	if($caity_use_dbver=='auto')
	{
		$caity_use_dbver=GetMysqlVerForDb();
		if(!$caity_use_dbver)
		{
	...
	//编码
	if($caity_use_dbver>='4.1')
	{
		$q='';
		if($caity_db_char)
		{
			$q='character_set_connection='.$caity_db_char.',character_set_results='.$caity_db_char.',character_set_client=binary';
		}
		if($caity_use_dbver>='5.0')
		{
			$q.=(empty($q)?'':',').'sql_mode=\'\'';
		}
		if($q)
	...
	$db=@mysql_select_db($caity_db_dbname);
	//数据库不存在
	if(!$db)
	{
		if($caity_use_dbver>='4.1')
		{
			$createdb=@mysql_query("CREATE DATABASE IF NOT EXISTS ".$caity_db_dbname." DEFAULT CHARACTER SET ".$caity_db_dbchar);
		}
		else
		{
			$createdb=@mysql_query("CREATE DATABASE IF NOT EXISTS ".$caity_db_dbname);
		}
		if(!$createdb)
		{
			InstallShowMsg('您输入的数据库名不存在');
		}
		@mysql_select_db($caity_db_dbname);
	}
	return $caity_use_dbver;
}
//配置数据库
function SetDb($add){
	global $version;
	...
	@mysql_query("update ".$add['mydbtbpre']."enewspl_set set plurl='".$siteurl."afdglxt/pl/' limit 1");
	@mysql_query("update ".$add['mydbtbpre']."enewsshoppayfs set payurl='".$siteurl."afdglxt/payapi/ShopPay.php?paytype=alipay' where payid=3");
	@mysql_close();
...
function RepEcmsConfig($add,$siteurl){
	...
	if(!$fp)
	{
		InstallShowMsg('请检查 /afdglxt/install/data/config.php 文件是否存在!');
	}
	$data=@fread($fp,filesize("data/config.php"));
	fclose($fp);
	$data=str_replace('<!--dbtype.caity.net-->',$add['mydbtype'],$data);
	$data=str_replace('<!--dbver.caity.net-->',$add['mydbver'],$data);
	$data=str_replace('<!--host.caity.net-->',$add['mydbhost'],$data);
	$data=str_replace('<!--port.caity.net-->',$add['mydbport'],$data);
	$data=str_replace('<!--username.caity.net-->',$add['mydbusername'],$data);
	$data=str_replace('<!--password.caity.net-->',$add['mydbpassword'],$data);
	$data=str_replace('<!--name.caity.net-->',$add['mydbname'],$data);
	$data=str_replace('<!--char.caity.net-->',$add['mysetchar'],$data);
	$data=str_replace('<!--dbchar.caity.net-->',$add['mydbchar'],$data);
	$data=str_replace('<!--tbpre.caity.net-->',$add['mydbtbpre'],$data);
	$data=str_replace('<!--cookiepre.caity.net-->',$add['mycookievarpre'],$data);
	$data=str_replace('<!--admincookiepre.caity.net-->',$add['myadmincookievarpre'],$data);
	$data=str_replace('<!--headerchar.caity.net-->',$headerchar,$data);
	$data=str_replace('<!--cookiernd.caity.net-->',ins_make_password(30),$data);
	$data=str_replace('<!--qcookiernd.caity.net-->',ins_make_password(30),$data);
	$data=str_replace('<!--qcookierndtwo.caity.net-->',ins_make_password(30),$data);
	$data=str_replace('<!--ecms.newsurl-->',$siteurl,$data);
	$data=str_replace('<!--ecms.fileurl-->',$siteurl."d/file/",$data);
	$data=str_replace('<!--ecms.plurl-->',$siteurl."afdglxt/pl/",$data);
	...
	if(!$fp1)
	{
		InstallShowMsg(' /afdglxt/config/config.php 文件权限没有设为0777，配置数据库不成功');
	}
	@fputs($fp1,$data);
	@fclose($fp1);
}
...
function RepEcmsConfigLoginauth($add){
	...
	if(!$fp)
	{
		InstallShowMsg('请检查 /afdglxt/config/config.php 文件是否存在!');
	}
	$data=@fread($fp,filesize("../config/config.php"));
	fclose($fp);
	$data=str_replace('<!--loginauth.caity.net-->',$add['loginauth'],$data);
	//写入配置文件
	$fp1=@fopen("../config/config.php","w");
	if(!$fp1)
	{
		InstallShowMsg(' /afdglxt/config/config.php 文件权限没有设为0777，配置不成功');
	}
	@fputs($fp1,$data);
	@fclose($fp1);
}
...
function ReturnInstallSql($defaultdata=1){
	if($defaultdata==0)
	{
		$sqlfile="data/Avatarcms.com.sql";
	}
	elseif($defaultdata==2)
	{
		$sqlfile="data/Avatarcms.temp.sql";
	}
	elseif($defaultdata==3)
	{
		$sqlfile="data/Avatarcms.mod.sql";
	}
	else
	{
		$sqlfile="data/Avatarcms.data.sql";
	}
	$fp=fopen($sqlfile,'r');
	...
	if(empty($sql))
	{
		InstallShowMsg(' /afdglxt/install/'.$sqlfile.' 文件丢失,安装不成功','index.php?enews=setdb&f=4');
	}
	//替换测试数据网址
	if($sqlfile=='data/Avatarcms.data.sql')
	{
...
function ReturnEcmsSiteUrl(){
	$siteurl=str_replace('afdglxt/install/index.php','',$_SERVER['PHP_SELF']);
	$siteurl=str_replace('afdglxt/install/','',$siteurl);
	$siteurl=str_replace('afdglxt/install','',$siteurl);
	return $siteurl;
}
//删除存文本文件
...
function InstallReplaceTestDataUrl($text){
	...
	$text=str_replace('http://demo.caity.net/defdata/demopic/',$baseurl.'testdata/demopic/',$text);
	return $text;
}
?>