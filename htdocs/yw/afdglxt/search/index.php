...
{
	$tbnamenum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewstable where tbname='$s_tbname' limit 1");
	if(!$tbnamenum)
	{
		printerror("SearchNotRecord",$getfrom,1);
	}
	$tbname=$s_tbname;
	//模型id
	$thestemp_r=$Avatar->fetch1("select modid from ".GetTemptb("enewssearchtemp")." where tempid='$s_tempid'");
...
if(empty($class_r[$trueclassid][searchtempid]))
{
	if(empty($modid))
	{
		$tempr=$Avatar->fetch1("select modid from ".GetTemptb("enewssearchtemp")." where isdefault=1 limit 1");
	}
	else
	{
		$tempr[modid]=$modid;
...
$mr=$Avatar->fetch1("select searchvar,tbname from {$dbtbpre}enewsmod where mid='$tempr[modid]'");
...
$search_r=$Avatar->fetch1("select searchid from {$dbtbpre}enewssearch where checkpass='$checkpass' limit 1");
...
if($searchid)
{
    $search_num=$Avatar->gettotal($query);
	$sql=$Avatar->query("update {$dbtbpre}enewssearch set searchtime='$searchtime',result_num='$search_num',onclick=onclick+1,orderby='$orderby',myorder='$myorder',tempid='$s_tempid' where searchid='$searchid'");
	if(empty($search_num))
	{
		$searchid=0;
	}
