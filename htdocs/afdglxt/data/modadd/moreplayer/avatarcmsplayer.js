...
//show
function AvatarCMSPlayVideo(vtype,furl,width,height,autostart,baseurl){
	var filetype;
	var cfiletype;
	var playstr='';
	...
	if(vtype=='auto'||vtype=='')
	{
		filetype=AvatarCMSPlayerGetFiletype(furl);
		cfiletype='|'+filetype+'|';
		if(filetypejwplayer.indexOf(cfiletype)!=-1)//jwplayer
		{
			playstr=AvatarCMSShowJwplayer(furl,width,height,autostart,baseurl);
		}
		else if(filetypeflash.indexOf(cfiletype)!=-1)//flash
		{
			playstr=AvatarCMSShowFlash(furl,width,height,autostart,baseurl);
		}
		else if(filetyperealplayer.indexOf(cfiletype)!=-1)//realplayer
		{
			playstr=AvatarCMSShowRealPlayer(furl,width,height,autostart,baseurl);
		}
		else//mediaplayer
		{
			playstr=AvatarCMSShowMediaPlayer(furl,width,height,autostart,baseurl);
		}
	}
	else
	{
...
//filetype
function AvatarCMSPlayerGetFiletype(sfile){
	var filetype,s;
	s=sfile.lastIndexOf(".");
...
//flash
function AvatarCMSShowFlash(furl,width,height,autostart,baseurl){
	var str='';
...
//realplayer
function AvatarCMSShowRealPlayer(furl,width,height,autostart,baseurl){
	var str='';
...
//jwplayer
function AvatarCMSShowJwplayer(furl,width,height,autostart,baseurl){
	var str='';
	str='<scri'+'pt type="text/javascript" src="'+baseurl+'afdglxt/data/modadd/moreplayer/jwplayer/jwplayer.js"></scri'+'pt><div id="AvatarCMSmyElement">Loading the player...</div><scri'+'pt type="text/javascript">jwplayer("AvatarCMSmyElement").setup({ autostart:'+autostart+',file: "'+furl+'",height:'+height+',width:'+width+' });</scri'+'pt>';
	return str;
}
