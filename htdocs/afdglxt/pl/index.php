...
if($doaction=='dozt')
{
	...
	$n_r=$Avatar->fetch1("select ztid,ztname,intro,ztimg,ztpagekey,restb from {$dbtbpre}enewszt where ztid='$classid'");
	if(!$n_r['ztid'])
	{
		printerror("ErrorUrl","history.go(-1)",1);
	}
...
else
{
	...
	$n_r=$Avatar->fetch1("select * from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where id='$id' limit 1");
...
if($_GET['tempid'])
{
	...
	$tempnum=$Avatar->gettotal("select count(*) as total from ".GetTemptb("enewspltemp")." where tempid='$tempid'");
...
@require(ECMS_PATH.'afdglxt/data/filecache/template/pl'.$tempid.'.php');
db_close();
$Avatar=null;
?>