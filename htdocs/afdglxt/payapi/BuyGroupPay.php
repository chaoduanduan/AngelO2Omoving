...
$payr=$Avatar->fetch1("select * from {$dbtbpre}enewspayapi where payid='$payid' and isclose=0 limit 1");
if(!$payr[payid])
{
	printerror('请选择支付平台','',1,0,1);
}
$buyr=$Avatar->fetch1("select * from {$dbtbpre}enewsbuygroup where id='$id'");
if(!$buyr['id'])
{
	printerror('请选择充值类型','',1,0,1);
}
...
esetcookie("paycaity","BuyGroupPay",0);
