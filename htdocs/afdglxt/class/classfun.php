...
function ChangeZtForm(){
	...
	$mtemp='';
	$sql=$Avatar->query("select fname,f,fhtml from {$dbtbpre}enewsztf order by myorder,fid");
...
function AddZtF($add,$userid,$username){
	...
	//字段是否重复
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}enewsztadd");
	$b=0;
	...
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}enewszt");
	$b=0;
	while($r=$Avatar->fetch($s))
	{
		if($r[Field]==$add[f])
	...
	//新增字段
	$asql=$Avatar->query("alter table {$dbtbpre}enewsztadd add ".$field);
	//替换代码
	...
	$sql=$Avatar->query("insert into {$dbtbpre}enewsztf(f,fname,fform,fhtml,fzs,myorder,ftype,flen,fvalue,fformsize) values('$add[f]','$add[fname]','$add[fform]','".eaddslashes2($fhtml)."','".eaddslashes($add[fzs])."',$add[myorder],'$add[ftype]','$add[flen]','".eaddslashes2($add[fvalue])."','$add[fformsize]');");
...
function EditZtF($add,$userid,$username){
	...
	$field="`".$add[f]."` ".$type." NOT NULL".$def;
	$usql=$Avatar->query("alter table {$dbtbpre}enewsztadd change `".$add[oldf]."` ".$field);
	//替换代码
	...
	//处理变量
	$add[myorder]=(int)$add[myorder];
	$sql=$Avatar->query("update {$dbtbpre}enewsztf set f='$add[f]',fname='$add[fname]',fform='$add[fform]',fhtml='".eaddslashes2($fhtml)."',fzs='".eaddslashes($add[fzs])."',myorder=$add[myorder],ftype='$add[ftype]',flen='$add[flen]',fvalue='".eaddslashes2($add[fvalue])."',fformsize='$add[fformsize]' where fid=$fid");
	//更新表单
	ChangeZtForm();
	if($usql&&$sql)
	{
...
function DelZtF($add,$userid,$username){
	...
	$r=$Avatar->fetch1("select f from {$dbtbpre}enewsztf where fid='$fid'");
	if(!$r[f]){
		printerror("EmptyFid","history.go(-1)");
	}
	$usql=$Avatar->query("alter table {$dbtbpre}enewsztadd drop COLUMN `".$r[f]."`");
	$sql=$Avatar->query("delete from {$dbtbpre}enewsztf where fid='$fid'");
	//更新表单表
	ChangeZtForm();
	if($usql&&$sql)
	{
...
function EditZtFOrder($fid,$myorder,$userid,$username){
	...
	for($i=0;$i<count($myorder);$i++)
	{
		...
		$usql=$Avatar->query("update {$dbtbpre}enewsztf set myorder=$newmyorder where fid='$fid[$i]'");
    }
	//更新表单表
	ChangeZtForm();
...
function ReturnZtAddF($add,$ecms=0){
	...
	$ret_r[0]='';
	$ret_r[1]='';
	$fsql=$Avatar->query("select f from {$dbtbpre}enewsztf");
	if($ecms==0)//增加
	{
...
function AddZt($add,$userid,$username){
	global $Avatar,$class_r,$dbtbpre,$public_r;
...
function EditZt($add,$userid,$username){
	global $Avatar,$class_r,$dbtbpre,$loginlevel;
...
function DelZt($ztid,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"zt");
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewszt where ztid='$ztid'");
	if(empty($r[ztid])){
		printerror("NotDelZtid","history.go(-1)");
	}
	//删除专题
	$sql=$Avatar->query("delete from {$dbtbpre}enewszt where ztid='$ztid'");
	$Avatar->query("delete from {$dbtbpre}enewsztadd where ztid='$ztid'");
	$delpath="../../".$r[ztpath];
	$del=DelPath($delpath);
	//删除专题子类
	$zttypesql=$Avatar->query("select cid from {$dbtbpre}enewszttype where ztid='$ztid'");
	while($zttyper=$Avatar->fetch($zttypesql))
	{
		$Avatar->query("delete from {$dbtbpre}enewszttypeadd where cid='$zttyper[cid]'");
	}
	$Avatar->query("delete from {$dbtbpre}enewszttype where ztid='$ztid'");
	$Avatar->query("delete from {$dbtbpre}enewsztinfo where ztid='$ztid'");
	//删除附件
...
function TogZt($add,$userid,$username){
	...
	$r=$Avatar->fetch1("select ztid,ztname from {$dbtbpre}enewszt where ztid='$ztid'");
	if(empty($r['ztid']))
	{
		printerror("ErrorUrl","history.go(-1)");
	}
	...
	$tbr=$Avatar->fetch1("select tid from {$dbtbpre}enewstable where tbname='$tbname' limit 1");
	if(!$tbr['tid'])
	{
		printerror('EmptyTogZt','history.go(-1)');
	}
...
function SaveTogZtInfo($add,$userid,$username){
	...
	$r=$Avatar->fetch1("select togid from {$dbtbpre}enewstogzts where togztname='$add[togztname]'");
	if($r[togid])
	{
		$sql=$Avatar->query("update {$dbtbpre}enewstogzts set keyboard='".eaddslashes($add[keyboard])."',searchf='$searchf',query='".eaddslashes($add[query])."',specialsearch='$specialsearch',classid=$add[classid],retype=$add[retype],startday='".eaddslashes($add[startday])."',endday='".eaddslashes($add[endday])."',startid=$add[startid],endid=$add[endid],pline=$add[pline],doecmszt=$add[doecmszt] where togid='$r[togid]'");
		$togid=$r[togid];
	}
	else
	{
		$sql=$Avatar->query("insert into {$dbtbpre}enewstogzts(keyboard,searchf,query,specialsearch,classid,retype,startday,endday,startid,endid,pline,doecmszt,togztname) values('".eaddslashes($add[keyboard])."','$searchf','".eaddslashes($add[query])."','$specialsearch',$add[classid],$add[retype],'".eaddslashes($add[startday])."','".eaddslashes($add[endday])."',$add[startid],$add[endid],$add[pline],$add[doecmszt],'".eaddslashes($add[togztname])."');");
		$togid=$Avatar->lastid();
	}
	if($sql)
	{
...
function DelTogZtInfo($add,$userid,$username){
	...
	$r=$Avatar->fetch1("select togid,togztname from {$dbtbpre}enewstogzts where togid='$togid'");
	if(!$r[togid])
	{
		printerror('EmptyDelTogztid','history.go(-1)');
	}
	$sql=$Avatar->query("delete from {$dbtbpre}enewstogzts where togid='$togid'");
	if($sql)
	{
...
function ChangeClassForm(){
	...
	$mtemp='';
	$sql=$Avatar->query("select fname,f,fhtml from {$dbtbpre}enewsclassf order by myorder,fid");
	while($r=$Avatar->fetch($sql))
	{
		$mtemp.="<tr bgcolor='#FFFFFF' height=25><td>".$r['fname']."</td><td>".$r['fhtml']."</td></tr>";
    }
...
function AddClassF($add,$userid,$username){
	...
	//字段是否重复
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}enewsclassadd");
	$b=0;
	while($r=$Avatar->fetch($s))
	{
		if($r[Field]==$add[f])
	...
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}enewsclass");
	$b=0;
	while($r=$Avatar->fetch($s))
	{
		if($r[Field]==$add[f])
	...
	$field="`".$add[f]."` ".$type." NOT NULL".$def;
	//新增字段
	$asql=$Avatar->query("alter table {$dbtbpre}enewsclassadd add ".$field);
	//替换代码
	...
	//处理变量
	$add[myorder]=(int)$add[myorder];
	$sql=$Avatar->query("insert into {$dbtbpre}enewsclassf(f,fname,fform,fhtml,fzs,myorder,ftype,flen,fvalue,fformsize) values('$add[f]','$add[fname]','$add[fform]','".eaddslashes2($fhtml)."','".eaddslashes($add[fzs])."',$add[myorder],'$add[ftype]','$add[flen]','".eaddslashes2($add[fvalue])."','$add[fformsize]');");
	$lastid=$Avatar->lastid();
	//更新表单
	ChangeClassForm();
	if($asql&&$sql)
...
function EditClassF($add,$userid,$username){
	...
	$field="`".$add[f]."` ".$type." NOT NULL".$def;
	$usql=$Avatar->query("alter table {$dbtbpre}enewsclassadd change `".$add[oldf]."` ".$field);
	//替换代码
	if($add[f]<>$add[oldf]||$add[fform]<>$add[oldfform]||$add[fvalue]<>$add[oldfvalue]||$add[fformsize]<>$add[oldfformsize]){
		$fhtml=GetClassFform($add[fform],$add[f],$add[fvalue],$add[fformsize]);
		if($add[fform]=='select'||$add[fform]=='radio'||$add[fform]=='checkbox')
	...
	//处理变量
	$add[myorder]=(int)$add[myorder];
	$sql=$Avatar->query("update {$dbtbpre}enewsclassf set f='$add[f]',fname='$add[fname]',fform='$add[fform]',fhtml='".eaddslashes2($fhtml)."',fzs='".eaddslashes($add[fzs])."',myorder=$add[myorder],ftype='$add[ftype]',flen='$add[flen]',fvalue='".eaddslashes2($add[fvalue])."',fformsize='$add[fformsize]' where fid=$fid");
	//更新表单
	ChangeClassForm();
	if($usql&&$sql)
	{
...
function DelClassF($add,$userid,$username){
	...
	$r=$Avatar->fetch1("select f from {$dbtbpre}enewsclassf where fid='$fid'");
	if(!$r[f]){
		printerror("EmptyFid","history.go(-1)");
	}
	$usql=$Avatar->query("alter table {$dbtbpre}enewsclassadd drop COLUMN `".$r[f]."`");
	$sql=$Avatar->query("delete from {$dbtbpre}enewsclassf where fid='$fid'");
	//更新表单表
	ChangeClassForm();
	if($usql&&$sql)
	{
...
function EditClassFOrder($fid,$myorder,$userid,$username){
	...
	for($i=0;$i<count($myorder);$i++)
	{
		$fid[$i]=(int)$fid[$i];
		$newmyorder=(int)$myorder[$i];
		$usql=$Avatar->query("update {$dbtbpre}enewsclassf set myorder=$newmyorder where fid='$fid[$i]'");
    }
	//更新表单表
	ChangeClassForm();
...
function ReturnClassAddF($add,$ecms=0){
	...
	$ret_r[0]='';
	$ret_r[1]='';
	$fsql=$Avatar->query("select f from {$dbtbpre}enewsclassf");
	if($ecms==0)//增加
	{
		while($fr=$Avatar->fetch($fsql))
		{
...
function TogNotReClass($changecache=0){
	global $Avatar,$dbtbpre;
	$sql=$Avatar->query("select classid,nreclass,nreinfo,nrejs,nottobq from {$dbtbpre}enewsclass where nreclass=1 or nreinfo=1 or nrejs=1 or nottobq=1");
	$nreclass=',';
	$nreinfo=',';
	$nrejs=',';
	$nottobq=',';
	...
	$Avatar->query("update {$dbtbpre}enewspublic set nreclass='$nreclass',nreinfo='$nreinfo',nrejs='$nrejs',nottobq='$nottobq' limit 1");
	if($changecache==1)
	{
		GetConfig();
	}
...
function AddWbClass($add,$userid,$username){
	...
	{
		//取得上一级父栏目
		$r=$Avatar->fetch1("select featherclass,islast,wburl from {$dbtbpre}enewsclass where classid='$add[bclassid]'");
		if($r[islast])//是否终极栏目
		{
	...
	{
		//删除导航缓存
		$Avatar->query("delete from {$dbtbpre}enewsclassnavcache where navtype='listclass'");
		$cache_enews='doclass';
...
function AddClass($add,$userid,$username){
	...
	if(!$add[islast])
	{
		...
		{
			//删除导航缓存
			$Avatar->query("delete from {$dbtbpre}enewsclassnavcache where navtype='listclass' or navtype='listenews' or navtype='jsclass' or (navtype='modclass' and modid='$add[modid]')");
	...
	else
	{
		...
		if($add[bclassid])
		{
			$b_r=$Avatar->fetch1("select sonclass,featherclass from {$dbtbpre}enewsclass where classid='$add[bclassid]'");
			if(empty($b_r[sonclass]))
			{
				$b_r[sonclass]="|";
			}
			$new_sonclass=$b_r[sonclass].$lastid."|";
			$update=$Avatar->query("update {$dbtbpre}enewsclass set sonclass='$new_sonclass' where classid='$add[bclassid]'");
			//更改父类别的父栏目的子栏目
			$where=ReturnClass($b_r[featherclass]);
			if(empty($where)){
				$where="classid=0";
			}
			$bsql=$Avatar->query("select sonclass,classid from {$dbtbpre}enewsclass where ".$where);
			while($br=$Avatar->fetch($bsql))
			{
				...
				$update=$Avatar->query("update {$dbtbpre}enewsclass set sonclass='$new_sonclass' where classid='$br[classid]'");
            }
	    }
		//更新附件
		UpdateTheFileOther(1,$lastid,$add['filepass'],'other');
...
function AlterClassPath($classid,$islast,$oldclasspath,$classpath){
	...
	if($oldclasspath!=$classpath)
	{
		...
		if(empty($islast))
		{
			$sql=$Avatar->query("update {$dbtbpre}enewsclass set classpath=REPLACE(classpath,'".$oldclasspath."/','".$classpath."/') where featherclass like '%|".$classid."|%'");
		}
		DelListEnews();
	}
}
...
function EditWbClass($add,$userid,$username){
	...
	if($add[bclassid]<>$add[oldbclassid])
	{
		...
		else
		{
			...
			//取得现在大栏目的值
	 		$b=$Avatar->fetch1("select featherclass,sonclass,islast,wburl from {$dbtbpre}enewsclass where classid='$add[bclassid]'");
			//检测大栏目是否为终级栏目
			if($b[islast])
			{
				printerror("BclassNotLast","");
...
function EditClass($add,$userid,$username){
	...
	if(!$add[islast]){
		...
		if($add[bclassid]<>$add[oldbclassid]){
			...
			if(empty($add[bclassid])){
				...
				$featherclass="";
				//取得本栏目的子栏目
				$r=$Avatar->fetch1("select sonclass,featherclass,classpath from {$dbtbpre}enewsclass where classid='$add[classid]'");
				//改变父栏目的子栏目
				$where=ReturnClass($r[featherclass]);
				if(empty($where)){
					$where="classid=0";
				}
				$osql=$Avatar->query("select sonclass,classid from {$dbtbpre}enewsclass where ".$where);
				while($o=$Avatar->fetch($osql)){
					$newsonclass=str_replace($r[sonclass],"|",$o[sonclass]);
					$uosql=$Avatar->query("update {$dbtbpre}enewsclass set sonclass='$newsonclass' where classid='$o[classid]'");
				}
				//修改子栏目的父栏目
				$osql=$Avatar->query("select featherclass,classid,classpath from {$dbtbpre}enewsclass where featherclass like '%|".$add[classid]."%|'");
				while($o=$Avatar->fetch($osql)){
					$newclasspath=str_replace($r[classpath]."/",$classpath."/",$o[classpath]);
					$newfeatherclass=str_replace($r[featherclass],"|",$o[featherclass]);
					$uosql=$Avatar->query("update {$dbtbpre}enewsclass set featherclass='$newfeatherclass',classpath='$newclasspath' where classid='$o[classid]'");
				}
			}
			//转到中级栏目
			else
			{
				...
				$featherclass=$b[featherclass].$add[bclassid]."|";
				//取得现在栏目本身的值
				$o=$Avatar->fetch1("select featherclass,sonclass,classpath from {$dbtbpre}enewsclass where classid='$add[classid]'");
				//修改子栏目的父栏目
				$osql=$Avatar->query("select featherclass,classid,classpath from {$dbtbpre}enewsclass where featherclass like '%|".$add[classid]."|%'");
				while($or=$Avatar->fetch($osql)){
					...
					$uosql=$Avatar->query("update {$dbtbpre}enewsclass set featherclass='$newfeatherclass',classpath='$newclasspath' where classid='$or[classid]'");
				}
				//改变旧大栏目的所有子栏目
				...
				$oosql=$Avatar->query("select sonclass,classid from {$dbtbpre}enewsclass where ".$owhere);
				while($oo=$Avatar->fetch($oosql)){
					$newsonclass=str_replace($o[sonclass],"|",$oo[sonclass]);
					$usql=$Avatar->query("update {$dbtbpre}enewsclass set sonclass='$newsonclass' where classid='$oo[classid]'");
				}
				//改变新大栏目的子栏目
				...
				$nbsql=$Avatar->query("select sonclass,classid from {$dbtbpre}enewsclass where ".$where);
				while($nb=$Avatar->fetch($nbsql)){
					if(empty($nb[sonclass]))
					{$nb[sonclass]="|";}
					$newsonclass=$nb[sonclass].substr($o[sonclass],1);
					$usql=$Avatar->query("update {$dbtbpre}enewsclass set sonclass='$newsonclass' where classid='$nb[classid]'");
				}
			}
			$change=",bclassid=$add[bclassid],featherclass='$featherclass'";
		}
		...
		{
			$Avatar->query("update {$dbtbpre}enewsclass set wapstyleid='$add[wapstyleid]' where featherclass like '%|".$add[classid]."|%'");
		}
		//修改数据库资料
	...
	else
	{
		if($add[modid]<>$add[oldmodid])//换系统模型
		{
			...
			{
				$chmchecknum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_".$chmtbr[tbname]."_index where classid='$add[classid]'");
				if($chmchecknum)
				{
		...
		if($add[bclassid]<>$add[oldbclassid]){
			...
			if(empty($add[bclassid])){
				...
				$featherclass="";
				//取得栏目原本的大栏目
				$r=$Avatar->fetch1("select featherclass,classpath from {$dbtbpre}enewsclass where classid='$add[classid]'");
				//改变原本大栏目的子栏目
				$where=ReturnClass($r[featherclass]);
				if(empty($where)){
					$where="classid=0";
				}
				$bsql=$Avatar->query("select classid,sonclass from {$dbtbpre}enewsclass where ".$where);
				while($br=$Avatar->fetch($bsql)){
					$newsonclass=str_replace("|".$add[classid]."|","|",$br[sonclass]);
					$usql=$Avatar->query("update {$dbtbpre}enewsclass set sonclass='$newsonclass' where classid='$br[classid]'");
				}
			}
			//转到中级栏目
			else
			{
				//取得现在大栏目的值
				$b=$Avatar->fetch1("select featherclass,islast,wburl from {$dbtbpre}enewsclass where classid='$add[bclassid]'");
				//检测大栏目是否为终级栏目
				if($b[islast])
				{
					printerror("BclassNotLast","");
				...
				//改变旧大栏目的子栏目
				$o=$Avatar->fetch1("select sonclass,featherclass from {$dbtbpre}enewsclass where classid='$add[classid]'");
				$where=ReturnClass($o[featherclass]);
				if(empty($where)){
					$where="classid=0";
				}
				$osql=$Avatar->query("select sonclass,classid from {$dbtbpre}enewsclass where ".$where);
				while($ob=$Avatar->fetch($osql)){
				   $newsonclass=str_replace("|".$add[classid]."|","|",$ob[sonclass]);
				   $usql=$Avatar->query("update {$dbtbpre}enewsclass set sonclass='$newsonclass' where classid='$ob[classid]'");
			   }
			}
			$change=",bclassid=$add[bclassid],featherclass='$featherclass'";
		}
	...
	if($add[bclassid]<>$add[oldbclassid]||($add[oldclasspath]<>$classpath&&$add['classpath']==$add['oldcpath'])){
		...
		if($add['oldcpath']<>$add['classpath'])//更换栏目目录
		{
			...
			//删除导航缓存
			$Avatar->query("delete from {$dbtbpre}enewsclassnavcache where navtype='listclass' or navtype='listenews' or navtype='jsclass' or navtype='userenews'");
			GetClass();
		}
	}
	//删除缓存文件
	...
	if($add[oldclassname]<>$add[classname]||$add[bclassid]<>$add[oldbclassid])
	{
		...
		DelFiletext("../d/js/js/addinfo".$add[modid].".js");
		//删除导航缓存
		$Avatar->query("delete from {$dbtbpre}enewsclassnavcache where navtype='listclass' or navtype='listenews' or navtype='jsclass' or navtype='userenews' or (navtype='modclass' and modid='$add[modid]')");
		$cache_mid=$add[modid];
    }
	else
	{
		...
		if($add[openadd]<>$add[oldopenadd]||$add[modid]<>$add[oldmodid])
		{
			...
			DelFiletext("../d/js/js/addinfo".$add[modid].".js");
			//删除导航缓存
			$Avatar->query("delete from {$dbtbpre}enewsclassnavcache where navtype='modclass' and modid='$add[modid]'");
			$cache_mid=$add[modid];
			if($add[modid]<>$add[oldmodid])
			{
				...
				//删除导航缓存
				$Avatar->query("delete from {$dbtbpre}enewsclassnavcache where navtype='modclass' and modid='$add[oldmodid]'");
...
function ChangeClassIslast($reclassid,$userid,$username){
	...
	//取得本栏目信息
	$r=$Avatar->fetch1("select classid,sonclass,featherclass,islist,islast,classname,modid,tbname,wburl from {$dbtbpre}enewsclass where classid=$classid");
	if(empty($r[classid]))
	{
		printerror("NotChangeIslastClassid","");
	}
	...
	if(!$r[islast])
	{
		$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsclass where bclassid=$classid");
		if($num)
		{
		...
		$sql=$Avatar->query("select classid,sonclass from {$dbtbpre}enewsclass where ".$where);
		while($br=$Avatar->fetch($sql))
		{
			if(empty($br[sonclass]))
			{
				$br[sonclass]="|";
		...
		$dosql=$Avatar->query("update {$dbtbpre}enewsclass set islast=1 where classid=$classid");
		$mess="ChangeClassToLastSuccess";
	}
	//终极栏目
	else
	{
		$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_".$r[tbname]."_index where classid='$classid'");
		if($num)
		{
		...
		$dosql=$Avatar->query("update {$dbtbpre}enewsclass set islast=0 where classid=$classid");
	...
	{
		//删除导航缓存
		$Avatar->query("delete from {$dbtbpre}enewsclassnavcache where navtype='listclass' or navtype='listenews' or navtype='jsclass' or navtype='userenews' or (navtype='modclass' and modid='$r[modid]')");
...
function DelClass($classid,$userid,$username){
	...
	//操作权限
	CheckLevel($userid,$username,$classid,"class");
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewsclass where classid='$classid'");
	if(empty($r[classid]))
	{
...
function DelClass1($classid){
	...
	if($r[wburl])
	{
		$sql=$Avatar->query("delete from {$dbtbpre}enewsclass where classid='$classid'");
		$Avatar->query("delete from {$dbtbpre}enewsclassadd where classid='$classid'");
		$Avatar->query("delete from {$dbtbpre}enewsclass_stats where classid='$classid'");
		//删除栏目附件
	...
	{
		//删除主表信息
		$indexsql=$Avatar->query("delete from {$dbtbpre}ecms_".$r[tbname]."_index where classid='$classid'");
		$sql=$Avatar->query("delete from {$dbtbpre}ecms_".$r[tbname]." where classid='$classid'");
		$Avatar->query("delete from {$dbtbpre}ecms_".$r[tbname]."_check where classid='$classid'");
		$Avatar->query("delete from {$dbtbpre}ecms_".$r[tbname]."_doc where classid='$classid'");
		//删除副表信息
		...
		DelFileOtherTable("modtype=1 and id='$classid'");
		//删除栏目本身
	    $sql1=$Avatar->query("delete from {$dbtbpre}enewsclass where classid='$classid'");
		$Avatar->query("delete from {$dbtbpre}enewsclassadd where classid='$classid'");
		$Avatar->query("delete from {$dbtbpre}enewsclass_stats where classid='$classid'");
		$delpath="../../".$r[classpath];
		$del=DelPath($delpath);
	...
	else
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			$delcid=$delcr[$i];
			//删除主表信息
			$indexsql=$Avatar->query("delete from {$dbtbpre}ecms_".$class_r[$delcid][tbname]."_index where classid='$delcid'");
			$sql=$Avatar->query("delete from {$dbtbpre}ecms_".$class_r[$delcid][tbname]." where classid='$delcid'");
			$Avatar->query("delete from {$dbtbpre}ecms_".$class_r[$delcid][tbname]."_check where classid='$delcid'");
			$Avatar->query("delete from {$dbtbpre}ecms_".$class_r[$delcid][tbname]."_doc where classid='$delcid'");
			//删除副表信息
		...
		//删除子栏目副表
		$fcsql=$Avatar->query("select classid from {$dbtbpre}enewsclass where featherclass like '%|".$classid."|%'");
		while($fcr=$Avatar->fetch($fcsql))
		{
			$Avatar->query("delete from {$dbtbpre}enewsclassadd where classid='$fcr[classid]'");
			$Avatar->query("delete from {$dbtbpre}enewsclass_stats where classid='$fcr[classid]'");
		}
		//删除子栏目
		$sql1=$Avatar->query("delete from {$dbtbpre}enewsclass where featherclass like '%|".$classid."|%'");
		//改变父栏目的子类
		$where=ReturnClass($r[featherclass]);
		if(empty($where))
		{$where="classid=0";}
		$bbsql=$Avatar->query("select classid,sonclass from {$dbtbpre}enewsclass where ".$where);
		while($bbr=$Avatar->fetch($bbsql))
		{
			$newsonclass=str_replace($r[sonclass],"|",$bbr[sonclass]);
			$usql=$Avatar->query("update {$dbtbpre}enewsclass set sonclass='$newsonclass' where classid='$bbr[classid]'");
		}
		//删除栏目本身
		$sql2=$Avatar->query("delete from {$dbtbpre}enewsclass where classid='$classid'");
		$Avatar->query("delete from {$dbtbpre}enewsclassadd where classid='$classid'");
		$Avatar->query("delete from {$dbtbpre}enewsclass_stats where classid='$classid'");
		//删除栏目附件
		DelFileOtherTable("modtype=1 and id='$classid'");
...
function EditClassOrder($classid,$myorder,$userid,$username){
	...
	{
		$newmyorder=(int)$myorder[$i];
		$sql=$Avatar->query("update {$dbtbpre}enewsclass set myorder=$newmyorder where classid='$classid[$i]'");
    }
	//删除缓存
	DelListEnews();
	//删除导航缓存
...
function ChangeSonclass($start,$userid,$username){
	...
	$start=(int)$start;
	$b=0;
	$sql=$Avatar->query("select classid from {$dbtbpre}enewsclass where islast=0 and classid>".$start." order by classid limit ".$public_r[relistnum]);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$r[classid];
		//子栏目
		$sonclass="|";
		$ssql=$Avatar->query("select classid from {$dbtbpre}enewsclass where islast=1 and featherclass like '%|".$r[classid]."|%' order by classid");
		while($sr=$Avatar->fetch($ssql))
		{
			$sonclass.=$sr[classid]."|";
	    }
		$usql=$Avatar->query("update {$dbtbpre}enewsclass set sonclass='$sonclass' where classid='$r[classid]'");
    }
	//完毕
	if(empty($b))
	{
...
function DelFcListClass(){
	...
	DelListEnews();
	//删除导航缓存
	$Avatar->query("delete from {$dbtbpre}enewsclassnavcache");
	$cache_enews='doclass,doinfo,douserinfo,domod,dostemp';
...
function SetMoreClass($add,$userid,$username){
	...
	$sql=$Avatar->query("update {$dbtbpre}enewsclass set ".$seting." where ".$whereclass);
	//内容模板应用于子生成的信息
	if($donewstemp==1)
	{
		$csql=$Avatar->query("select classid,tbname from {$dbtbpre}enewsclass where (".$whereclass.") and islast=1");
		while($r=$Avatar->fetch($csql))
		{
