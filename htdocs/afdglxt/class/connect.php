...
define('InAvatarCMS',TRUE);
define('ECMS_PATH',substr(dirname(__FILE__),0,-13));
...
require_once ECMS_PATH.'afdglxt/config/config.php';

if(!defined('AvatarCMSConfig'))
{
	exit();
}
...
if(defined('AvatarCMSAdmin'))
{
	...
	{
		session_start();
		define('AvatarCMSDefSession',TRUE);
	}
}
else
{
...
function printerror($error="",$gotourl="",$ecms=0,$noautourl=0,$novar=0){
	global $Avatar,$editor,$public_r,$ecms_config;
	if($editor==1){$a="../";}
	elseif($editor==2){$a="../../";}
	elseif($editor==3){$a="../../../";}
	else{$a="";}
...
function printerror2($error='',$gotourl='',$ecms=0,$noautourl=0){
	...
	{
		@include(ECMS_PATH.'afdglxt/message/index.php');
	}
	db_close();
	exit();
}
...
function eGetTableModids($tid,$tbname){
	...
	$sql=$Avatar->query("select mid from {$dbtbpre}enewsmod where ".$where);
...
function RepTempvarPostStrT($val,$ispagef=0){
	if($ispagef==1)
	{
		$val=str_replace('[!--Avatarnews.page--]','[!!!-Avatarcms.page-!!]',$val);
	}
	$val=str_replace('[!--','&#091;!--',$val);
	if($ispagef==1)
	{
		$val=str_replace('[!!!-Avatarcms.page-!!]','[!--Avatarnews.page--]',$val);
	}
	return $val;
}
...
function eReturnRewriteInfoUrl($classid,$id,$ecms=0){
	...
	{
		$r['pageurl']=$public_r['newsurl']."afdglxt/action/ShowInfo.php?classid=$classid&id=$id";
		$r['rewrite']=0;
	}
	else
	{
...
function eReturnRewriteClassUrl($classid,$ecms=0){
	...
	{
		$r['pageurl']=$public_r['newsurl']."afdglxt/action/ListInfo/?classid=$classid";
		$r['rewrite']=0;
	}
	else
	{
...
function eReturnRewriteTitleTypeUrl($ttid,$ecms=0){
	...
	{
		$r['pageurl']=$public_r['newsurl']."afdglxt/action/InfoType/?ttid=$ttid";
		$r['rewrite']=0;
	}
	else
	{
...
function eReturnRewriteTagsUrl($tagid,$tagname,$ecms=0){
	...
	{
		$r['pageurl']=$public_r['newsurl']."afdglxt/tags/?tagname=".$tagname;
		$r['rewrite']=0;
	}
	else
	{
...
function ReturnFirsttitleNameList($firsttitle,$isgood){
	global $Avatar,$dbtbpre;
	$pubr=$Avatar->fetch1("select firsttitlename,isgoodname from {$dbtbpre}enewspublic limit 1");
	//头条
...
function DoFileMkDir($file){
	$path=dirname($file.'Avatarcms.txt');
	DoMkdir($path);
}
...
function sys_ReturnBqTitleLink($r){
	...
	else
	{
		...
		{
			$titleurl=$public_r['newsurl'].'afdglxt/public/jump/?classid='.$r['classid'].'&id='.$r['id'];
		}
		return $titleurl;
	}
}
...
function sys_ReturnBqTitleLinkDt($r){
	...
	if(empty($r['isurl']))
	{
		...
		{
			$titleurl=$public_r[newsurl]."afdglxt/action/ShowInfo/?classid=$r[classid]&id=$r[id]";
			return $titleurl;
		}
...
function eGetTableRowNum($tbname){
	global $Avatar,$dbtbpre;
	$total_r=$Avatar->fetch1("SHOW TABLE STATUS LIKE '".$tbname."';");
	return $total_r['Rows'];
}
...
function AddClassInfos($classid,$addallstr,$addstr,$checked=1){
	...
	$Avatar->query("update {$dbtbpre}enewsclass set ".$updatestr." where classid='$classid' limit 1");
}
...
function ReturnClassInfoNum($cr,$ecms=0){
	...
	else
	{
		...
		$num=$Avatar->gettotal("select sum(".$f.") as total from {$dbtbpre}enewsclass where ".ReturnClass($class_r[$cr[classid]][sonclass]));
		$num=(int)$num;
	}
	return $num;
}
...
function ResetClassInfos($classid){
	global $Avatar,$dbtbpre,$class_r;
	$infos=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where classid='$classid'");
	$checkinfos=$Avatar->gettotal("select count(*) as total from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_check where classid='$classid'");
	$allinfos=$infos+$checkinfos;
	$Avatar->query("update {$dbtbpre}enewsclass set allinfos='$allinfos',infos='$infos' where classid='$classid' limit 1");
}
...
function UpdateSingleInfoPlnum($classid,$id,$checked=1){
	...
	$r=$Avatar->fetch1("select id,restb,plnum from ".$infotb." where id='$id' limit 1");
	if(empty($r['restb']))
	{
		return '';
	}
	$pubid=ReturnInfoPubid($classid,$id);
	$plnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewspl_".$r['restb']." where pubid='$pubid' limit 1");
	if($plnum==$r['plnum'])
	{
		return '';
	}
	$Avatar->query("update ".$infotb." set plnum='$plnum' where id='$id' limit 1");
}
...
function DoUpdateAddDataNum($type='info',$stb,$addnum=1){
	...
	$sqladdupdate='';
	$time=time();
	$pur=$Avatar->fetch1("select ".$lasttimef.",".$lastnumtbf.$sqladdf." from {$dbtbpre}enewspublic_update limit 1");
	if($stb)
	{
	...
	$Avatar->query("update {$dbtbpre}enewspublic_update set ".$lastnumf."=".$lastnumf."+".$addnum.$sqladdupdate." limit 1");
}
...
function DoResetAddDataNum($type='info'){
	...
	$time=time();
	$Avatar->query("update {$dbtbpre}enewspublic_update set ".$lasttimef."='$time',".$lastnumf."=0,".$lastnumtbf."='' limit 1");
}

//更新昨日信息数统计
function DoUpdateYesterdayAddDataNum(){
	global $Avatar,$dbtbpre;
	$pur=$Avatar->fetch1("select * from {$dbtbpre}enewspublic_update limit 1");
	$todaydate=date('Y-m-d');
	if($todaydate==date('Y-m-d',$pur['todaytimeinfo'])&&$todaydate==date('Y-m-d',$pur['todaytimepl']))
	{
		return '';
	...
	$time=time();
	$Avatar->query("update {$dbtbpre}enewspublic_update set todaytimeinfo='$time',todaytimepl='$time',todaynuminfo=0,yesterdaynuminfo='$yesterdaynuminfo',todaynumpl=0,yesterdaynumpl='$yesterdaynumpl' limit 1");
}

//返回栏目自定义字段内容
function ReturnClassAddField($classid,$f){
	global $Avatar,$dbtbpre,$navclassid;
	if(empty($classid))
	{
		$classid=$navclassid;
	}
	$fr=$Avatar->fetch1("select ".$f." from {$dbtbpre}enewsclassadd where classid='$classid' limit 1");
	if(strstr($f,','))
	{
		return $fr;
	}
...
function ReturnZtAddField($classid,$f){
	...
	$fr=$Avatar->fetch1("select ".$f." from {$dbtbpre}enewsztadd where ztid='$classid' limit 1");
	if(strstr($f,','))
	{
		return $fr;
	}
...
function ReturnPublicAddVar($myvar){
	...
	if(strstr($myvar,','))
	{
		...
		{
			$v=$myvr[$i];
			$vr=$Avatar->fetch1("select varvalue from {$dbtbpre}enewspubvar where myvar='$v' limit 1");
	...
	{
		$vr=$Avatar->fetch1("select varvalue from {$dbtbpre}enewspubvar where myvar='$myvar' limit 1");
		return $vr['varvalue'];
	}
}
...
function ReturnIndexTableInfo($tbname,$f,$classid,$id){
	global $dbtbpre;
	$r=$Avatar->fetch1("select ".$f." from {$dbtbpre}ecms_".$tbname."_index where id='$id' limit 1");
	return $r;
}
...
function eCheckTbHaveField($tid,$tbname,$f){
	...
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsf where ".$where." limit 1");
	return $num;
}
...
function ReplaceListVars($no,$listtemp,$subnews,$subtitle,$formatdate,$url,$haveclass=0,$r,$field,$docode=0){
	global $Avatar,$public_r,$class_r,$class_zr,$fun_r,$dbtbpre,$emod_r,$class_tr,$level_r,$navclassid,$etable_r;
	if($haveclass)
	{
	...
	for($i=1;$i<$fcount;$i++)
	{
		...
		elseif($f=='titlepic')//标题图片
		{
			if(empty($value))
		    {
				$value=$public_r[newsurl].'afdglxt/data/images/notimg.gif';
			}
			$spf=1;
		}
...
function ReplaceBefrom($befrom){
	...
	$r=$Avatar->fetch1("select befromid,sitename,siteurl from {$dbtbpre}enewsbefrom where sitename='$befrom' limit 1");
	if(empty($r[befromid]))
	{return $befrom;}
...
function ReplaceWriter($writer){
	...
	$r=$Avatar->fetch1("select wid,writer,email from {$dbtbpre}enewswriter where writer='$writer' limit 1");
...
function BakDown($classid,$id,$pathid,$userid,$username,$title,$cardfen,$online=0){
	...
	$cardfen=(int)$cardfen;
	$classid=(int)$classid;
	$sql=$Avatar->query("insert into {$dbtbpre}enewsdownrecord(id,pathid,userid,username,title,cardfen,truetime,classid,online) values($id,$pathid,$userid,'$username','".addslashes($title)."',$cardfen,$truetime,$classid,'$online');");
}
...
function BakBuy($userid,$username,$buyname,$userfen,$money,$userdate,$type=0){
	...
	$Avatar->query("insert into {$dbtbpre}enewsbuybak(userid,username,card_no,cardfen,money,buytime,userdate,type) values('$userid','$username','$buyname','$userfen','$money','$buytime','$userdate','$type');");
}
...
function eSendMsg($title,$msgtext,$to_username,$from_userid,$from_username,$isadmin,$issys,$ecms=0){
	...
	$Avatar->query("insert into ".$tbname."(title,msgtext,haveread,msgtime,to_username,from_userid,from_username,isadmin,issys) values('$title','$msgtext',0,'$msgtime','$to_username','$from_userid','$from_username','$isadmin','$issys');");
	//消息状态
	$userr=$Avatar->fetch1("select ".eReturnSelectMemberF('userid,havemsg')." from ".eReturnMemberTable()." where ".egetmf('username')."='$to_username' limit 1");
	if(!$userr['havemsg'])
	{
		$newhavemsg=eReturnSetHavemsg($userr['havemsg'],0);
		$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('havemsg')."='$newhavemsg' where ".egetmf('userid')."='".$userr['userid']."' limit 1");
	}
}
...
function eSendNotice($title,$msgtext,$to_username,$from_userid,$from_username,$ecms=0){
	...
	$msgtime=date("Y-m-d H:i:s");
	$Avatar->query("insert into ".$tbname."(title,msgtext,haveread,msgtime,to_username,from_userid,from_username) values('".$title."','".$msgtext."',0,'$msgtime','$to_username','$from_userid','$from_username');");
}
...
function SubSchallSmalltext($value,$len){
	$value=str_replace(array("\r\n","&nbsp;","[!--Avatarnews.page--]","[/!--Avatarnews.page--]"),array("","","",""),$value);
	$value=strip_tags($value);
	if($len)
	{
		$value=sub($value,0,$len,false);
...
function GetInfoFilename($classid,$id){
	...
	$infor=$Avatar->fetch1("select isurl,groupid,classid,newspath,filename,id from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where id='$id' limit 1");
...
function eInsertFileTable($filename,$filesize,$path,$adduser,$classid,$no,$type,$id,$cjid,$fpath,$pubid,$modtype=0,$fstb=1){
	...
	{
		$fstb=eReturnFileStb($fstb);
		$sql=$Avatar->query("insert into {$dbtbpre}enewsfile_".$fstb."(pubid,filename,filesize,adduser,path,filetime,classid,no,type,id,cjid,onclick,fpath) values('$pubid','$filename','$filesize','$adduser','$path','$filetime','$classid','$no','$type','$id','$cjid',0,'$fpath');");
	}
	elseif($modtype==5)//公共
	{
		$sql=$Avatar->query("insert into {$dbtbpre}enewsfile_public(filename,filesize,adduser,path,filetime,modtype,no,type,id,cjid,onclick,fpath) values('$filename','$filesize','$adduser','$path','$filetime',0,'$no','$type','$id','$cjid',0,'$fpath');");
	}
	elseif($modtype==6)//会员
	{
		$sql=$Avatar->query("insert into {$dbtbpre}enewsfile_member(filename,filesize,adduser,path,filetime,no,type,id,cjid,onclick,fpath) values('$filename','$filesize','$adduser','$path','$filetime','$no','$type','$id','$cjid',0,'$fpath');");
	}
	else//其他
	{
		$sql=$Avatar->query("insert into {$dbtbpre}enewsfile_other(filename,filesize,adduser,path,filetime,modtype,no,type,id,cjid,onclick,fpath) values('$filename','$filesize','$adduser','$path','$filetime','$modtype','$no','$type','$id','$cjid',0,'$fpath');");
	}
	return $sql;
}
...
function UpdateTheFileOther($modtype,$id,$checkpass,$tb='other'){
	...
	$sql=$Avatar->query("update {$dbtbpre}enewsfile_{$tb} set id='$id',cjid=0 where cjid='$checkpass'".$where);
}
...
function UpdateTheFileEditOther($modtype,$id,$tb='other'){
	...
	$sql=$Avatar->query("update {$dbtbpre}enewsfile_{$tb} set cjid=0 where id='$id'".$where);
}
...
function ReturnDownQzPath($path,$urlid){
	...
	{
		$r=$Avatar->fetch1("select urlid,url,downtype from {$dbtbpre}enewsdownurlqz where urlid='$urlid'");
		if($r['urlid'])
		{
...
function ReturnDSofturl($downurl,$qz,$path='../../',$isdown=0){
	...
	$url=$urlr['repath'];
	@include_once(ECMS_PATH."afdglxt/DownSys/class/enpath.php");//防盗链
	if($isdown)
	{
...
function DoSafeCheckFromurl(){
	global $ecms_config;
	if($ecms_config['esafe']['ckfromurl']==0||defined('AvatarCMSNFPage'))//不启用
	{
		return '';
	}
	...
	{
		if(defined('AvatarCMSAdmin')&&!stristr($fromurl,$domain.'/'))
		{
			echo"";
			exit();
		}
...
function eCheckAccessDoIp($doing){
	global $public_r,$Avatar,$dbtbpre;
	$pr=$Avatar->fetch1("select opendoip,closedoip,doiptype from {$dbtbpre}enewspublic limit 1");
...
function RepPltextFace($text){
	...
	{
		$r=explode('##',$facer[$i]);
		$text=str_replace($r[0],"<img src='".$public_r['newsurl']."afdglxt/data/face/".$r[1]."' border=0>",$text);
	}
	return $text;
}
...
function ecmsSetShowKey($varname,$val,$ecms=0){
	...
	$val=md5($val);
	$time=time();
	$checkpass=md5(md5($val.'AvatarCMS'.$time).$public_r['keyrnd']);
...
function ecmsCheckShowKey($varname,$postval,$dopr,$ecms=0){
	...
	$checkpass=md5(md5(md5($postval).'AvatarCMS'.$cktime).$public_r['keyrnd']);
	if($checkpass<>$pass)
	{
		printerror('FailKey','',$dopr);
	}
...
function toReturnFname($tbname,$f){
	global $Avatar,$dbtbpre;
	$r=$Avatar->fetch1("select fname from {$dbtbpre}enewsf where f='$f' and tbname='$tbname' limit 1");
	return $r[fname];
}
...
function ReturnPinyinFun($hz){
	global $ecms_config;
	include_once(ECMS_PATH.'afdglxt/class/epinyin.php');
	//编码
...
function sys_CheckMemberGroup($groupid){
	if(!defined('InAvatarCMSUser'))
	{
		include_once ECMS_PATH.'afdglxt/member/class/user.php';
	}
...
function DoAvatarCMSFireWall(){
	...
	if(!defined('AvatarCMSAPage')&&$ecms_config['fw']['adminckpassvar']&&$ecms_config['fw']['adminckpassval'])
	{
		FWCheckPassword();
	}
}
...
function FWSetPassword(){
	...
	$ecmsckpass=md5(md5($ecms_config['fw']['adminckpassval'].'-Avatarcms-'.$ecms_config['fw']['epass']).'-'.$ip.'-'.$ecms_config['fw']['adminckpassval'].'-caity.net-');
