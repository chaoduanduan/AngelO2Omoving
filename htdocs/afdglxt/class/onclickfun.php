...
function InfoOnclick($classid,$id){
	...
	$r=$Avatar->fetch1("select tbname,tid from {$dbtbpre}enewsclass where classid='$classid'");
	if(empty($r[tbname]))
	{
		return '';
	}
	...
	{
		$Avatar->query("update {$dbtbpre}ecms_".$r[tbname]." set onclick=onclick+1 where id='$id'");
	}
	elseif($public_r['onclicktype']==1)
	{
		$filename=ECMS_PATH.'afdglxt/data/filecache/onclick/ocinfo'.$r[tid].'.log';
...
function ClassOnclick($classid){
	...
	if($public_r['onclicktype']==0)
	{
		$Avatar->query("update {$dbtbpre}enewsclass set onclick=onclick+1 where classid='$classid'");
	}
	elseif($public_r['onclicktype']==1)
	{
		$filename=ECMS_PATH.'afdglxt/data/filecache/onclick/occlass.log';
...
function ZtOnclick($ztid){
	...
	if($public_r['onclicktype']==0)
	{
		$Avatar->query("update {$dbtbpre}enewszt set onclick=onclick+1 where ztid='$ztid'");
	}
	elseif($public_r['onclicktype']==1)
	{
		$filename=ECMS_PATH.'afdglxt/data/filecache/onclick/oczt.log';
...
function eDoUpdateOnclick($table,$idf,$onclickf,$filename){
	...
	if(filesize($filename)>=$public_r['onclickfilesize']*1024||time()-filectime($filename)>=$public_r['onclickfiletime']*60)
	{
		...
		if(@$lr=file($filename))
		{
			...
			{
				$Avatar->query("UPDATE LOW_PRIORITY $table SET $onclickf=$onclickf+'$oc' WHERE $idf IN (0$ids)");
			}
		}
	}
}
