...
function ChangeInfoOtherLink($start,$classid,$from,$retype,$startday,$endday,$startid,$endid,$tbname,$userid,$username){
	...
	$b=0;
	$sql=$Avatar->query("select id,classid,checked from {$dbtbpre}ecms_".$tbname."_index where id>$start".$add1." order by id limit ".$public_r[infolinknum]);
	while($r=$Avatar->fetch($sql))
	{
		...
		$infopr=$Avatar->fetch1("select diyotherlink from {$dbtbpre}enewsinfovote where pubid='$pubid' limit 1");
		...
		$infor=$Avatar->fetch1("select stb,keyboard from ".$infotb." where id='$r[id]' limit 1");
		//返回表信息
		...
		$usql=$Avatar->query("update ".$infodatatb." set keyid='$newkeyid' where id='$r[id]' limit 1");
	}
	if(empty($b))
	{
	    insert_dolog("");//操作日志
...
function AddUserpage($add,$userid,$username){
	...
	$gid=(int)$add['gid'];
	$sql=$Avatar->query("insert into {$dbtbpre}enewspage(title,path,pagetext,classid,pagetitle,pagekeywords,pagedescription,tempid) values('$title','$path','".eaddslashes2($pagetext)."','$classid','".eaddslashes($pagetitle)."','".eaddslashes($pagekeywords)."','".eaddslashes($pagedescription)."','$tempid');");
	$id=$Avatar->lastid();
...
function EditUserpage($add,$userid,$username){
	...
	$tempid=(int)$add['tempid'];
	$gid=(int)$add['gid'];
	$sql=$Avatar->query("update {$dbtbpre}enewspage set title='$title',path='$path',pagetext='".eaddslashes2($pagetext)."',classid='$classid',pagetitle='".eaddslashes($pagetitle)."',pagekeywords='".eaddslashes($pagekeywords)."',pagedescription='".eaddslashes($pagedescription)."',tempid='$tempid' where id='$id'");
	ReUserpage($id,$pagetext,$path,$title,$pagetitle,$pagekeywords,$pagedescription,$tempid);
	if($sql)
	{
		//操作日志
...
function DelUserpage($id,$cid,$userid,$username){
	...
	$gid=(int)$_GET['gid'];
	$r=$Avatar->fetch1("select title,id,path from {$dbtbpre}enewspage where id='$id'");
	if(empty($r['id']))
	{
		printerror("NotDelUserpageid","history.go(-1)");
	}
	$sql=$Avatar->query("delete from {$dbtbpre}enewspage where id='$id'");
	DelFiletext($r['path']);
	if($sql)
	{
		//操作日志
...
function DoReUserpage($add,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$ur=$Avatar->fetch1("select id,path,pagetext,title,pagetitle,pagekeywords,pagedescription,tempid from {$dbtbpre}enewspage where id='".$id[$i]."'");
...
function DoRepNewstext($start,$oldword,$newword,$field,$classid,$tid,$tbname,$over,$dozz,$dotxt,$userid,$username){
	...
	$fr=$Avatar->fetch1("select tid,savetxt,tbdataf from {$dbtbpre}enewsf where tbname='$tbname' and f='$field' limit 1");
	//系统字段
	...
	if($dotype==1)
	{
		...
		if($fr['tbdataf'])//副表
		{
			...
			if($tbr['datatbs'])
			{
				...
				for($i=1;$i<$count-1;$i++)
				{
					$Avatar->query("update {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." set ".$field."='$repnewword'".$add2);
				}
			}
			//未审核
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_check_data set ".$field."='$repnewword'".$add2);
		}
		else//主表
		{
			$Avatar->query("update {$dbtbpre}ecms_".$tbname." set ".$field."='$repnewword'".$add2);
			//未审核
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_check set ".$field."='$repnewword'".$add2);
		}
		//替换完毕
	...
	elseif($fr['savetxt'])
	{
		...
		$start=(int)$start;
		$b=0;
		$sql=$Avatar->query("select id,checked from {$dbtbpre}ecms_".$tbname."_index where id>$start".$add1." order by id limit ".$public_r[dorepnum]);
		while($r=$Avatar->fetch($sql))
		{
			$b=1;
			$newstart=$r[id];
			...
			$infotb=ReturnInfoMainTbname($tbname,$r['checked']);
			//主表
			$infor=$Avatar->fetch1("select isurl".$selectf." from ".$infotb." where id='$r[id]' limit 1");
			if($infor['isurl'])
			{
				continue;
			}
			...
			if($fr['tbdataf'])
			{
				//返回表信息
				$infodatatb=ReturnInfoDataTbname($tbname,$r['checked'],$infor['stb']);
				$finfor=$Avatar->fetch1("select ".$field." from ".$infodatatb." where id='$r[id]' limit 1");
	...
	elseif($dozz==1)
	{
		...
		while($r=$Avatar->fetch($sql))
		{
			...
			if($fr['tbdataf'])//副表
			{
				...
				$Avatar->query("update ".$infodatatb." set ".$field."='".addslashes($newvalue)."' where id='$r[id]'");
			}
			else//主表
			{
				$newvalue=DoRepNewstextZz($oldword,$newword,stripSlashes($infor[$field]));//正则替换
				$Avatar->query("update ".$infotb." set ".$field."='".addslashes($newvalue)."' where id='$r[id]'");
			}
		}
		//替换完毕
		if(empty($b))
	...
	else
	{
		...
		if($fr['tbdataf'])//副表
		{
			...
			if($tbr['datatbs'])
			{
				...
				for($i=1;$i<$count-1;$i++)
				{
					$Avatar->query("update {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." set ".$field."=REPLACE(".$field.",'$repoldword','$repnewword')".$add2);
				}
			}
			//未审核
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_check_data set ".$field."=REPLACE(".$field.",'$repoldword','$repnewword')".$add2);
		}
		else//主表
		{
			$Avatar->query("update {$dbtbpre}ecms_".$tbname." set ".$field."=REPLACE(".$field.",'$repoldword','$repnewword')".$add2);
			//未审核
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_check set ".$field."=REPLACE(".$field.",'$repoldword','$repnewword')".$add2);
		}
		//替换完毕
		insert_dolog("tbname=".$tbname."&field=".$field."<br>oldword=".$oldword."<br>newword=".$newword);//操作日志
		printerror("DoRepNewstextSuccess","db/RepNewstext.php?tid=$tid".hReturnEcmsHashStrHref2(0));
...
function DoRepNewstextZz($oldword,$newword,$text){
	$zztext=RepInfoZZ($oldword,"Avatar-cms-wm.chief-caity",0);
...
function AddThisClass($add,$userid,$username){
	...
	$sql=$Avatar->query("insert into ".$thisr['tbname']."(classname) values('$add[classname]');");
	if($sql)
	{
...
function EditThisClass($add,$userid,$username){
	...
	//验证权限
    CheckLevel($userid,$username,$classid,$thisr['thelevel']);
	$sql=$Avatar->query("update ".$thisr['tbname']." set classname='$add[classname]' where classid='$classid'");
	if($sql)
	{
		//操作日志
...
function DelThisClass($classid,$doing,$userid,$username){
	...
	//验证权限
    CheckLevel($userid,$username,$classid,$thisr['thelevel']);
	$r=$Avatar->fetch1("select classname from ".$thisr['tbname']." where classid=$classid");
	$sql=$Avatar->query("delete from ".$thisr['tbname']." where classid=$classid");
	if($sql)
	{
		//操作日志
...
function RepDownLevel($add,$userid,$username){
	...
	$fr=$Avatar->fetch1("select tid,savetxt,tbdataf from {$dbtbpre}enewsf where tbname='$tbname' and f='$checkf' limit 1");
	if($fr['tbdataf'])//副表
	{
		$sfield=$field;
		$field='';
	...
	$update="";
	$b=0;
	$sql=$Avatar->query("select id,stb".$field." from {$dbtbpre}ecms_".$tbname." where id>$start".$wheresql." order by id limit ".$public_r['dorepdlevelnum']);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$r[id];
		...
		if($fr['tbdataf'])
		{
			$finfor=$Avatar->fetch1("select id".$sfield." from {$dbtbpre}ecms_".$tbname."_data_".$r[stb]." where id='$r[id]'");
		...
		if($fr['tbdataf'])
		{
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_data_".$r[stb]." set ".$update." where id='$r[id]'");
		}
		else
		{
			$Avatar->query("update {$dbtbpre}ecms_".$tbname." set ".$update." where id='$r[id]'");
		}
	}
	//替换完毕
	if(empty($b))
	{
		//未审核
		$sql=$Avatar->query("select id,stb".$field." from {$dbtbpre}ecms_".$tbname."_check".($wheresql?' where '.substr($wheresql,5):''));
		while($r=$Avatar->fetch($sql))
		{
			//副表
			if($fr['tbdataf'])
			{
				$finfor=$Avatar->fetch1("select id".$sfield." from {$dbtbpre}ecms_".$tbname."_check_data where id='$r[id]' limit 1");
				$r['downpath']=$finfor['downpath'];
				$r['onlinepath']=$finfor['onlinepath'];
			}
			$update='';
			...
			if($fr['tbdataf'])
			{
				$Avatar->query("update {$dbtbpre}ecms_".$tbname."_check_data set ".$update." where id='$r[id]'");
			}
			else
			{
				$Avatar->query("update {$dbtbpre}ecms_".$tbname."_check set ".$update." where id='$r[id]'");
			}
		}
		//操作日志
...
function ClearTmpFileData($add,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"changedata");
	//临时文件目录
	$tmppath=ECMS_PATH.'afdglxt/data/tmp';
	...
	//临时模板导出目录
	$temppath=ECMS_PATH.'afdglxt/data/tmp/temp';
	...
	//临时模型导出目录
	$modpath=ECMS_PATH.'afdglxt/data/tmp/mod';
	...
	//临时采集导出目录
	$cjpath=ECMS_PATH.'afdglxt/data/tmp/cj';
	$hand=@opendir($cjpath);
	while($file=@readdir($hand))
	{
	...
	//采集临时表
	$Avatar->query("TRUNCATE `{$dbtbpre}enewslinktmp`;");
	//远程发布临时表
	$Avatar->query("TRUNCATE `{$dbtbpre}enewspostdata`;");
...
function ClearBreakInfo($add,$userid,$username){
	...
	$tbr=$Avatar->fetch1("select datatbs,tbname from {$dbtbpre}enewstable where tid='$tid'");
	if(!$tbr['tbname'])
	{
		printerror('EmptyDocTb','');
	}
	...
	if($tbr['datatbs'])
	{
		...
		if($dodatatbs)
		{
			$Avatar->query("delete from {$dbtbpre}ecms_".$tbr['tbname']." where id not in (".$dodatatbs.")");
			$affnum=$Avatar->affectnum();
		}
	}
