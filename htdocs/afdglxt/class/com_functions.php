...
function AddGbookClass($add,$do=0,$userid,$username){
	...
	$sql=$Avatar->query("insert into ".$table."(bname".$mychecked.") values('$add[bname]'".$mycheckedvalue.");");
	if($sql)
	{
		$bid=$Avatar->lastid();
		//操作日志
...
function EditGbookClass($add,$do=0,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,$level);
	$sql=$Avatar->query("update ".$table." set bname='$add[bname]'".$mychecked." where bid='$add[bid]';");
	if($sql)
	{
		//操作日志
...
function DelGbookClass($bid,$do=0,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,$level);
	$r=$Avatar->fetch1("select bname from ".$table." where bid='$bid';");
	$sql=$Avatar->query("delete from ".$table." where bid='$bid';");
	$sql1=$Avatar->query("delete from ".$tabledata." where bid='$bid';");
	if($sql)
	{
		//操作日志
...
function ReturnGbookClass($bid,$do=0){
	...
	$sql=$Avatar->query("select bid,bname from ".$table." order by bid");
...
function ReGbook($lyid,$retext,$bid,$userid,$username){
	...
	$sql=$Avatar->query("update {$dbtbpre}enewsgbook set retext='$retext' where lyid='$lyid';");
	if($sql)
	{
		//操作日志
...
function DelGbook($lyid,$bid,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"gbook");
	$sql=$Avatar->query("delete from {$dbtbpre}enewsgbook where lyid='$lyid';");
	if($sql)
	{
		//操作日志
		insert_dolog("lyid=".$lyid);
...
function DelGbook_all($lyid,$bid,$userid,$username){
	...
	$sql=$Avatar->query("delete from {$dbtbpre}enewsgbook where ".$add);
	if($sql)
	{
		//操作日志
		insert_dolog("");
...
function CheckGbook_all($lyid,$bid,$userid,$username){
	...
	$add=substr($add,0,strlen($add)-4);
	$sql=$Avatar->query("update {$dbtbpre}enewsgbook set checked=0 where ".$add);
	if($sql)
	{
		//操作日志
		insert_dolog("");
...
function DelFeedbackFile($filename,$filepath){
	global $Avatar,$dbtbpre,$public_r,$efileftp_dr;
	if($filename)
	{
		$fpath=0;
		$getfpath=0;
		...
		for($j=0;$j<$fcount;$j++)
		{
			if(!$getfpath)
			{
				$ftr=$Avatar->fetch1("select fpath from {$dbtbpre}enewsfile_other where modtype=4 and path='$filepath' and filename='".$filer[$j]."' limit 1");
				$fpath=$ftr[fpath];
				$getfpath=1;
			}
		...
		$delsql=$Avatar->query("delete from {$dbtbpre}enewsfile_other where modtype=4 and path='$filepath' and (".$where.")");
	}
}
...
function DelFeedback($id,$bid,$userid,$username){
	...
	$r=$Avatar->fetch1("select id,title,filepath,filename,bid from {$dbtbpre}enewsfeedback where id='$id';");
	if(!$r['id'])
	{
		printerror("NotChangeFeedbackid","history.go(-1)");
    }
	...
	$sql=$Avatar->query("delete from {$dbtbpre}enewsfeedback where id='$id';");
	//删除附件
...
function DelFeedback_all($id,$bid,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		$id[$i]=(int)$id[$i];
		//删除附件
		$r=$Avatar->fetch1("select id,filepath,filename,bid from {$dbtbpre}enewsfeedback where id='".$id[$i]."';");
	...
	if($inid)
	{
		$sql=$Avatar->query("delete from {$dbtbpre}enewsfeedback where id in (".$inid.");");
	}
	if($sql)
	{
		//操作日志
...
function AddFeedbackF($add,$userid,$username){
	...
	//字段是否重复
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}enewsfeedback");
	$b=0;
	...
	//新增字段
	$asql=$Avatar->query("alter table {$dbtbpre}enewsfeedback add ".$field);
	//处理变量
	$add[myorder]=(int)$add[myorder];
	$sql=$Avatar->query("insert into {$dbtbpre}enewsfeedbackf(f,fname,fform,fzs,myorder,ftype,flen,fformsize,fvalue) values('$add[f]','$add[fname]','$add[fform]','".eaddslashes($add[fzs])."',$add[myorder],'$add[ftype]','$add[flen]','$add[fformsize]','".eaddslashes2($add[fvalue])."');");
...
function EditFeedbackF($add,$userid,$username){
	...
	$field="`".$add[f]."` ".$type." NOT NULL".$def;
	$usql=$Avatar->query("alter table {$dbtbpre}enewsfeedback change `".$add[oldf]."` ".$field);
	//处理变量
	$add[myorder]=(int)$add[myorder];
	$sql=$Avatar->query("update {$dbtbpre}enewsfeedbackf set f='$add[f]',fname='$add[fname]',fform='$add[fform]',fzs='".eaddslashes($add[fzs])."',myorder=$add[myorder],ftype='$add[ftype]',flen='$add[flen]',fformsize='$add[fformsize]',fvalue='".eaddslashes2($add[fvalue])."' where fid=$fid");
	//字段名更换
	if($add[f]<>$add[oldf])
	{
		$record="<!--record-->";
		...
		$slike=",".$add[oldf].",";
		$newslike=",".$add[f].",";
		$csql=$Avatar->query("select bid,enter,mustenter,filef,checkboxf from {$dbtbpre}enewsfeedbackclass where enter like '%$like%'");
		while($cr=$Avatar->fetch($csql))
		{
			...
			$cusql=$Avatar->query("update {$dbtbpre}enewsfeedbackclass set enter=REPLACE(enter,'$like','$newlike')".$setf." where bid='$cr[bid]'");
		}
	}
	if($usql&&$sql)
	{
...
function DelFeedbackF($add,$userid,$username){
	...
	//验证权限
	//CheckLevel($userid,$username,$classid,"feedbackf");
	$r=$Avatar->fetch1("select f from {$dbtbpre}enewsfeedbackf where fid=$fid");
	if(!$r[f])
	{
		printerror("EmptyFid","history.go(-1)");
	}
	...
	$usql=$Avatar->query("alter table {$dbtbpre}enewsfeedback drop COLUMN `".$r[f]."`");
	$sql=$Avatar->query("delete from {$dbtbpre}enewsfeedbackf where fid=$fid");
	//更新分类表
	$record="<!--record-->";
	$field="<!--field--->";
	...
	while($cr=$Avatar->fetch($csql))
	{
		...
		$cusql=$Avatar->query("update {$dbtbpre}enewsfeedbackclass set enter='$enter'".$setf." where bid='$cr[bid]'");
	}
	if($usql&&$sql)
	{
		//操作日志
...
function EditFeedbackFOrder($fid,$myorder,$userid,$username){
	...
	for($i=0;$i<count($myorder);$i++)
	{
		...
		$fid[$i]=(int)$fid[$i];
		$usql=$Avatar->query("update {$dbtbpre}enewsfeedbackf set myorder=$newmyorder where fid='$fid[$i]'");
    }
...
function ReturnAdminFeedbackClass($bid,$userid,$username){
	...
	$select='';
	$no=0;
	$sql=$Avatar->query("select bid,bname from {$dbtbpre}enewsfeedbackclass where usernames='' or usernames like '%,".$username.",%'");
	while($r=$Avatar->fetch($sql))
	{
		$no++;
		$bids.=$dh.$r['bid'];
	...
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsfeedbackclass");
	$ret_r['allbid']=0;
	if($num==$no)
	{
		$ret_r['allbid']=1;
...
function ReturnFeedbackBtemp($cname,$center,$mustenter){
	...
	{
		$v=$center[$i];
		$fr=$Avatar->fetch1("select fform,fformsize,fvalue from {$dbtbpre}enewsfeedbackf where f='$v' limit 1");
		if($fr['fform']=="file")
		{
...
function ReFeedbackClassFile($bid){
	global $Avatar,$dbtbpre;
	$r=$Avatar->fetch1("select btemp from {$dbtbpre}enewsfeedbackclass where bid='$bid'");
	//替换公共变量
	$url="<?=\$url?>";
	$pagetitle="<?=\$bname?>";
...
function ReMoreFeedbackClassFile($start=0,$userid,$username){
	...
	$sql=$Avatar->query("select bid from {$dbtbpre}enewsfeedbackclass order by bid");
	while($r=$Avatar->fetch($sql))
	{
...
function AddFeedbackClass($add,$userid,$username){
	...
	$sql=$Avatar->query("insert into {$dbtbpre}enewsfeedbackclass(bname,btemp,bzs,enter,mustenter,filef,groupid,checkboxf,usernames) values('$add[bname]','".eaddslashes2($add[btemp])."','".eaddslashes($add[bzs])."','$enter','$mustenter','$filef',$groupid,'$checkboxf','$add[usernames]');");
	$bid=$Avatar->lastid();
	//生成表单页面
...
function EditFeedbackClass($add,$userid,$username){
	...
	$sql=$Avatar->query("update {$dbtbpre}enewsfeedbackclass set bname='$add[bname]',btemp='".eaddslashes2($add[btemp])."',bzs='".eaddslashes($add[bzs])."',enter='$enter',mustenter='$mustenter',filef='$filef',groupid=$groupid,checkboxf='$checkboxf',usernames='$add[usernames]' where bid=$bid");
	//生成表单页面
	ReFeedbackClassFile($bid);
	if($sql)
	{
...
function DelFeedbackClass($add,$userid,$username){
	...
	//验证权限
	//CheckLevel($userid,$username,$classid,"feedbackf");
	$r=$Avatar->fetch1("select bid,bname from {$dbtbpre}enewsfeedbackclass where bid=$bid;");
	if(!$r['bid'])
	{printerror("NotChangeGbookClassid","history.go(-1)");}
	$sql=$Avatar->query("delete from {$dbtbpre}enewsfeedbackclass where bid=$bid;");
	//删除附件
	$fsql=$Avatar->query("select id,filepath,filename from {$dbtbpre}enewsfeedback where bid=$bid");
	while($fr=$Avatar->fetch($fsql))
	{
		DelFeedbackFile($fr['filename'],$fr['filepath']);
	}
	$sql1=$Avatar->query("delete from {$dbtbpre}enewsfeedback where bid=$bid;");
	//删除表单文件
	$file="../../tool/feedback/temp/feedback".$bid.".php";
	DelFiletext($file);
	if($sql)
...
function DelMoreMsg($add,$userid,$username){
	...
	$sql=$Avatar->query("delete from ".$tbname." where msgtime>'$starttime' and msgtime<'$endtime'".$a);
	if($sql)
	{
		//操作日志
...
function DoSendMsg($add,$ecms=0,$userid,$username){
	...
	if($ecms==1)//发送邮件
	{
		...
		$pr=$Avatar->fetch1("select sendmailtype,smtphost,fromemail,loginemail,emailusername,emailpassword,smtpport,emailname from {$dbtbpre}enewspublic limit 1");
		//发送初使化
	...
	$sql=$Avatar->query("select ".eReturnSelectMemberF('userid,username,havemsg,groupid,email')." from ".eReturnMemberTable()." where ".egetmf('userid').">$start".$a." order by ".egetmf('userid')." limit ".$line);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$r['userid'];
...
function SendSiteMsg($title,$msgtext,$msgtime,$userid,$username,$havemsg){
	global $Avatar,$dbtbpre;
	$isql=$Avatar->query("insert into {$dbtbpre}enewsqmsg(title,msgtext,haveread,msgtime,to_username,from_userid,from_username,isadmin,issys) values('".addslashes($title)."','".addslashes($msgtext)."',0,'$msgtime','$username',0,'',1,1);");
	if(!$havemsg)
	{
		$newhavemsg=eReturnSetHavemsg($havemsg,0);
		$usql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('havemsg')."='$newhavemsg' where ".egetmf('userid')."='".$userid."' limit 1");
	}
}
?>