...
function CopyNewTable($add,$userid,$username){
	...
	$tr=$Avatar->fetch1("select tbname,intb from {$dbtbpre}enewstable where tid='$tid'");
	if(!$tr[tbname])
	{
		printerror("EmptyTbname","");
	}
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewstable where tbname='$newtbname' limit 1");
	if($num)
	{
		printerror("ReTbname","history.go(-1)");
	}
	$sql=$Avatar->query("insert into {$dbtbpre}enewstable(tbname,tname,tsay,isdefault,datatbs,deftb,yhid,mid,intb) values('$newtbname','$add[tname]','$add[tsay]',0,',1,','1','$add[yhid]',0,'$tr[intb]');");
	$newtid=$Avatar->lastid();
	//复制表
...
function AddTable($add,$userid,$username){
	...
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewstable where tbname='$add[tbname]' limit 1");
	if($num)
	{
		printerror("ReTbname","history.go(-1)");
	}
	$sql=$Avatar->query("insert into {$dbtbpre}enewstable(tbname,tname,tsay,isdefault,datatbs,deftb,yhid,intb) values('$add[tbname]','$add[tname]','$add[tsay]',0,',1,','1','$add[yhid]','$add[intb]');");
	$tid=$Avatar->lastid();
	//初使化表
...
function EditTable($add,$userid,$username){
	...
	if($add[tbname]!=$add[oldtbname])
	{
		...
		$tbnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewstable where tbname='$add[tbname]' and tid<>$tid limit 1");
		if($tbnum)
		{
			printerror("ReTbname","history.go(-1)");
		}
		$tbr=$Avatar->fetch1("select tid,isdefault,datatbs,deftb from {$dbtbpre}enewstable where tid='$tid' limit 1");
		if(!$tbr['tid'])
		{
			printerror("EmptyTbname","history.go(-1)");
		}
		//主表
		$Avatar->query("ALTER TABLE `{$dbtbpre}ecms_".$add[oldtbname]."` RENAME `{$dbtbpre}ecms_".$add[tbname]."`;");
		//索引表
		$Avatar->query("ALTER TABLE `{$dbtbpre}ecms_".$add[oldtbname]."_index` RENAME `{$dbtbpre}ecms_".$add[tbname]."_index`;");
		//副表
		if($tbr['datatbs'])
		{
			...
			for($i=1;$i<$count-1;$i++)
			{
				$Avatar->query("ALTER TABLE `{$dbtbpre}ecms_".$add[oldtbname]."_data_".$dtbr[$i]."` RENAME `{$dbtbpre}ecms_".$add[tbname]."_data_".$dtbr[$i]."`;");
			}
		}
		//归档表
		$Avatar->query("ALTER TABLE `{$dbtbpre}ecms_".$add[oldtbname]."_doc` RENAME `{$dbtbpre}ecms_".$add[tbname]."_doc`;");
		$Avatar->query("ALTER TABLE `{$dbtbpre}ecms_".$add[oldtbname]."_doc_data` RENAME `{$dbtbpre}ecms_".$add[tbname]."_doc_data`;");
		$Avatar->query("ALTER TABLE `{$dbtbpre}ecms_".$add[oldtbname]."_doc_index` RENAME `{$dbtbpre}ecms_".$add[tbname]."_doc_index`;");
		//审核表
		$Avatar->query("ALTER TABLE `{$dbtbpre}ecms_".$add[oldtbname]."_check` RENAME `{$dbtbpre}ecms_".$add[tbname]."_check`;");
		$Avatar->query("ALTER TABLE `{$dbtbpre}ecms_".$add[oldtbname]."_check_data` RENAME `{$dbtbpre}ecms_".$add[tbname]."_check_data`;");
		//采集
	    $Avatar->query("ALTER TABLE `{$dbtbpre}ecms_infoclass_".$add[oldtbname]."` RENAME `{$dbtbpre}ecms_infoclass_".$add[tbname]."`;");
		$Avatar->query("ALTER TABLE `{$dbtbpre}ecms_infotmp_".$add[oldtbname]."` RENAME `{$dbtbpre}ecms_infotmp_".$add[tbname]."`;");
		//字段
		$Avatar->query("update {$dbtbpre}enewsf set tbname='$add[tbname]' where tid='$tid'");
		//栏目
		$Avatar->query("update {$dbtbpre}enewsclass set tbname='$add[tbname]' where tid='$tid'");
		//$Avatar->query("update {$dbtbpre}enewszt set tbname='$add[tbname]' where tid='$tid'");
		$Avatar->query("update {$dbtbpre}enewsinfoclass set tbname='$add[tbname]' where tid='$tid'");
		$Avatar->query("update {$dbtbpre}enewsmod set tbname='$add[tbname]' where tid='$tid'");
		$Avatar->query("update {$dbtbpre}enewsinfotype set tbname='$add[tbname]' where tid='$tid'");
		//搜索
		$Avatar->query("update {$dbtbpre}enewssearch set tbname='$add[tbname]' where tbname='$add[oldtbname]'");
		$Avatar->query("update {$dbtbpre}enewssearchall_load set tbname='$add[tbname]' where tbname='$add[oldtbname]'");
		//默认表
		if($tbr['isdefault'])
		{
			$Avatar->query("update {$dbtbpre}enewspublic set tbname='$add[tbname]',tid='$tid'");
		}
		//文本型
		TogSaveTxtF(1);
	}
	$sql=$Avatar->query("update {$dbtbpre}enewstable set tbname='$add[tbname]',tname='$add[tname]',tsay='$add[tsay]',yhid='$add[yhid]',intb='$add[intb]' where tid='$tid'");
	GetConfig(1);//更新缓存
	if($sql)
	{
		//操作日志
...
function DelTable($tid,$userid,$username){
	...
	//操作权限
	CheckLevel($userid,$username,$classid,"table");
	$r=$Avatar->fetch1("select tid,tbname,isdefault,datatbs,deftb from {$dbtbpre}enewstable where tid='$tid'");
	if(empty($r[tid]))
	{
		printerror("NotChangeTb","");
	}
	...
	$sql=$Avatar->query("delete from {$dbtbpre}enewstable where tid='$tid'");
	//删除数据表
	$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_".$r[tbname].";");
	$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_".$r[tbname]."_index;");
	if($r['datatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_".$r[tbname]."_data_".$dtbr[$i].";");
		}
	}
	//删除采集表
	$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_infoclass_".$r[tbname].";");
	$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_infotmp_".$r[tbname].";");
	//删除归档表
	$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_".$r[tbname]."_doc;");
	$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_".$r[tbname]."_doc_data;");
	$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_".$r[tbname]."_doc_index;");
	//删除审核表
	$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_".$r[tbname]."_check;");
	$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_".$r[tbname]."_check_data;");
	//删除数据
	$Avatar->query("delete from {$dbtbpre}enewsf where tid='$tid'");
	$Avatar->query("delete from {$dbtbpre}enewsmod where tid='$tid'");
	$Avatar->query("delete from {$dbtbpre}enewsinfoclass where tid='$tid'");
	//文本型
	TogSaveTxtF(1);
	GetConfig(1);//更新缓存
	if($sql)
...
function DefaultTable($tid,$userid,$username){
	...
	//操作权限
	CheckLevel($userid,$username,$classid,"table");
	$r=$Avatar->fetch1("select tid,tbname from {$dbtbpre}enewstable where tid='$tid'");
	if(empty($r[tid]))
	{
		printerror("NotChangeDefaultTb","history.go(-1)");
	}
	$usql=$Avatar->query("update {$dbtbpre}enewstable set isdefault=0");
	$sql=$Avatar->query("update {$dbtbpre}enewstable set isdefault=1 where tid='$tid'");
	$upsql=$Avatar->query("update {$dbtbpre}enewspublic set tbname='$r[tbname]',tid='$tid'");
	GetConfig(1);//更新缓存
	if($sql&&$usql&&$upsql)
	{
		//操作日志
...
function AddDataTable($add,$userid,$username){
	...
	$tr=$Avatar->fetch1("select tid,datatbs from {$dbtbpre}enewstable where tid='$tid'");
	if(!$tr['tid'])
	{
		printerror("EmptyDataTable","history.go(-1)");
	}
	...
	CopyEcmsTb($odtb,$dtb);
	$sql=$Avatar->query("update {$dbtbpre}enewstable set datatbs='$newdatatbs' where tid='$tid'");
	GetConfig(1);//更新缓存
	if($sql)
	{
		//操作日志
...
function DefDataTable($add,$userid,$username){
	...
	$sql=$Avatar->query("update {$dbtbpre}enewstable set deftb='$datatb' where tid='$tid'");
	GetConfig(1);//更新缓存
	if($sql)
	{
		//操作日志
...
function DelDataTable($add,$userid,$username){
	global $Avatar,$dbtbpre,$emod_r,$class_r;
	//操作权限
	CheckLevel($userid,$username,$classid,"table");
	$tid=(int)$add['tid'];
	$tbname=RepPostVar($add['tbname']);
	...
	$tr=$Avatar->fetch1("select tid,tbname,datatbs,deftb from {$dbtbpre}enewstable where tid='$tid'");
	if(!$tr['tid'])
	{
		printerror("NotChangeDataTable","history.go(-1)");
	}
	...
	$sql=$Avatar->query("update {$dbtbpre}enewstable set datatbs='$newdatatbs' where tid='$tid'");
	//删除信息
	$infosql=$Avatar->query("select * from {$dbtbpre}ecms_".$tr[tbname]." where stb='$datatb'");
	while($infor=$Avatar->fetch($infosql))
	{
		...
		if($pf)
		{
			...
			{
				$finfor=$Avatar->fetch1("select ".$pf." from {$dbtbpre}ecms_".$tr[tbname]."_data_".$datatb." where id='$infor[id]'");
				$infor[$pf]=$finfor[$pf];
			}
		}
		//存文本
		...
		//删除主表信息
		$Avatar->query("delete from {$dbtbpre}ecms_".$tr[tbname]."_index where id='$infor[id]'");
		//删除其它表记录和附件
		DelSingleInfoOtherData($infor[classid],$infor[id],$infor,0,0);
	}
	$deltb=$Avatar->query("delete from {$dbtbpre}ecms_".$tr[tbname]." where stb='$datatb'");
	//删除归档信息
	$docinfosql=$Avatar->query("select * from {$dbtbpre}ecms_".$tr[tbname]."_doc where stb='$datatb'");
	while($infor=$Avatar->fetch($docinfosql))
	{
		$mid=$class_r[$infor[classid]]['modid'];
		$pf=$emod_r[$mid]['pagef'];
		$stf=$emod_r[$mid]['savetxtf'];
		...
		if($pf)
		{
			if(strstr($emod_r[$mid]['tbdataf'],','.$pf.','))
			{
				$finfor=$Avatar->fetch1("select ".$pf." from {$dbtbpre}ecms_".$tr[tbname]."_doc_data where id='$infor[id]'");
				$infor[$pf]=$finfor[$pf];
			}
		}
		//存文本
		...
		DelNewsFile($infor[filename],$infor[newspath],$infor[classid],$infor[$pf],$infor[groupid]);
		//删除主表信息
		$Avatar->query("delete from {$dbtbpre}ecms_".$tr[tbname]."_doc_index where id='$infor[id]'");
		//删除副表信息
		$Avatar->query("delete from {$dbtbpre}ecms_".$tr[tbname]."_doc_data where id='$infor[id]'");
		//删除其它表记录与附件
		DelSingleInfoOtherData($infor['classid'],$infor['id'],$infor,0,0);
	}
	$deltb=$Avatar->query("delete from {$dbtbpre}ecms_".$tr[tbname]."_doc where stb='$datatb'");
	//删除审核信息
	$bakinfosql=$Avatar->query("select * from {$dbtbpre}ecms_".$tr[tbname]."_check where stb='$datatb'");
	while($infor=$Avatar->fetch($bakinfosql))
	{
		$mid=$class_r[$infor[classid]]['modid'];
		$pf=$emod_r[$mid]['pagef'];
		$stf=$emod_r[$mid]['savetxtf'];
		...
		if($pf)
		{
			if(strstr($emod_r[$mid]['tbdataf'],','.$pf.','))
			{
				$finfor=$Avatar->fetch1("select ".$pf." from {$dbtbpre}ecms_".$tr[tbname]."_check_data where id='$infor[id]'");
				$infor[$pf]=$finfor[$pf];
			}
		}
		//存文本
		...
		$Avatar->query("delete from {$dbtbpre}ecms_".$tr[tbname]."_index where id='$infor[id]'");
		//删除副表信息
		$Avatar->query("delete from {$dbtbpre}ecms_".$tr[tbname]."_check_data where id='$infor[id]'");
		//删除其它表记录和附件
		DelSingleInfoOtherData($infor['classid'],$infor['id'],$infor,0,0);
	}
	$deltb=$Avatar->query("delete from {$dbtbpre}ecms_".$tr[tbname]."_check where stb='$datatb'");
	//删除表
	$deltb=$Avatar->query("DROP TABLE IF EXISTS {$dbtbpre}ecms_".$tr[tbname]."_data_".$datatb.";");
	GetConfig(1);//更新缓存
	if($sql)
	{
		//操作日志
...
function CheckReTbF($add,$ecms=0){
	...
	//主表
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}ecms_".$add[tbname]);
	$b=0;
	while($r=$Avatar->fetch($s))
	{
		if($r[Field]==$add[f])
		{
			$b=1;
	...
	//副表
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}ecms_".$add[tbname]."_data_1");
	$b=0;
	while($r=$Avatar->fetch($s))
	{
		if($r[Field]==$add[f])
	...
	//索引表
	$s=$Avatar->query("SHOW FIELDS FROM {$dbtbpre}ecms_".$add[tbname]."_index");
	$b=0;
	while($r=$Avatar->fetch($s))
	{
		if($r[Field]==$add[f])
...
function AddF($add,$userid,$username){
	...
	if($add[savetxt]==1)
	{
		$txtnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsf where savetxt=1 and tid='$tid'");
		if($txtnum)
		{
			printerror('ReTxtF','');
		}
	...
	if($add['ispage']==1)
	{
		$pagenum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsf where ispage=1 and tid='$tid'");
		if($pagenum)
		{
			printerror('RePageF','');
		}
	...
	{
		$tbr=$Avatar->fetch1("select datatbs from {$dbtbpre}enewstable where tid='$tid'");
		if($tbr['datatbs'])
		{
			$dtbr=explode(',',$tbr['datatbs']);
			$count=count($dtbr);
			for($i=1;$i<$count-1;$i++)
			{
				$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." add ".$field);
				if($add[iskey]==1)//索引
				{
					$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." ADD INDEX(".$add[f].")");
				}
			}
		}
		//归档副表
		$asql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc_data add ".$field);
		if($add[iskey]==1)//索引
		{
			$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_doc_data ADD INDEX(".$add[f].")");
		}
		//审核副表
		$asql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check_data add ".$field);
		if($add[iskey]==1)//索引
		{
			$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_check_data ADD INDEX(".$add[f].")");
		}
	}
	else//主表
	{
		$asql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname." add ".$field);
		if($add[iskey]==1)//索引
		{
			$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname." ADD INDEX(".$add[f].")");
		}
		//归档主表
		$asql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc add ".$field);
		if($add[iskey]==1)//索引
		{
			$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_doc ADD INDEX(".$add[f].")");
		}
		//审核主表
		$asql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check add ".$field);
		if($add[iskey]==1)//索引
		{
			$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_check ADD INDEX(".$add[f].")");
		}
	}
	//采集表新增字段
	if($add[iscj]==1)
	{
		$asql=$Avatar->query("alter table {$dbtbpre}ecms_infoclass_".$tbname." add zz_".$add[f]." text not null,add z_".$add[f]." varchar(255) not null,add qz_".$add[f]." varchar(255) not null,add save_".$add[f]." varchar(10) not null;");
		$asql=$Avatar->query("alter table {$dbtbpre}ecms_infotmp_".$tbname." add ".$field);
	}
	//替换代码
...
function EditF($add,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"f");//验证权限
	//是否系统内部字段
	$cr=$Avatar->fetch1("select * from {$dbtbpre}enewsf where fid='$add[fid]'");
	if(empty($cr[isadd]))
	{
	...
	{
		$txtnum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsf where savetxt=1 and fid<>".$add[fid]." and tid='$tid'");
		if($txtnum)
		{
			printerror('ReTxtF','');
		}
	...
	{
		$pagenum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsf where ispage=1 and fid<>".$add[fid]." and tid='$tid'");
		if($pagenum)
		{
			printerror('RePageF','');
		}
	...
	if($cr[f]<>$add[f]||$add[iskey]<>$cr[iskey]||$cr[iscj]<>$add[iscj]||$cr[ftype]<>$add[ftype]||$cr[flen]<>$add[flen])
	{
		...
		if($cr[tbdataf]==1)//附加表
		{
			...
			if($tbr['datatbs'])
			{
				...
				for($i=1;$i<$count-1;$i++)
				{
					$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." change ".$add[oldf]." ".$field);
					if($add[iskey]==1)//索引
					{
						if($cr[iskey]==0)
						{
					...
					{
						$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." DROP INDEX ".$add[oldf]);
					}
				}
			}
			//归档副表
			$usql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc_data change ".$add[oldf]." ".$field);
			if($add[iskey]==1)//索引
			{
				if($cr[iskey]==0)
				{
			...
			elseif($cr[iskey]==1&&$add[iskey]==0)//删除索引
			{
				$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_doc_data DROP INDEX ".$add[oldf]);
			}
			//审核副表
			$usql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check_data change ".$add[oldf]." ".$field);
			if($add[iskey]==1)//索引
			{
				if($cr[iskey]==0)
				{
			...
			elseif($cr[iskey]==1&&$add[iskey]==0)//删除索引
			{
				$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_check_data DROP INDEX ".$add[oldf]);
			}
		}
		else//主表
		{
			$usql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname." change ".$add[oldf]." ".$field);
			if($add[iskey]==1)//索引
			{
				if($cr[iskey]==0)
				{
			...
			elseif($cr[iskey]==1&&$add[iskey]==0)//删除索引
			{
				$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname." DROP INDEX ".$add[oldf]);
			}
			//归档主表
			$usql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc change ".$add[oldf]." ".$field);
			if($add[iskey]==1)//索引
			{
				if($cr[iskey]==0)
				{
			...
			elseif($cr[iskey]==1&&$add[iskey]==0)//删除索引
			{
				$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_doc DROP INDEX ".$add[oldf]);
			}
			//审核主表
			$usql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check change ".$add[oldf]." ".$field);
			if($add[iskey]==1)//索引
			{
				if($cr[iskey]==0)
				{
			...
			elseif($cr[iskey]==1&&$add[iskey]==0)//删除索引
			{
				$keysql=$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_check DROP INDEX ".$add[oldf]);
			}
		}
		//采集表
		if($add[iscj]==1)
		{
			if($cr[iscj]==1)
			{
				$Avatar->query("alter table {$dbtbpre}ecms_infotmp_".$tbname." change ".$add[oldf]." ".$field);
				$Avatar->query("alter table {$dbtbpre}ecms_infoclass_".$tbname." change zz_".$add[oldf]." zz_".$add[f]." text not null,change z_".$add[oldf]." z_".$add[f]." varchar(255) not null,change qz_".$add[oldf]." qz_".$add[f]." varchar(255) not null,change save_".$add[oldf]." save_".$add[f]." varchar(10) not null;");
			}
			else
			{
				$Avatar->query("alter table {$dbtbpre}ecms_infoclass_".$tbname." add zz_".$add[f]." text not null,add z_".$add[f]." varchar(255) not null,add qz_".$add[f]." varchar(255) not null,add save_".$add[f]." varchar(10) not null;");
				$Avatar->query("alter table {$dbtbpre}ecms_infotmp_".$tbname." add ".$field);
			}
		}
		elseif($add[iscj]==0&&$cr[iscj]==1)
		{
			$Avatar->query("alter table {$dbtbpre}ecms_infotmp_".$tbname." drop COLUMN ".$cr[f]);
			$Avatar->query("alter table {$dbtbpre}ecms_infoclass_".$tbname." drop COLUMN zz_".$cr[f].",drop COLUMN z_".$cr[f].",drop COLUMN qz_".$cr[f].",drop COLUMN save_".$cr[f]);
		}
	}
	//替换代码
	...
	$sql=$Avatar->query("update {$dbtbpre}enewsf set f='$add[f]',fname='$add[fname]',fform='$add[fform]',fhtml='".eaddslashes2($fhtml)."',fzs='".eaddslashes($add[fzs])."',iscj=$add[iscj],cjhtml='".eaddslashes2($cjhtml)."',myorder=$add[myorder],ftype='$add[ftype]',flen='$add[flen]',fvalue='".eaddslashes2($add[fvalue])."',iskey=$add[iskey],tobr=$add[tobr],dohtml=$add[dohtml],qfhtml='".eaddslashes2($qfhtml)."',isonly='$add[isonly]',linkfieldval='$add[linkfieldval]',samedata='$add[samedata]',fformsize='$add[fformsize]',ispage='$add[ispage]',adddofun='$add[adddofun]',editdofun='$add[editdofun]',qadddofun='$add[qadddofun]',qeditdofun='$add[qeditdofun]',linkfieldtb='$add[linkfieldtb]',linkfieldshow='$add[linkfieldshow]',editorys='$add[editorys]',issmalltext='$add[issmalltext]',fmvnum='$add[fmvnum]' where fid='$add[fid]'");
	TogSaveTxtF(1);//公共变量
	...
	$slike=",".$add[oldf].",";
	$newslike=",".$add[f].",";
	$fsql=$Avatar->query("select mid,mtemp,cj,enter,tempvar,searchvar,tid,qenter,mustqenterf,qmtemp,listandf,listtempvar,canaddf,caneditf,orderf from {$dbtbpre}enewsmod where tid='$tid'");
	while($fr=$Avatar->fetch($fsql))
	{
		$and="";
		$enter=$fr['enter'];
		if($add[f]<>$add[oldf])
		{
			...
			if($and)
			{
				$Avatar->query("update {$dbtbpre}enewsmod set mid='$fr[mid]'".$and." where mid='$fr[mid]'");
			}
		}
...
function EditSysF($add,$userid,$username){
	...
	if($f=='title'||$f=='titlepic')
	{
		if(!empty($add['flen']))
		{
			...
			//信息表
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname." change ".$f." ".$field);
			//归档表
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc change ".$f." ".$field);
			//审核表
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check change ".$f." ".$field);
			//采集临时表
			$Avatar->query("alter table {$dbtbpre}ecms_infotmp_".$tbname." change ".$f." ".$field);
		}
	...
	if($f=='title'||$f=='titlepic')
	{
		if($iskey==1)//索引
		{
			if($add['oldiskey']==0)
			{
				$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname." ADD INDEX(".$f.")");
				$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_doc ADD INDEX(".$f.")");
				$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_check ADD INDEX(".$f.")");
			}
		}
		elseif($add['oldiskey']==1&&$iskey==0)//删除索引
		{
			$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname." DROP INDEX ".$f);
			$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_doc DROP INDEX ".$f);
			$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_check DROP INDEX ".$f);
		}
	}
	//处理变量
	$add[isonly]=(int)$add[isonly];
...
function DelF($fid,$tid,$tbname,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"f");//验证权限
	//是否系统内部字段
	$cr=$Avatar->fetch1("select isadd,f,tbdataf,iscj from {$dbtbpre}enewsf where fid='$fid'");
	if(empty($cr[isadd]))
	{
		printerror("NotIsAdd","history.go(-1)");
	}
	...
	if($cr['tbdataf']==1)
	{
		...
		if($tbr['datatbs'])
		{
			...
			for($i=1;$i<$count-1;$i++)
			{
				$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." drop COLUMN ".$cr[f]);
			}
		}
		//归档副表
		$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc_data drop COLUMN ".$cr[f]);
		//审核副表
		$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check_data drop COLUMN ".$cr[f]);
	}
	else
	{
		$usql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname." drop COLUMN ".$cr[f]);
		$usql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc drop COLUMN ".$cr[f]);
		$usql=$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check drop COLUMN ".$cr[f]);
	}
	//采集表字段
	if($cr[iscj]==1)
	{
		$usql=$Avatar->query("alter table {$dbtbpre}ecms_infotmp_".$tbname." drop COLUMN ".$cr[f]);
		$usql=$Avatar->query("alter table {$dbtbpre}ecms_infoclass_".$tbname." drop COLUMN zz_".$cr[f].",drop COLUMN z_".$cr[f].",drop COLUMN qz_".$cr[f].",drop COLUMN save_".$cr[f]);
	}
	$sql=$Avatar->query("delete from {$dbtbpre}enewsf where fid='$fid'");
	TogSaveTxtF(1);//公共变量
	//删除模型中字段项
	$record="<!--record-->";
	$field="<!--field--->";
	...
	while($r=$Avatar->fetch($dsql))
	{
		...
		if($and)
		{
			$Avatar->query("update {$dbtbpre}enewsmod set ".$and." where mid='$r[mid]'");
		}
    }
	GetConfig(1);//更新缓存
    if($sql)
...
function EditFOrder($fid,$myorder,$tid,$tbname,$userid,$username){
	...
	for($i=0;$i<count($myorder);$i++)
	{
		...
		$usql=$Avatar->query("update {$dbtbpre}enewsf set myorder=$newmyorder where fid='$fid[$i]'");
    }
...
function ChangeDataTableF($add,$userid,$username){
	global $Avatar,$dbtbpre,$public_r,$fun_r;
	//验证权限
	CheckLevel($userid,$username,$classid,"f");
	$fid=(int)$add[fid];
	$tid=(int)$add[tid];
	...
	$fr=$Avatar->fetch1("select * from {$dbtbpre}enewsf where fid='$fid'");
	if(!$fr[fid])
	{
		printerror("ErrorUrl","history.go(-1)");
	}
	...
	if(empty($start))
	{
		...
		if($fr[tbdataf])//转移到主表
		{
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname." add ".$field);
			if($fr[iskey]==1)//索引
			{
				$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname." ADD INDEX(".$fr[f].")");
			}
			//归档主表
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc add ".$field);
			if($fr[iskey]==1)//索引
			{
				$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_doc ADD INDEX(".$fr[f].")");
			}
			//审核主表
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check add ".$field);
			if($fr[iskey]==1)//索引
			{
				$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_check ADD INDEX(".$fr[f].")");
			}
		}
		else//转移到副表
		{
			...
			if($tbr['datatbs'])
			{
				...
				for($i=1;$i<$count-1;$i++)
				{
					...
					if($fr[iskey]==1)//索引
					{
						$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." ADD INDEX(".$fr[f].")");
					}
				}
			}
			//归档副表
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc_data add ".$field);
			if($fr[iskey]==1)//索引
			{
				$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_doc_data ADD INDEX(".$fr[f].")");
			}
			//审核副表
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check_data add ".$field);
			if($fr[iskey]==1)//索引
			{
				$Avatar->query("ALTER TABLE {$dbtbpre}ecms_".$tbname."_check_data ADD INDEX(".$fr[f].")");
			}
		}
	}
	$selectf='';
	...
	$b=0;
	$sql=$Avatar->query("select id,checked from {$dbtbpre}ecms_".$tbname."_index where id>$start order by id limit ".$line);
	while($r=$Avatar->fetch($sql))
	{
		...
		$infor=$Avatar->fetch1("select stb".$selectf." from ".$infotb." where id='$r[id]'");
		$infodatatb=ReturnInfoDataTbname($tbname,$r['checked'],$infor['stb']);
		if($fr[tbdataf])//副表
		{
			$finfor=$Avatar->fetch1("select ".$f." from ".$infodatatb." where id='$r[id]'");
			$value=$finfor[$f];
			$Avatar->query("update ".$infotb." set ".$f."='".StripAddsData($value)."' where id='$r[id]'");
		}
		else//主表
		{
			$value=$infor[$f];
			$Avatar->query("update ".$infodatatb." set ".$f."='".StripAddsData($value)."' where id='$r[id]'");
		}
	}
	if(empty($b))
	{
...
function ChangeDocDataTableF($add,$userid,$username){
	...
	$b=0;
	$sql=$Avatar->query("select id,stb".$selectf." from {$dbtbpre}ecms_".$tbname."_doc where id>$start order by id limit ".$line);
	while($r=$Avatar->fetch($sql))
	{
		$b=1;
		$newstart=$r['id'];
		if($fr[tbdataf])//副表
		{
			$finfor=$Avatar->fetch1("select ".$f." from {$dbtbpre}ecms_".$tbname."_doc_data where id='$r[id]'");
			$value=$finfor[$f];
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_doc set ".$f."='".StripAddsData($value)."' where id='$r[id]'");
		}
		else//主表
		{
			$value=$r[$f];
			$Avatar->query("update {$dbtbpre}ecms_".$tbname."_doc_data set ".$f."='".StripAddsData($value)."' where id='$r[id]'");
		}
	}
	if(empty($b))
	{
		...
		if($fr[tbdataf])//转移到主表
		{
			...
			if($tbr['datatbs'])
			{
				...
				for($i=1;$i<$count-1;$i++)
				{
					$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_data_".$dtbr[$i]." drop COLUMN ".$fr[f]);
				}
			}
			//归档副表
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc_data drop COLUMN ".$fr[f]);
			//审核副表
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check_data drop COLUMN ".$fr[f]);
		}
		else//转移到副表
		{
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname." drop COLUMN ".$fr[f]);
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_doc drop COLUMN ".$fr[f]);
			$Avatar->query("alter table {$dbtbpre}ecms_".$tbname."_check drop COLUMN ".$fr[f]);
		}
		$newtbdataf=$fr[tbdataf]?0:1;
		$Avatar->query("update {$dbtbpre}enewsf set tbdataf='$newtbdataf' where fid='$fid'");
		//删除模型中字段项
		if(empty($tbr['datatbs']))
		{
			...
			$slike=",".$fr[f].",";
			$dsql=$Avatar->query("select mid,searchvar,listandf,listtempvar,orderf from {$dbtbpre}enewsmod where tid='$tid' and (searchvar like '%".$slike."%' or listtempvar like '%".$like."%' or listandf like '%".$slike."%' or orderf like '%".$slike."%')");
			while($r=$Avatar->fetch($dsql))
			{
				$listtempvar="";
				$searchvar="";
...
function UpdateTbDefMod($tid,$tbname,$mid){
	global $Avatar,$dbtbpre;
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsmod where tid='$tid'");
	if($num==1)
	{
		$Avatar->query("update {$dbtbpre}enewstable set mid='$mid' where tid='$tid'");
		$Avatar->query("update {$dbtbpre}enewsmod set isdefault=1 where mid='$mid'");
	}
}
...
function ChangeMForm($mid,$tid,$mtemp){
	...
	$sql=$Avatar->query("select f,fhtml from {$dbtbpre}enewsf where tid='$tid'");
	while($r=$Avatar->fetch($sql))
	{
...
function ChangeQmForm($mid,$tid,$mtemp){
	...
	$sql=$Avatar->query("select f,qfhtml from {$dbtbpre}enewsf where tid='$tid'");
	while($r=$Avatar->fetch($sql))
	{
...
function ChangeMCj($mid,$tid,$cj){
	...
	for($i=0;$i<count($r)-1;$i++)
	{
		...
		$fr=$Avatar->fetch1("select cjhtml,fhtml from {$dbtbpre}enewsf where f='$r1[1]' and tid='$tid' limit 1");
...
function ReturnMTobrF($enter,$tid,$dof="tobr"){
	...
	$field="<!--field--->";
	$f=",";
	$sql=$Avatar->query("select f from {$dbtbpre}enewsf where ".$dof."=0 and tid='$tid'");
	while($r=$Avatar->fetch($sql))
	{
...
function AddM($add,$cname,$cchange,$schange,$center,$cqenter,$menter,$listand,$ltempf,$ptempf,$canadd,$canedit,$listorder,$userid,$username){
	...
	$sql=$Avatar->query("insert into {$dbtbpre}enewsmod(mname,mtemp,mzs,cj,enter,tempvar,sonclass,searchvar,tid,tbname,qenter,mustqenterf,qmtemp,listandf,setandf,listtempvar,qmname,canaddf,caneditf,definfovoteid,showmod,usemod,myorder,orderf,isdefault,listfile,printtempid) values('$add[mname]','".eaddslashes2($add[mtemp])."','$add[mzs]','$cj','$enter','$tempvar','','$searchvar',$tid,'$tbname','$qenter','$mustqenterf','".eaddslashes2($add[qmtemp])."','".addslashes($listandf)."','$setandf','$listtempvar','$add[qmname]','$canaddf','$caneditf',$add[definfovoteid],'$showmod','$usemod','$myorder','$orderf',0,'$listfile','$add[printtempid]');");
	$mid=$Avatar->lastid();
...
function DelM($mid,$tid,$tbname,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"m");
	$r=$Avatar->fetch1("select mname,isdefault from {$dbtbpre}enewsmod where mid='$mid'");
	$sql=$Avatar->query("delete from {$dbtbpre}enewsmod where mid='$mid'");
	$Avatar->query("delete from {$dbtbpre}enewsinfotype where mid='$mid'");//删除主题分类
	...
	if($r[isdefault])
	{
		$modr=$Avatar->fetch1("select mid from {$dbtbpre}enewsmod where tid='$tid' order by mid");
		if($modr[mid])
		{
			$Avatar->query("update {$dbtbpre}enewstable set mid='$modr[mid]' where tid='$tid'");
			$Avatar->query("update {$dbtbpre}enewsmod set isdefault=1 where mid='$modr[mid]'");
		}
	}
	GetConfig(1);//更新缓存
    if($sql)
...
function DefM($mid,$tid,$tbname,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"m");
	$r=$Avatar->fetch1("select mname from {$dbtbpre}enewsmod where mid='$mid'");
	$Avatar->query("update {$dbtbpre}enewsmod set isdefault=0 where tid='$tid'");
	$sql=$Avatar->query("update {$dbtbpre}enewsmod set isdefault=1 where mid='$mid'");
	$Avatar->query("update {$dbtbpre}enewstable set mid='$mid' where tid='$tid'");
	GetConfig(1);//更新缓存
	if($sql)
	{
...
function ChangeAllModForm($add,$userid,$username){
	...
	$sql=$Avatar->query("select mid,tid,mtemp,qmtemp,cj from {$dbtbpre}enewsmod");
	while($r=$Avatar->fetch($sql))
	{
...
function LoadInMod($add,$file,$file_name,$file_type,$file_size,$userid,$username){
	...
	//表名是否已存在
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewstable where tbname='$tbname' limit 1");
	if($num)
	{
		printerror("HaveLoadInTb","");
	}
	//上传文件
	$path=ECMS_PATH."afdglxt/data/tmp/mod/uploadm".time().make_password(10).".php";
	...
	GetConfig(1);//更新缓存
	//生成模型表单文件
	$modr=$Avatar->fetch1("select mtemp,qmtemp,cj from {$dbtbpre}enewsmod where mid='$mid'");
...
function LoadOutMod($add,$userid,$username){
	...
	$mr=$Avatar->fetch1("select * from {$dbtbpre}enewsmod where mid=$mid and tid=$tid");
	if(!$mr['mid'])
	{
		printerror("EmptyLoadMod","");
	}
	$tr=$Avatar->fetch1("select tbname,tname,tsay,intb from {$dbtbpre}enewstable where tid=$tid");
	if(!$tr['tbname'])
	{
		printerror("EmptyLoadMod","");
	}
...
".LoadModReturnstru($dbtbpre."ecms_".$mr['tbname'],$mr['tbname'],0)."\r\n";
	...
	//数据表
	$loadmod.="\$Avatar->query(\"insert into \".\$dbtbpre.\"enewstable(tbname,tname,tsay,isdefault,datatbs,deftb,yhid,mid,intb) values('\$tbname','".$tr[tname]."','".LMEscape_str($tr[tsay])."',0,',1,','1',0,0,'".$tr[intb]."');\");
\$tid=\$Avatar->lastid();
";
	//字段
	$fsql=$Avatar->query("select * from {$dbtbpre}enewsf where tid=$tid order by fid");
...
function LoadModReturnstru($table,$tb,$ecms=0){
	...
	$r=$Avatar->fetch1("SHOW CREATE TABLE `$table`;");//数据表结构
	...
	$dumpsql.="\$Avatar->query(str_replace(\"".$table."\",$reptb,SetCreateTable(\"".$create."\",\$ecms_config['db']['dbchar'])));\r\n";
	return $dumpsql;
}
