...
function ReturnAddF($modid,$rdata=0){
	...
	$record="<!--record-->";
	$field="<!--field--->";
	$mr=$Avatar->fetch1("select tempvar,enter,listandf,setandf,listtempvar from {$dbtbpre}enewsmod where mid='$modid'");
	//模板字段
	if($rdata==1)//内容
	{
...
function AddFeedback($add){
	global $Avatar,$dbtbpre,$level_r,$public_r;
	...
	//版面是否存在
	$br=$Avatar->fetch1("select bid,enter,mustenter,filef,groupid,checkboxf from {$dbtbpre}enewsfeedbackclass where bid='$bid';");
	if(empty($br['bid']))
	{
	...
	$pr=$Avatar->fetch1("select feedbacktfile,feedbackfilesize,feedbackfiletype from {$dbtbpre}enewspublic limit 1");
	//必填项
	...
	$ip=egetip();
	$eipport=egetipport();
	$sql=$Avatar->query("insert into {$dbtbpre}enewsfeedback(bid,saytime,ip,filepath,filename,userid,username,haveread,eipport".$addf.") values('$bid','$saytime','$ip','$filepath','$filename','$userid','$username',0,'$eipport'".$addval.");");
	$fid=$Avatar->lastid();
	//更新附件
...
function AddError($add){
	...
	$r=$Avatar->fetch1("select isurl,titleurl,classid,id from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where id='$id' limit 1");
	...
	$sql=$Avatar->query("insert into {$dbtbpre}enewsdownerror(id,errortext,errorip,errortime,email,classid,cid) values($id,'".addslashes($errortext)."','$ip','$errortime','".addslashes($email)."',$classid,'$cid');");
	if($sql)
	{
