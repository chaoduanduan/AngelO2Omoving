...
function Moreport_ChangeCacheAll($add,$userid,$username){
	global $Avatar,$dbtbpre,$public_r,$ecms_config,$fun_r;
	...
	$num=1;
	$b=0;
	$sql=$Avatar->query("select * from {$dbtbpre}enewsmoreport where pid>$start order by pid limit ".$num);
	while($r=$Avatar->fetch($sql))
	{
		...
		if(!$r['ppath']||!file_exists($r['ppath'].'afdglxt/config/config.php'))
		{
			continue;
		}
...
function Moreport_ChangeData($portr,$ecms=0){
	...
	if($ecms==1)//更新动态页面
	{
		...
		ReSchAlltemp();//全站搜索模板
		//防采集缓存
		$yfile=ECMS_PATH.'afdglxt/data/dbcache/notcj.php';
		$nfile=ECMS_SELFPATH.'afdglxt/data/dbcache/notcj.php';
		@copy($yfile,$nfile);
	}
	elseif($ecms==2)//清理临时文件
	{
		//临时文件目录
		$tmppath=ECMS_SELFPATH.'afdglxt/data/tmp';
	...
	elseif($ecms==3)//更新栏目缓存文件
	{
		...
		$ypath=ECMS_PATH.'afdglxt/data/fc';
		$npath=ECMS_SELFPATH.'afdglxt/data/fc';
		CopyPath($ypath,$npath);
		$ypath=ECMS_PATH.'afdglxt/data/html';
		$npath=ECMS_SELFPATH.'afdglxt/data/html';
		CopyPath($ypath,$npath);
		$ypath=ECMS_PATH.'afdglxt/data/template';
		$npath=ECMS_SELFPATH.'afdglxt/data/template';
		CopyPath($ypath,$npath);
	}
	elseif($ecms==4)//更新动态首页
	{
		if($portr['mustdt']||$public_r['indexpagedt'])
		{
			...
			@copy(ECMS_SELFPATH.'afdglxt/data/template/dtindexpage.txt',ECMS_SELFPATH.'index.php');
		}
	}
	else//更新数据库缓存
	{
