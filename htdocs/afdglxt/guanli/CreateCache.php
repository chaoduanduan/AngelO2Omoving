...
function CreateClassCache($bclassid,$exp,$expjs,$expmodjs,$adminclass,$doall,$mid,$addminfocid,$oldmid,$oldaddminfocid,$userid){
	global $Avatar,$fun_r,$dbtbpre,$public_r;
	if(empty($bclassid))
	{
		$bclassid=0;
		$exp='';
	...
	$sql=$Avatar->query("select classid,classname,bclassid,islast,classpath,classurl,listdt,sonclass,tbname,modid,myorder,onclick,openadd,wburl from {$dbtbpre}enewsclass where bclassid='$bclassid' order by myorder,classid");
	$returnr['listclass']='';
...
function HaveNavClassCache($where){
	...
	$navcachenum=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsclassnavcache where ".$where." limit 1");
	return $navcachenum;
}
...
function InsertNavClassCache($navtype,$userid,$modid){
	...
	$userid=(int)$userid;
	$modid=(int)$modid;
	$Avatar->query("insert into {$dbtbpre}enewsclassnavcache(navtype,userid,modid) values('$navtype','$userid','$modid');");
}
...
$user_r=$Avatar->fetch1("select adminclass,groupid from {$dbtbpre}enewsuser where userid='$thisuid'");
if(!$user_r['groupid'])
{
	db_close();
	$Avatar=null;
...
//用户组权限
$gr=$Avatar->fetch1("select doall from {$dbtbpre}enewsgroup where groupid='$user_r[groupid]'");
//用户
$userid=$thisuid;
if($gr['doall'])
{
...
{
	$modr=$Avatar->fetch1("select sonclass from {$dbtbpre}enewsmod where mid='$mid'");
...
{
	$oldmodr=$Avatar->fetch1("select sonclass from {$dbtbpre}enewsmod where mid='$oldmid'");
