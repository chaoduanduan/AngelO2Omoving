...
function ShopSys_DdRetext($add,$userid,$username){
	...
	$r=$Avatar->fetch1("select ddid,ddno from {$dbtbpre}enewsshopdd where ddid='$ddid'");
	if(!$r['ddid'])
	{
		printerror('ErrorUrl','');
	}
	$sql=$Avatar->query("update {$dbtbpre}enewsshopdd_add set retext='$retext' where ddid='$ddid'");
	if($sql)
	{
		$log_bz='';
		$log_addbz="";
...
function ShopSys_EditPretotal($add,$userid,$username){
	...
	$r=$Avatar->fetch1("select ddid,ddno,pretotal from {$dbtbpre}enewsshopdd where ddid='$ddid'");
	if(!$r['ddid'])
	{
		printerror('ErrorUrl','');
	}
	$sql=$Avatar->query("update {$dbtbpre}enewsshopdd set pretotal='$pretotal' where ddid='$ddid'");
	if($sql)
	{
		$log_bz=$bz;
...
function Shopsys_DoCutMaxnum($add,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$ddaddr=$Avatar->fetch1("select buycar from {$dbtbpre}enewsshopdd_add where ddid='$doddid'");
		if(empty($ddaddr['buycar']))
		{
			continue;
		}
		$ddr=$Avatar->fetch1("select havecutnum from {$dbtbpre}enewsshopdd where ddid='$doddid'");
...
function ShopSys_DdInsertLog($ddid,$ecms,$bz,$addbz){
	global $Avatar,$dbtbpre,$logininid,$loginin;
	$ddid=(int)$ddid;
	...
	$Avatar->query("insert into {$dbtbpre}enewsshop_ddlog(ddid,userid,username,ecms,bz,addbz,logtime) values('$ddid','$logininid','$loginin','$ecms','$bz','$addbz','$logtime');");
}
?>