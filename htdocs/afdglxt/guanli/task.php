...
if($ecms=='dotask')
{
	...
	$r=$Avatar->fetch1("select id,filename,lastdo,doweek,doday,dohour,dominute,userid,taskname from {$dbtbpre}enewstask where id=$id and isopen=1 limit 1");
	...
	if($lasttime)
	{
		...
		require_once($file);
		$usql=$Avatar->query("update {$dbtbpre}enewstask set lastdo='$lasttime' where id=$id");
...
elseif($ecms=='TodoTask')
{
	...
	$r=$Avatar->fetch1("select id,filename,taskname from {$dbtbpre}enewstask where id=$id limit 1");
	$file='../tasks/'.$r['filename'];
