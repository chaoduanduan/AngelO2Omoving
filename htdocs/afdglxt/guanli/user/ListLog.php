...
function DelLog($loginid,$userid,$username){
	...
	$sql=$Avatar->query("delete from {$dbtbpre}enewslog where loginid='$loginid'");
	if($sql)
	{
		//操作日志
...
function DelLog_all($loginid,$userid,$username){
	...
	$sql=$Avatar->query("delete from {$dbtbpre}enewslog where".$add);
	if($sql)
	{
		//操作日志
		insert_dolog("");
...
function DelLog_date($add,$userid,$username){
	...
	$endday=$end.' 23:59:59';
	$sql=$Avatar->query("delete from {$dbtbpre}enewslog where logintime<='$endday' and logintime>='$startday'");
	if($sql)
	{
		//操作日志
  ...
  ?>
    <tr bgcolor="#FFFFFF"> 
      <td height="25" colspan="6"> 
        <?=$returnpage?>
        &nbsp;&nbsp; <input type="submit" name="Submit" value="批量删除"> <input name="enews" type="hidden" id="caity" value="DelLog_all"> 
