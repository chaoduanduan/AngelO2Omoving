...
function DelYMemberTranFile($file,$tf,$username=''){
	...
	$count=count($r);
	$filename=$r[$count-1];
	$fr=$Avatar->fetch1("select filename,path,fileid,fpath,classid from {$dbtbpre}enewsfile_member where no='Member[".$tf."]' and filename='$filename' and adduser='[EditInfo]".$username."' limit 1");
	if($fr['fileid'])
	{
		$sql=$Avatar->query("delete from {$dbtbpre}enewsfile_member where fileid='$fr[fileid]'");
		DoDelFile($fr);
	}
}
...
function ReturnDoMemberF($fid,$add,$mr,$ecms=0,$username='',$admin=0){
	...
	$pr=$Avatar->fetch1("select openmembertranimg,memberimgsize,memberimgtype,openmembertranfile,memberfilesize,memberfiletype from {$dbtbpre}enewspublic limit 1");
	$formr=$Avatar->fetch1("select fid,enter,mustenter,filef,imgf,canaddf,caneditf,checkboxf from {$dbtbpre}enewsmemberform where fid='$fid'");
	//检测必填字段
...
function DoDelMemberF($fid,$userid,$username){
	global $Avatar,$dbtbpre;
	$r=$Avatar->fetch1("select * from {$dbtbpre}enewsmemberadd where userid='$userid'");
	$sql=$Avatar->query("delete from {$dbtbpre}enewsmemberadd where userid='$userid'");
	$sql2=$Avatar->query("delete from {$dbtbpre}enewsmemberpub where userid='$userid'");
	if(empty($r[userid]))
	{
		return '';
	}
	$formr=$Avatar->fetch1("select fid,filef,imgf from {$dbtbpre}enewsmemberform where fid='$fid'");
