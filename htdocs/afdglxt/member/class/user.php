...
function DoEpassport($ecms,$userid,$username,$password,$salt,$email,$groupid,$retime){
	...
	include_once ECMS_PATH.'afdglxt/epassport/epp_config.php';
	include_once ECMS_PATH.'afdglxt/epassport/epp_function.php';
...
function GetMemberFormId($groupid){
	...
	$groupid=(int)$groupid;
	$r=$Avatar->fetch1("select formid from {$dbtbpre}enewsmembergroup where groupid='$groupid'");
	return $r['formid'];
}
...
function GetUserEmail($userid,$username){
	global $Avatar,$dbtbpre;
	$r=$Avatar->fetch1("select ".eReturnSelectMemberF('email')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' limit 1");
	return $r['email'];
}
...
function ReturnUserInfo($userid){
	global $Avatar,$dbtbpre;
	$r=$Avatar->fetch1("select ".eReturnSelectMemberF('username,email,groupid,userfen,money,userdate,zgroupid,checked,registertime')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' limit 1");
	return $r;
}
...
function qGetLoginAuthstr($userid,$username,$rnd,$groupid,$cookietime=0){
	global $ecms_config;
	$checkpass=md5(md5($rnd.'-'.$userid.'-'.$username.'-'.$groupid).'-#Avatar.cms!-'.$ecms_config['cks']['ckrndtwo']);
...
function qCheckLoginAuthstr(){
	...
	$pass=md5(md5($re['rnd'].'-'.$re['userid'].'-'.$re['username'].'-'.$re['groupid']).'-#Avatar.cms!-'.$ecms_config['cks']['ckrndtwo']);
	if($pass!=$checkpass)
	{
		return $re;
	}
...
function islogin($uid=0,$uname='',$urnd=''){
	global $Avatar,$dbtbpre,$public_r,$ecmsreurl,$ecms_config;
	if($uid)
	{$userid=(int)$uid;}
	else
	...
	{$gotourl=$public_r['newsurl']."afdglxt/member/login/";}
	$petype=1;
	$rnd=RepPostVar($rnd);
	...
	$cr=$Avatar->fetch1("select ".eReturnSelectMemberF('userid,username,email,groupid,userfen,money,userdate,zgroupid,havemsg,checked,registertime')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' and ".egetmf('username')."='$username' and ".egetmf('rnd')."='$rnd' limit 1");
	if(!$cr['userid'])
	{
		EmptyEcmsCookie();
		if(!getcvar('returnurl'))
	...
	if(empty($cr['groupid']))
	{
		...
		$usql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('groupid')."='$user_groupid' where ".egetmf('userid')."='".$cr[userid]."'");
...
function DoEcmsMemberLogin($r,$lifetime=0){
	...
	$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('rnd')."='$rnd',".egetmf('groupid')."='$r[groupid]' where ".egetmf('userid')."='$r[userid]'");
	//设置cookie
...
function DoEcmsMemberCheckUserPass($add){
	...
	$num=0;
	$r=$Avatar->fetch1("select ".eReturnSelectMemberF('*')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username' limit 1");
	if(!$r['userid'])
	{
...
function AddInfoFen($cardfen,$userid,$checkfen=1){
	...
	if($checkfen==1)
	{
		if($cardfen<0)
		{
			$ur=$Avatar->fetch1("select ".eReturnSelectMemberF('userid,userdate,userfen')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' limit 1");
			if(!$ur['userid'])
			{
				return '';
			}
	...
	$sql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('userfen')."=".egetmf('userfen')."+".$cardfen." where ".egetmf('userid')."='$userid'");
}
...
function OutTimeZGroup($userid,$zgroupid){
	...
	if($zgroupid)
	{
		$sql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('groupid')."='".$zgroupid."',".egetmf('userdate')."=0 where ".egetmf('userid')."='$userid'");
	}
	else
	{
		$sql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('userdate')."=0 where ".egetmf('userid')."='$userid'");
	}
}
...
function eAddFenToUser($fen,$date,$groupid,$zgroupid,$user){
	...
	$sql=$Avatar->query("update ".eReturnMemberTable()." set ".$update." where ".egetmf('userid')."='".$user[userid]."'");
	if(!$sql)
	{
...
function DoCheckMDownNum($userid,$groupid,$ecms=0){
	...
	$ur=$Avatar->fetch1("select userid,todaydate,todaydown from {$dbtbpre}enewsmemberpub where userid='$userid' limit 1");
	$thetoday=date("Y-m-d");
	if($ur['userid'])
	{
...
function DoUpdateMemberAuthstr($userid,$authstr){
	global $Avatar,$dbtbpre;
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewsmemberpub where userid='$userid' limit 1");
	if($num)
	{
		$sql=$Avatar->query("update {$dbtbpre}enewsmemberpub set authstr='$authstr' where userid='$userid'");
	}
	else
	{
		$sql=$Avatar->query("replace into {$dbtbpre}enewsmemberpub(userid,authstr) values('$userid','$authstr');");
	}
	return $sql;
}
?>