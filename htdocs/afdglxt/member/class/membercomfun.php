...
function CardGetFen($username,$reusername,$card_no,$password){
	...
	$user=$Avatar->fetch1("select ".eReturnSelectMemberF('userid,userdate,username')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username' limit 1");
	if(!$user['userid'])
	{
		printerror("ExiestCardUsername","history.go(-1)",1);
	}
	$num=$Avatar->gettotal("select count(*) as total from {$dbtbpre}enewscard where card_no='".$card_no."' and password='".$password."' limit 1");
	if(!$num)
	{
	...
	$r=$Avatar->fetch1("select cardfen,money,endtime,carddate,cdgroupid,cdzgroupid from {$dbtbpre}enewscard where card_no='$card_no' limit 1");
	...
	$sql1=$Avatar->query("delete from {$dbtbpre}enewscard where card_no='$card_no'");//删除卡号
	//备份购买记录
