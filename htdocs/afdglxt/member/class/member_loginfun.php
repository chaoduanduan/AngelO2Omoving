...
function qlogin($add){
	...
	$usql=$Avatar->query("update ".eReturnMemberTable()." set ".egetmf('rnd')."='$rnd',".egetmf('groupid')."='$r[groupid]' where ".egetmf('userid')."='$r[userid]'");
	$Avatar->query("update {$dbtbpre}enewsmemberadd set lasttime='$lasttime',lastip='$lastip',loginnum=loginnum+1,lastipport='$lastipport' where userid='$r[userid]'");
	//设置cookie
	...
	if(strstr($_SERVER['HTTP_REFERER'],"afdglxt/member/iframe"))
	{
		$location="../member/iframe/";
	}
	if(strstr($location,"enews=exit")||strstr($location,"afdglxt/member/register")||strstr($_SERVER['HTTP_REFERER'],"afdglxt/member/register"))
	{
		$location="../member/cp/";
		$_POST['ecmsfrom']='';
	}
...
function qloginout($userid,$username,$rnd){
	global $Avatar,$public_r,$ecms_config;
	//是否登陆
	$user_r=islogin();
	...
	if(strstr($_SERVER['HTTP_REFERER'],"afdglxt/member/iframe"))
	{
		$gotourl=$public_r['newsurl']."afdglxt/member/iframe/";
	}
	//易通行系统
